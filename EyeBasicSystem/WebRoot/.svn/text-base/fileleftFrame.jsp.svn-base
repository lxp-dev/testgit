...
<c:if test="${not empty(itemList)}">
	<c:forEach items="${itemList}" var="item" varStatus="lineNum">
  ...
  <TR>
    <TD width=159 background=box_bg.gif colSpan=3 height=0px>
      <TABLE cellSpacing=1 cellPadding=2 width=157 border=0>
        <TBODY>
        <c:forEach items="${itemLittleList}" var="itemLittle" varStatus="lineNumLittle">
        ...
        <TR>
          ...
          onmouseout="javascript:TDOverOROut('M_${itemLittle.moduleID}');"><IMG height=7 hspace=5 
            src="${ctx}/img/sys/arrow.gif" width=5 align=bottom> 
            <c:if test="${itemLittle.moduleDirectory == null}">
            	<a href="javaScript:void(0)" target="mainFrame">${itemLittle.moduleCname}</a>
            </c:if>
            <c:if test="${itemLittle.moduleDirectory != null}">
            	<a href="${itemLittle.moduleDirectory}?moduleID=${itemLittle.moduleID }" target="mainFrame">${itemLittle.moduleCname}</a>
            </c:if>
          </TD></TR>
            </c:if>
        </c:forEach>
