...
<script>
	...
	function search(){
		...
		departmentstForm.action = "selDepartmentOpen.action?isclosed="+$('#isclosed').val();
	...
	function setValue(){ 
        ...
        {         		
          		...
          		{	
          			...
          			if($("input[id=chk][departmentid="+temp[i]+"]").val()){
          				mm=false;
          			}
	...
	function chkAll(){ 
        ...
        var companyid = $('input[id=chk]').eq(0).attr("bdpcompanysid");
...
<input type="hidden" name="readonlyopen" value="${readonlyopen}">
                    ...
                    <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        ...
                        <td width="23%" class="table_none" colspan="3">
                            <c:if test="${readonlyopen eq '1'}">
                            	    ${cpo.fcnName }&nbsp;
		                        	<input type="hidden" name="companyid" value="${cpo.fcnId }"/>
                            </c:if>
                            
                            <c:if test="${readonlyopen ne '1'}">
	                            <c:if test="${not empty(cpo) }">
	                        		${cpo.fcnName }&nbsp;
		                        	<input type="hidden" name="companyid" value="${cpo.fcnId }"/>
	                        	</c:if>
	                        	
	                        	<c:if test="${not empty(cpos) }">
	                        		<select clean="clean" id="companyid" name="companyid">
		                              <option value="">----请选择----</option>
		                              <c:forEach var="po" items="${cpos}">
		                              <option value="${po.fcnId}" ${companyid == po.fcnId ? 'selected="selected"':''}>${po.fcnName}</option>
		                              </c:forEach>
		                            </select>
	                        	</c:if>
                            </c:if>                            
                    ...
                    <table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH width="20%" scope=col>部门类型</TH>
                          <TH width="20%" scope=col>所属公司</TH>
						  </TR>
						<s:iterator value="departmentList">	
							<TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
								<TD height="26">
		                          	<input type="checkbox" id="chk" departmentid="${bdpdepartmentid}" bdpcompanysid="${bdpcompanysid}" onClick="setValue(this);" value="{'bdpdepartmentid':'${bdpdepartmentid}','bdpdepartmentname':'${bdpdepartmentname}','bdpcompanysid':'${bdpcompanysid}'}">
		                        </TD>
