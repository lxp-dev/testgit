...
<script>	
	...
	function search(){
		if(checkForm(document.all.goodsForm)){  
			...
			goodsForm.action = "selProcurementOrdersForOpen.action";
			goodsForm.submit();
			showLoadingBar();
		}
	}
	 ...
	 *  调用页面赋值
	 */
	function setValue(poID,id,name,type){ 	
		window.parent.setValue(poID,id,name,type);
		parent.hidePopWin();       
	}
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
                          ...
                          <TD class="table_none" width="24%"><input class="text_input160" id="poID" name="poID" value="${poID}"></TD>
						...
						<TR> 					 
                          ...
                          <c:if test="${categoryID == ''}">
	                          <select id="category" name="category">
								...
								<option value="1" ${category== '1' ? 'selected="selected"':''} >镜架</option>
								<option value="2" ${category== '2' ? 'selected="selected"':''} >配件</option>
								<option value="3" ${category== '3' ? 'selected="selected"':''} >成品镜片</option>
								<option value="4" ${category== '4' ? 'selected="selected"':''} >隐形镜片</option>
								<option value="5" ${category== '5' ? 'selected="selected"':''} >隐形护理液</option>
								<option value="6" ${category== '6' ? 'selected="selected"':''} >太阳镜</option>
								<option value="8" ${category== '8' ? 'selected="selected"':''} >老花镜</option>
								<option value="7" ${category== '7' ? 'selected="selected"':''} >耗材</option>
								<option value="9" ${category== '9' ? 'selected="selected"':''} >视光</option>
	                          </select>
                          </c:if>
                          ...
                          <c:if test="${categoryID != ''}">
	                          ...
	                          <select id="category" name="category" disabled="disabled">
						  		<option value="">----请选择----</option>
						  		<option value="1" ${categoryID== '1' ? 'selected="selected"':''} >镜架</option>
								<option value="2" ${categoryID== '2' ? 'selected="selected"':''} >配件</option>
								<option value="3" ${categoryID== '3' ? 'selected="selected"':''} >成品镜片</option>
								<option value="4" ${categoryID== '4' ? 'selected="selected"':''} >隐形镜片</option>
								<option value="5" ${categoryID== '5' ? 'selected="selected"':''} >隐形护理液</option>
								<option value="6" ${categoryID== '6' ? 'selected="selected"':''} >太阳镜</option>
								<option value="8" ${categoryID== '8' ? 'selected="selected"':''} >老花镜</option>
								<option value="7" ${categoryID== '7' ? 'selected="selected"':''} >耗材</option>
								<option value="9" ${categoryID== '9' ? 'selected="selected"':''} >视光</option>
	                          </select>
                          </c:if>
                          </TD>
                        </TR>
...
<img src="${ctx }/img/newbtn/btn_empty_0.png" onmouseover="JavaScript:$(this).attr('src','${ctx }/img/newbtn/btn_empty_1.png');" onmouseout="JavaScript:$(this).attr('src','${ctx }/img/newbtn/btn_empty_0.png');" title='清空' onclick="clean()" ></TD>
                        </TR>
                      </TBODY>
                    </TABLE>
                    <!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <TD>
		                    <img src="${ctx }/img/newbtn/select_0.png" title='选择' onmouseover="JavaScript:$(this).attr('src','${ctx }/img/newbtn/select_1.png');" onmouseout="JavaScript:$(this).attr('src','${ctx }/img/newbtn/select_0.png');" onClick="javascript:setValue('${po.cstpid}','${po.cstpsupplierid }','${po.bspsuppliername }','${ updatepage}')">
		                  </TD>
