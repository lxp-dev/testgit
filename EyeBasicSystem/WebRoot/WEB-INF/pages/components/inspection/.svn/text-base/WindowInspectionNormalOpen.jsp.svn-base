...
<script>	
	...
	function search(){
		...
		goodsForm.action = "selWindowInspectionStealOpen.action";
		goodsForm.submit();
		showLoadingBar();
	}
	function clean(){
	...
	function setValue(objValue){ 	
		...
		var goodsCategoryID = $('#categoryID_open').val();     
		var setwhich = $('#setwhich').val();          
		var json = {'goodsName':objValue,'xzindex':xzindex,'glassType':glassType,'goodsCategoryID':goodsCategoryID,'setwhich':setwhich};
		
		window.parent.addOption(json);
       	parent.hidePopWin();
...
<input type="hidden" name="glassType" id="glassType" value="${glassType}">
<input type="hidden" name="goodscategoryID" value="${goodscategoryID}">
<input type="hidden" name="xzindex" id="xzindex" value="${xzindex}">
<input type="hidden" name="categoryID_open" id="categoryID_open" value="${categoryID_open}">
<input type="hidden" name="setwhich" id="setwhich" value="${setwhich}">
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                    ...
                    <table id="searchBar" cellspacing="2">
						<tr height="10">
							<td>
								<img id="submitButton" title='查询' src="${ctx }/img/newbtn/btn_search_0.png" btn=btn onClick="javascript:search()">
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH width="24%" scope=col>商品代码</TH>
                          <TH width="24%" scope=col>商品品种</TH>
                          <TH width="24%" scope=col>商品名称</TH>
                          <TH width="24%" scope=col>商品价格</TH>
						  </TR>
						<s:iterator value="goodsList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">                         
                          <TD height="26"><img src="${ctx }/img/newbtn/select_0.png" btn=btn title='选择' onClick="setValue('${bgibrandname}');"></TD>
                          <TD>${bgigoodsid}</TD>
                          <TD>${bgibrandname}</TD>      
                          <TD>${bgigoodsname}</TD>     
