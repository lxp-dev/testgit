...
<script>	
	...
	function search(){
		if(checkForm(document.all.goodsForm)){
			...
			goodsForm.action = "selConsignProcessReceiptwOpen.action";
			goodsForm.submit();
			showLoadingBar();
		}
	}
	 ...
	 *  调用页面赋值
	 */
	function setValue(billID){ 	         
		window.parent.openBillValues(billID);
  ...
  <TR>
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            <TD class=menubar_title><img border='0' src='${ctx }/img/pic/module.gif' align='absmiddle' hspace='3' vspace='3'>选择委外收货单</TD>
          ...
          <TR>
            <TD class=menubar_function_text height=27><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：委外订单查询</TD>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
                          ...
                          <TD class="table_none" width="40%"><input class="text_input200" id="billID" name="billID" value="${billID}"></TD>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->
					<c:if test="${not empty(consignProcessReceiptList)}">
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                              <TBODY>
                                <TR>
                                  <TD width="5%"><div align="left"><img src="${ctx }/img/pic/queryCondition.gif" width="86" height="20" ></div></TD>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR class=table_title align=middle>
                          <TH width="25%" height="30" scope=col>单据编号</TH>
                          <TH width="25%" scope=col>关联订单号</TH>
						  <th width="15%">单据日期</th>
                          <TH width="10%" scope=col>制单人</TH>
                          <TH width="10%" scope=col>审核人</TH>
						  <TH width="10%" scope=col>选中</TH>
                        </TR>
                        <c:forEach var="po" items="${consignProcessReceiptList}">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <TD height="28">${po.cstcprreceiptbillid }</TD>
                          <TD>${po.cstcprsourcebillid}</TD>
						  <td>${fn:substring(po.cstcprbilldate,0,10)}</td>
                          <TD>${po.cstcprcreatepersonname}</TD>
                          <TD>${po.cstcprauditpersonname}</TD>
                          <TD>
		                     <input icon="icon-apply" type='button' value='选中' onClick="javascript:setValue('${po.cstcprreceiptbillid}')">
		                  </TD>
                        </TR>
                        </c:forEach>
                      </TBODY>
