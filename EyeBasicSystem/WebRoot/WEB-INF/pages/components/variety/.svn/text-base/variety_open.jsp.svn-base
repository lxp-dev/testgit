...
<script>	
	...
	function search(){
		...
		varietyForm.action = "selVarietyOpen.action";
		varietyForm.submit();
	}	
	function update(id, brandid){
		showPopWin("","initVarietyUpdate.action?hid="+id+"&brandid" + brandid ,screen.width-200,screen.height-200, '',null,true);
		selectHidden();
	}
	function insert(){
	...
	function del(id, brandid){
		showPopWin("","initVarietyDelete.action?hid="+id+"&brandid" + brandid,400,250, '',null,true);
		selectHidden();
	}	
	...
	function clean(){
	    ...
	    ${not empty(categoryID_open) ? '': 'document.all.selbvygcid.value="";'}	
	}
	...
	function setValue(){
		var json = {
			'goodsCategory' : arguments[0], 
			'brandID' : arguments[1], 
			'varietyID' : arguments[2], 
			'varietyName' :　arguments[3], 
			'supplierID': arguments[4]};
			
		window.parent.openVarietyValues(json);
    ...
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          ...
          <TR>
            <TD class=menubar_function_text height=27><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：查询品种维护</TD>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
					  	<TR>
			               ...
			               <TD width="16%" class="table_none"><input class="text_input100" type="text" name="selbvyid" value="${selbvyid }" /></TD>
					
						   <TD width="14%" height="30" class="table_body">品种名称</TD>
                          <TD width="16%" class="table_none"><input class="text_input200" name="selbvyvarietyname" value="${selbvyvarietyname }"></TD>
                      
                          <TD width="10%" class="table_body">所属品种</TD>
						   <TD width="25%" height="30" align="left" colspan="3" class="table_none">
						   <li class="horizontal_onlyRight">
						   		<input id="selbrandName" class="text_input200" id="selbrandName" name="selbrandName" value="${selbrandName }" readonly="readonly" ${not empty(brandID_open) ? 'disabled="disabled"' : '' }/>
						   		<input type="hidden" id="selbvybrandid" name="selbvybrandid" value="${selbvybrandid }" ${not empty(brandID_open) ? 'disabled="disabled"' : '' } />
						   		<input type="hidden" id="selsupplierID" name="selsupplierID" value="${selsupplierID }" ${not empty(brandID_open) ? 'disabled="disabled"' : '' } />
						   </li>
						   <li class="horizontal_onlyRight">
						  <INPUT class=button_bak id=ctl00_PageBody_Button1 icon="icon-zoom" type=button value="选 择" name=ctl00$PageBody$Button1 onClick="openBrand();" ${not empty(supplierID_open) ? 'disabled="disabled"' : '' } /></li></TD>
                        </TR>
                        <TR>
                        	<TD width="14%" height="30" class="table_body">商品类别</TD>
                          	<TD width="16%" class="table_none" colspan="7">
						  	<select name="selbvygcid" ${!empty(categoryID_open) ? 'disabled="disabled"' : '' }>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
		                  ...
		                  <TD>
		                     <input icon="icon-apply" type='button' value='选择' onClick="setValue('${po.bvygcid}', '${po.bvybrandid}', '${po.bvyid}', '${po.bvyvarietyname}', '${po.bvysupplierid}');">
		                  </TD>
                        </TR>
                        </c:forEach>
                      </TBODY>
