...
<script>
	...
	function RorL(obj){
		goodsForm.action="selectSellMirrorFrameAlls.action?rl="+$(obj).val();
		goodsForm.submit();
	}
                  ...
                  <c:if test="${not empty(other) && (empty(oneselfframe) || oneselfframe=='')}">            
                        <TR>
                  		...
                  		<tr align=left height="26">
		   					<td class="table_none" colspan="20" colspan="5">
		   					<input type="radio" name="rl" value="R" onclick="RorL(this)" ${rl == 'R' ? 'checked=checked':'' }/>右眼镜片&nbsp;&nbsp;&nbsp;
		   					<input type="radio" name="rl" value="L" onclick="RorL(this)" ${rl == 'L' ? 'checked=checked':'' }/>左眼镜片</td>
		   			    </tr>
		   			    </c:if>     
                      </TBODY>
                    </TABLE>
					...
					<!-- END 分页 -->
                      ...
                      <TBODY>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
						  <TD  height="26"><input id=chk type="checkbox" bgigoodsquantity="${bgigoodsquantity == '' ? '0':bgigoodsquantity}" bgiintransitgoodsnum="${empty(bgiintransitgoodsnum) ? '0':bgiintransitgoodsnum}" goodsid="${bgigoodsid}${rl}" stockid="${bgiwarehouseid }" onClick="setSingleValue(this);"  value="{'chkgoodis':'${bgigoodsid }${rl}','bgiordercycle':'${bgiordercycle }','bgigoodsid':'${bgigoodsid}','bgigoodsname':'${bgigoodsname}','bgiretailprice':'${bgiretailprice }','bgigoodsbarcode':'${bgigoodsbarcode }','bgicostprice':'${bgicostprice }','bginottaxrate':'${bginottaxrate }','rl':'${rl}','iscustomize':'${ bgiiscustomize}','bgiwarehouseid':'${ bgiwarehouseid}','bgigoodscategoryid':'${bgigoodscategoryid }','bgigoodsquantity':'${bgigoodsquantity }','maxdiscount':'${maxdiscount }','bgiintransitgoodsnum':'${empty(bgiintransitgoodsnum) ? '0':bgiintransitgoodsnum}','bgispec':'${bgispec}'}"></TD>
                          <TD>${bgigoodsid}</TD>
                          <TD>${bgigoodsname}</TD>
                          <c:if test="${systemParameterPo.fspisshowsupplierandbrand == '1'}"> 
                          <TD>${bgibrandname}</TD>
