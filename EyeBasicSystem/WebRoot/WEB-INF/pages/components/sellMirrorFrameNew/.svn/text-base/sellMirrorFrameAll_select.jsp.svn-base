...
<script>
	...
	function setSingleValue(obj){
		var bgigoodsquantity = $(obj).attr("bgigoodsquantity");
		var bgiintransitgoodsnum = $(obj).attr("bgiintransitgoodsnum");
		
		if(parseFloat(bgigoodsquantity) - parseFloat(bgiintransitgoodsnum) <= 0 && $(obj).attr("checked")){
			if(confirm("当前商品可用库存不足，是否负库存销售！")){
				isclose = "1";
			}else{
				$(obj).attr("checked","");
				isclose = "0";
	...
	function openSupplier(){
        ...
        if (typeof goodscategory == 'undefined' || '${other}' == null || '${other}' == ''){
	...
	function search(){
		...
		if(('${systemParameterPo.fspstealtheffective}' == '1' || '${systemParameterPo.fspstealtheffective}' == '2')){
			if($("select[id=othergoodscategory]").val() == '5' || $("select[id=othergoodscategory]").val() == '4' || $("input[id=goodscategory]").val() == '4' || $("input[id=goodscategory]").val() == '5'){
				goodsForm.action="selectSellMirrorFrameBatchNew.action";
			}else{
				goodsForm.action="selectSellMirrorFrameAllNew.action?orderstype="+orderstype;
			}
		}else{
			goodsForm.action="selectSellMirrorFrameAllNew.action?orderstype="+orderstype;
		}
	...
	function clean(){
		$(':input[type!=button][type!=hidden][type!=radio]').val('');
	...
	function setValue(json){
    	...
    	if (parent.$('input[name=salestype][checked]').val() == '1' && json.bgigoodscategoryid == '3'){
    		parent.alertNotBrandGlass(json);
    	}
	    ...
	    if('${systemParameterPo.fspsalestype}' == '1'){
	    	...
	    	*/
			...
			if(Number(num) > 0 || '${iscustomize }' =='D' || '${oneselfframe}'=='ZZ' || json.bgigoodscategoryid == '3'){
				if(json.bgigoodscategoryid == '3' && '${iscustomize }' != 'D' && '${oneselfframe}' != 'ZZ'){
				...
				}else if('${iscustomize }' != 'D' && '${oneselfframe}' != 'ZZ'){
					if(accAdd(1,parseFloat(parent.$('input[name=salesDetailPo.ssesditemids][value='+json.bgigoodsbarcode+']').parent().parent().parent().find("input[name=salesDetailPo.ssesdnumbers]").val())) > parseFloat(json.maxgoodsquantity)){
						alert("该商品库存不足！");
						$("input[goodsbarcode="+json.bgigoodsbarcode+"]").attr("checked","");
						return;
	...
	function RorL(obj){
        var goodsid = '';
		if (($(obj).val() == 'L' || $(obj).val() == 'R') && $('#goodsID').val() == ''){
			var goodsArray = window.parent.document.getElementsByName('salesDetailPo.ssesdsalesitemids');
			var flagsArray = window.parent.document.getElementsByName('salesDetailPo.ssesdglassflags');
			for (var i = 0; i < flagsArray.length; i++){
                if ((flagsArray[i].value == 'R' || flagsArray[i].value == 'L') && goodsArray[i].value.substring(2,4) != 'ZZ'){
                	goodsid = goodsArray[i].value.substring(0,7);
                	break;
                }
			}
		}
		if (goodsid == ''){
			goodsForm.action="selectSellMirrorFrameAllNew.action?rl="+$(obj).val();
		}else{
			goodsForm.action="selectSellMirrorFrameAllNew.action?rl="+$(obj).val()+"&goodsid="+goodsid;
		}
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
                        ...
                        <TR>
						  ...
						  <img src="${ctx }/img/newbtn/btn_change_0.png" btn=btn title="选 择" onClick="openBrand();"></li>
			               ...
			               <c:if test="${goodscategory == '1'}"> 
				               <c:if test="${systemParameterPo.fspsalestype == '1' && systemParameterPo.fspquerygoodsstorage == '2'}"> 
				               <tr>
			       		   ...
			       		   <c:if test="${((goodscategory == '2' && (other == '' || empty(other))) || goodscategory == '6' || goodscategory == '7' || goodscategory == '8' || goodscategory == '9')}"> 
				               <c:if test="${systemParameterPo.fspsalestype == '1' && systemParameterPo.fspquerygoodsstorage == '2'}"> 
				               <tr>
					               <TD height="26" class="table_body">库存状态</TD>
					               <td class="table_none" colspan="5">
                  ...
                  <c:if test="${not empty(other) && (empty(oneselfframe) || oneselfframe=='') && goodscategory == '2'}">            
                        <TR>
                           <TD width="8%" height="26" class="table_body">商品类型</TD>
			               <TD class="table_none" ${systemParameterPo.fspsalestype == '0' ? 'colspan="5"' : ''}>
                           <c:choose>
                               <c:when test="${othergoodscategory == '1'}">镜架</c:when>
                               <c:when test="${othergoodscategory == '2'}">配件</c:when>
                               <c:when test="${othergoodscategory == '3'}">镜片</c:when>
                               <c:when test="${othergoodscategory == '4'}">隐形</c:when>
                               <c:when test="${othergoodscategory == '5'}">护理液</c:when>
                               <c:when test="${othergoodscategory == '6'}">太阳镜</c:when>
                               <c:when test="${othergoodscategory == '7'}">耗材</c:when>
                               <c:when test="${othergoodscategory == '8'}">老花镜</c:when>
                               <c:when test="${othergoodscategory == '9'}">视光</c:when>
                           </c:choose>
                           <input type="hidden" id="othergoodscategory" name="othergoodscategory" value="${othergoodscategory}">
                              
                           <c:if test="${othergoodscategory == '2' && accessoryType == ''}">                       
			               ...
			               <td class="table_none" colspan="3">
			       		   ...
			       		   <c:if test="${systemParameterPo.fspsalestype == '1' && systemParameterPo.fspquerygoodsstorage == '1'}"> 
			               <TD height="26" class="table_none" colspan="5">&nbsp;</TD>
			       		   </c:if>
                        </TR>
                  </c:if>
					...
					<!-- END 分页 -->
                      ...
                      <TBODY>
                        ...
                        <TR class=table_title align=middle>
                          <TH scope=col width="3%" height="26" colspan="2">操作</TH>
