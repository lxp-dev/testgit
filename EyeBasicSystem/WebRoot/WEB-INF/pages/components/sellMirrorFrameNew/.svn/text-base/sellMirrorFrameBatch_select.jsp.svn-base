...
<script>
	...
	function setSingleValue1(goodsid,goodsbarcode,stockid){
		var obj = $("input[id=chk][goodsid="+ goodsid +"][barcode="+ goodsbarcode +"][stockid="+ stockid +"]");
		obj.click();
	...
	function search(){
		if('${other }' == '1'){
			if($("#othergoodscategory").val() == ''){
				alert("请选择商品类别！");
				$("#othergoodscategory").focus();
				return;
			}
		}
		...
		if(('${systemParameterPo.fspstealtheffective}' == '1' || '${systemParameterPo.fspstealtheffective}' == '2')){
			if($("select[id=othergoodscategory]").val() == '5' || $("select[id=othergoodscategory]").val() == '4' || $("input[id=goodscategory]").val() == '4' || $("input[id=goodscategory]").val() == '5'){
				...
				goodsForm.action="selectSellMirrorFrameAllNew.action";
			}
		}else{
			goodsForm.action="selectSellMirrorFrameAllNew.action";
		}
	...
	function RorL(obj){
		goodsForm.action="selectSellMirrorFrameBatchNew.action?rl="+$(obj).val();
		goodsForm.submit();
	}
</script>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
                  ...
                  <c:if test="${goodscategory == '5'}">
                        <c:if test="${iscustomize!='D' && systemParameterPo.fspsalestype == '1' && systemParameterPo.fspquerygoodsstorage == '2'}">
					...
					<!-- END 分页 -->
                      ...
                      <TBODY>
                        ...
                        <TR class=table_title align=middle>
                          <TH scope=col width="4%" height="26" colspan="2">操作</TH>
                          <c:choose>
                          	...
                          	<c:when test="${othergoodscategory == '5'}">
                          		<TH scope=col>商品代码</TH>
                          		<TH scope=col>商品条码</TH>
	                         	<TH scope=col>商品名称</TH>
	                          	<TH scope=col>品种</TH>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
						  <TD  height="26"><input id=chk type="checkbox" bgigoodsquantity="${bgigoodsquantity == '' ? '0':bgigoodsquantity}" bgiintransitgoodsnum="${empty(bgiintransitgoodsnum) ? '0':bgiintransitgoodsnum}" goodsid="${bgigoodsid}" stockid="${bgiwarehouseid }" barcode="${bgigoodsbarcode }" onClick="setSingleValue(this);"  value="{'bgiismutiluminosity':'${bgiismutiluminosity }','bgisungglassesfun':'${bgisungglassesfun }','bgistealthclass':'${bgistealthclass }','bgiusetype':'${bgiusetype }','bgiframematerialtype':'${bgiframematerialtype }','bgifunctionclass':'${bgifunctionclass }','bgirefractive':'${bgirefractive }','bgieyeglassmaterialtype':'${bgieyeglassmaterialtype }','bgisphul':'${bgisphul }','bgisphup':'${bgisphup }','bgicylul':'${bgicylul }','bgicylup':'${bgicylup }','bgisph':'${bgisph }','bgicyl':'${bgicyl }','bgiordercycle':'${bgiordercycle }','bgigoodsid':'${bgigoodsid}','bgigoodsname':'${bgigoodsname}','bgiretailprice':'${bgiretailprice }','bgigoodsbarcode':'${bgigoodsbarcode }','bgicostprice':'${bgicostprice }','bginottaxrate':'${bginottaxrate }','glassflag':'${rl}','iscustomize':'${ bgiiscustomize}','bgiwarehouseid':'${ bgiwarehouseid}','bgigoodscategoryid':'${bgigoodscategoryid }','bgigoodsquantity':'${bgigoodsquantity }','maxdiscount':'${maxdiscount }','guaranteeperiod':'${guaranteeperiod}','bgiintransitgoodsnum':'${empty(bgiintransitgoodsnum) ? '0':bgiintransitgoodsnum}','batch':'${batch }','customerdiscount':'${customerdiscount}','maxgoodsquantity':'','bgidefaultdiscountvalue':'${bgidefaultdiscountvalue}'}"></TD>
						  <TD  height="26"><img src="${ctx }/img/newbtn/select_0.png" btn=btn title='选择并关闭页面' onClick="javascript:setSingleValue1('${bgigoodsid}','${bgigoodsbarcode}','${bgiwarehouseid }')"></TD>
                           <c:choose>
                            <c:when test="${goodscategory == '1' && not empty(other) && othergoodscategory != '5'}">
                          	   <TD>${bgigoodsid}</TD>
                          	...
                          	<c:when test="${goodscategory == '4' && iscustomize=='0' }">
                               ...
                               <TD>${bgiretailprice}</TD>
	                        	...
	                        	<TD scope=col >
		                          	...
		                          	<c:if test="${bgiusetype=='3'}">塑型镜</c:if>	                          	
	                           </TD>
	                           <TD scope=col >
