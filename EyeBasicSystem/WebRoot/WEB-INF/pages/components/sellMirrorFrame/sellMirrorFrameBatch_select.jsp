...
<script>
	...
	function openBrand(){
		...
		if(goodscategory != ''){
			if('${other}' != null && '${other}' != ''){
				if(is_iPad()){
					showPopWin("selBrandOpen.action?categoryID_open="+$("select[name=othergoodscategory]").val(),970,screen.height-200,topRows,topCols,returnRefresh(true),false);
				}else{
					showPopWin("selBrandOpen.action?categoryID_open="+$("select[name=othergoodscategory]").val(),screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
				}
			}else{
				if(is_iPad()){
	...
	function search(){
		...
		if(('${systemParameterPo.fspstealtheffective}' == '1' || '${systemParameterPo.fspstealtheffective}' == '2')){
			if($("select[id=othergoodscategory]").val() == '5' || $("select[id=othergoodscategory]").val() == '4' || $("input[id=goodscategory]").val() == '4' || $("input[id=goodscategory]").val() == '5'){
				...
				goodsForm.action="selectSellMirrorFrameAll.action";
			}
		}else{
			goodsForm.action="selectSellMirrorFrameAll.action";
		}
	...
	function RorL(obj){
		goodsForm.action="selectSellMirrorFrameBatch.action?rl="+$(obj).val();
		goodsForm.submit();
	}
</script>
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
				   ...
				   <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        ...
                        <c:if test="${iscustomize!='D' && systemParameterPo.fspsalestype == '1' }">
                  ...
                  <c:if test="${goodscategory == '1' && empty(other) && (empty(oneselfframe) || oneselfframe=='')}">            
                        <TR>
                           <TD height="26" class="table_body">品种</TD>
			               <TD class="table_none" ${systemParameterPo.fspsalestype == '0' ? 'colspan="5"' : ''}>
                           <li class="horizontal_onlyRight">
                  ...
                  <c:if test="${not empty(other) && (empty(oneselfframe) || oneselfframe=='')}">            
                        <TR>
			               ...
			               <TD class="table_none" ${systemParameterPo.fspsalestype == '0' ? 'colspan="3"' : ''}>
                           <SELECT id="othergoodscategory" name="othergoodscategory" onchange="isshowaccessoriestype(this)">
                           	...
                           	<option value="5" ${othergoodscategory=='5' ?'selected="selected"':''}>隐形护理液</option>
                  ...
                  <c:if test="${goodscategory == '1' && not empty(other) && not empty(oneselfframe)}"> 
                  </c:if> 
                      </TBODY>
                    </TABLE>
