...
<script>
	$(document).ready(function() { 
        ...
        <c:if test="${systemParameterPo.fspisusegoodslevel eq '1'}">
			<c:if test="${not empty(personInfoPo.personDiscountDetailsPos)}">
	            <c:forEach var="po" items="${personInfoPo.personDiscountDetailsPos }">
					parent.$("#qoneprice").attr('${po.fpddgoodslevel}','${po.fpdddiscount}');
				</c:forEach>
	        </c:if>
		</c:if>
		<c:if test="${systemParameterPo.fspisusegoodslevel ne '1'}">
			parent.$("#qoneprice").attr("discount","${personInfoPo.personDiscount }");
		</c:if>
	}); 
	...
	function selectDiscount(){
		if(event.keyCode==13){
			...
			personDiscountForm.action = "selectQOnePrice.action";
			personDiscountForm.submit();
		}
	}	
	...
	function discount(){
		parent.$("[name=salesBasicPo.ssesbdiscountperson]").val('${personInfoPo.id }');
		parent.$("#isqoneprice").val("1");
		parent.hidePopWin();
                ...
                vAlign=top><DIV id=tabContent__1>
                  <DIV>		
				   <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                      <tr>
                      	<td width="%35" height="26" class="table_body" align="right">打折方式:</td>
                      	<td class="table_none">一口价</td>
                      </tr>
                      <TR id="trid1">
                      ...
                      <tr>
                      <td colspan="4" width="%100" height="26" class="table_none" align="left">
                      	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当前签批人：&nbsp;<font color="blue">${personInfoPo.personName}</font><br/>
                      	<c:if test="${systemParameterPo.fspisusegoodslevel eq '1'}">
							<c:if test="${not empty(personInfoPo.personDiscountDetailsPos)}">
	                      		<c:forEach var="po" items="${personInfoPo.personDiscountDetailsPos }">
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;商品级别:${po.fpddgoodslevelname}&nbsp;&nbsp;<font color="blue">${po.fpdddiscount}</font>
								</c:forEach>
	                      	</c:if>
	                      	<c:if test="${empty(personInfoPo.personDiscountDetailsPos)}">
	                      			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">此账号无相关折扣信息</font>
	                      	</c:if>
						</c:if>
						<c:if test="${systemParameterPo.fspisusegoodslevel ne '1'}">
							<c:if test="${not empty(personInfoPo.personDiscount)}">
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;普通折扣:&nbsp;&nbsp;<font color="blue">${personInfoPo.personDiscount }</font>
	                      	</c:if>
	                      	<c:if test="${empty(personInfoPo.personDiscount)}">
	                      			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">此账号无相关折扣信息</font>
	                      	</c:if>
						</c:if>
                      </td>
