...
<script>
	...
	function getPersonDetails(obj){
		if(event.keyCode==13){
			...
			var ssesbsalesid = document.getElementById("ssesbsalesid").value;
			if(ssesbsalesid!="")
			{
				barcodeScanForm.action="updateStartWorkingState.action?personcardid="+$("#personcardid").val();
				barcodeScanForm.submit();
			}else
			{
				barcodeScanForm.action = "updateStartWorkingAllState.action?personcardid="+$("#personcardid").val();
				barcodeScanForm.submit();
			}
		}
	}
	...
	function updateState(){
		...
		if(ssesbsalesid!="")
		{
			barcodeScanForm.action = "updateStartWorkingState.action";
			barcodeScanForm.submit();
		}else
		{
			barcodeScanForm.action = "updateStartWorkingAllState.action";
			barcodeScanForm.submit();
		}
	}
</script>
...
<input type="hidden" id="ssesbsalesid" name="ssesbsalesid" value="${ssesbsalesid }"/>
<input type="hidden" id="allsalesid" name="allsalesid" value="${allsalesid }"/>
