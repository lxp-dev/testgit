...
<script>	
	...
	function search(){
		...
		if(goodscategoryID ==''||goodscategoryID=="undefined"){
          alert('商品类别不能为空');
          document.getElementById('goodscategoryID').focus();
          return false;
	    }
		$("img").removeAttr("onclick");
		goodsForm.action = "selGoodsSingleZT.action";
		goodsForm.submit();
		showLoadingBar();
	}
	function clean(){
    ...
    function setSingleValue(obj){
        ...
        if(obj.checked==true){
        	...
        	if('${systemParameterPo.fspsalestype }' != '1'){
	        	if(accAdd(goodInfos[0].bgigoodsquantitys,goodInfos[0].bgiintransitgoodsnum) <= 0){
					alert("商品库存不足！");
					$(obj).attr("checked","");
					return;
	        	}
...
<input type="hidden" name="isshowstealth" value="${isshowstealth }"/>
<input type="hidden" name="indptid" value="${indptid }"/>
                    ...
                    <c:if test="${systemParameterPo.fspisshowsupplierandbrand == '1' || departmenttype != '1'}">     
                        ...
                        <TD width="60" height="26" class="table_body">商品类别</TD>
			               <TD class="table_none">
                            ...
                            <select id="goodscategoryID" name="goodscategoryID" onchange="category();" ${not empty(categoryID_open) ? 'disabled="disabled"' : '' }>
      		                 <option value="">----请选择----</option>
      		                 <s:iterator value="goodsCategorys">
      		                      <c:if test="${(systemParameterPo.fspstealtheffective=='1' || systemParameterPo.fspstealtheffective=='2') && bgcid != '4' && bgcid != '5' && isshowstealth == ''}">
                                  ...
                                  <c:if test="${isshowstealth == '1' || systemParameterPo.fspstealtheffective=='0'}">
				                    <option value="${bgcid}" <c:if test="${not empty goodscategoryID}"> ${goodscategoryID == bgcid ? 'selected="selected"' : '' }</c:if><c:if test="${empty goodscategoryID}">${categoryID_open == bgcid ? 'selected="selected"' : '' }</c:if>>${bgcgoodscategoryname}</option>
                                  </c:if>
	     	                 </s:iterator>
      	                    </select>
			               </TD>
			               <td class="table_body">仓位名称</td>
			               <td class="table_none">${stockname }</td>
					...
					<!-- END 分页 -->
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <c:choose>
	                          ...
	                          <TH scope=col width="6%">单位</TH>
	                           <TH scope=col width="6%">厂家色号 </TH>
	                           <TH scope=col width="6%">镜架材质 </TH>
	                           <TH scope=col width="6%">镜架尺寸</TH> 
                          	</c:when>
                          	...
                          	<c:when test="${goodscategoryID == '3'}">
	                          	...
	                          	<TH scope=col width="4%">球镜</TH>
	                           	<TH scope=col width="4%">柱镜 </TH>
	                           	<TH scope=col width="4%">下加光 </TH>
	                           	<TH scope=col width="4%">折射率</TH> 
                          	...
                          	<c:when test="${goodscategoryID == '5' }">
	                         	...
	                         	<TH scope=col width="6%">型号</TH>
	                          	<TH scope=col width="5%">单位</TH>
	                           	<TH scope=col width="6%">主容量 </TH>
	                           	<TH scope=col width="6%">次容量 </TH>
                          	</c:when>
						  ...
						  <TH scope=col width="5%">含税成本 </TH>
						  </c:if>
						        <c:if test="${not empty(jm)}">
						        <TH scope=col width="5%">批发价</TH>
						        </c:if>
						        <TH scope=col width="5%">仓位</TH>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">                         
                          <TD height="26">
                          <input type="checkbox" onClick="setSingleValue(this);" id="chk" goodsid="${bgigoodsbarcode}"
                           ...
                           'bgieyeglassmaterialtype':'${bgieyeglassmaterialtype }','bgiusetype':'${bgiusetype}','bgistealthclass':'${bgistealthclass}','bgirksj':'${bgirksj}',
                           'bgicapacity':'${bgicapacity }','bgicapacityentry':'${bgicapacityentry}','bgisuppliercolor':'${bgisuppliercolor}','bgiframematerialtypename':'${bgiframematerialtypename}','bgiwholesalepriceamount':'0.00','bginottaxrateamount':'0.00',
                           'bgisource':'${bgisource }','bgiwholesaleprice':'${bgiwholesaleprice }','bgigoodsquantitys':'${bgigoodsquantity }','bgiintransitgoodsnum':'${bgiintransitgoodsnum }','cshaaemaxquantity':'${((fn:substring(bgigoodsid,0,1) eq '4' || fn:substring(bgigoodsid,0,1) eq '5') && (systemParameterPo.fspstealtheffective == '1' || systemParameterPo.fspstealtheffective == '2')) ? (bgigoodsquantity + bgiintransitgoodsnum) : (bgigoodsquantity + bgiintransitgoodsnum) }'}">
                          </TD>
                           <c:choose>
                          	<c:when test="${goodscategoryID == '1' }">
	                          	 <TD>${bgigoodsid}</TD> 
						 ...
						 <TD>${bgicostprice}</TD>
	                           ...
	                           <c:if test="${systemParameterPo.fspintransitstorageflag == '1'}"> 	 
	                          	 <TD><a href="javascript:inTransitDetail('${bgigoodsid}','','${bgiwarehouseid}','0');">${bgiintransitgoodsnum}</a></TD>
	                           </c:if>
                          </TR>
						</s:iterator>
                      </TBODY>
