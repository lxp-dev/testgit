...
<script>	
	...
	$("img").removeAttr("onclick");
		goodsForm.action = "selGoodsSingleExchange.action";
		goodsForm.submit();
		showLoadingBar();
	}
	function clean(){
	 ...
	 *  调用页面赋值
	 */
	function setValue(id,name,counts,code){   
		var integral=document.getElementById('integral').value;
		if(parseInt(integral)<parseInt(counts))
		{
			alert("该会员积分不足兑换此商品，请重新选择商品！");
			return;
		}
		var json = {'id' : id, 'name' :　name, 'counts' : counts, 'code' : code};    
...
<input type="hidden" id="integral" name="integral" value="${integral}" />
					...
					<!-- END 分页 -->
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          <TH width="5%" height="26" scope=col>操作</TH>                        
                          <TH width="15%" scope=col>商品代码</TH>
                          <TH width="15%" scope=col>商品条码</TH>
                          <TH width="15%" scope=col>商品名称</TH>
                          <TH width="15%" scope=col>所需积分</TH>
                         
						</TR>
						<s:iterator value="goodsList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">                         
                         <TD height="26">
                           <img src="${ctx }/img/newbtn/select_0.png" btn=btn title="选择" 
                           onClick="setValue('${bgigoodsid}','${bgigoodsname}','${bgiIntegralCount}','${bgigoodsbarcode}');">
                          </TD>
                          <TD>${bgigoodsid}</TD>
                          <TD>${bgigoodsbarcode}</TD>
                          <TD>${bgigoodsname}</TD>
                          <TD>${bgiIntegralCount}</TD>                      
						</TR>
						</s:iterator>
                      </TBODY>
                    </TABLE>
