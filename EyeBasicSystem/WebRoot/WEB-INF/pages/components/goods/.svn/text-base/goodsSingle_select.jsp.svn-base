...
<script>
	...
	function search(){
		if(checkForm(goodsForm)){ 
			...
			goodsForm.action = "selGoodsSingle.action";
			goodsForm.submit();
			showLoadingBar();
		}
	}
	...
	function clean(){
	    ...
	    category1();
	    $('input[name=num0]').removeAttr('checked');
	}	

	function clean1(){
		$("#select_retail").val("${select_retail}");
		if($("#select_retail").val() == 0) {
			$("#div_retail").hide();
		} else {
			$("#div_retail").show();
		}			
...
<input type="hidden" name="isshowquantity" value="${isshowquantity }" />
<DIV>
                     ...
                     <TBODY>
                      ...
                      <TBODY>
                        ...
                        <TD width="60" height="26" class="table_body">商品类别</TD>
			               <TD class="table_none" ${(isshowquantity eq '1') ? '':'colspan=5' }>
			                <input type="hidden" id="storebatch" name="storebatch" value="${storebatch }"/>
                            <select id="goodscategoryID" name="goodscategoryID" onchange="category();cleanSB();" ${not empty(categoryID_open) ? 'disabled="disabled"' : '' }>
      		                 <option value="">----请选择----</option>
      		                 <c:if test="${storebatch == '1'}">
      		                 <s:iterator value="goodsCategorys">
      		                   <c:if test="${bgcid == '4' || bgcid == '5'}">
			               ...
			               <c:if test="${isshowquantity eq '1'}">
			               <TD height="26" class="table_body"> 库存为0<input type="checkbox" name="num0" value="1" ${num0 ne '' ? 'checked':'' }></TD>
			               <TD class="table_none" colspan="3">
                          	
                          	<select name="warehouseid">
                          	 <s:iterator value="warehouselist">
				               <option value="${bwhid}" ${(warehouseid eq bwhid) ? ' selected':''  }>${bwhwarehouseName}</option>
	     	                 </s:iterator>
                          	</select>
			               </TD>	
			               </c:if>			              
			             ...
			             <TR id="pj">	
			               <TD width="60" height="26" class="table_body">配件类型 </TD>
			               <TD class="table_none" colspan="5">
			               ...
			               <TD class="table_none">
                            <input class="text_input80" clean=clean maxlength="10"  type="text"  id="minSphjp" name="minSphjp" value="${requestScope.minSphjp}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入球镜范围！'}]">
                            	至
                            <input class="text_input80" clean=clean maxlength="10" type="text"  id="maxSphjp" name="maxSphjp" value="${requestScope.maxSphjp}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入球镜范围！'}]">
			               </TD>
			               <c:if test="${ app == 'app'}">
						   <TD height="26" class="table_body" >柱镜范围</TD>
			               <TD class="table_none" >
                            <input class="text_input80" type="text"  clean=clean maxlength="10" id="minCyljp" name="minCyljp" value="${requestScope.minCyljp}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入柱镜范围！'}]">
                            	至
                            <input class="text_input80" type="text"  clean=clean maxlength="10" id="maxCyljp" name="maxCyljp" value="${requestScope.maxCyljp}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入柱镜范围！'}]">
			               </TD>
			               <input type="hidden" id="iscustomize" name="iscustomize" value="0">
			               <TD width="8%" height="26" class="table_body">订做类型</TD>
			               <TD class="table_none">
                        ...
                        <tr id="yx">
                        	<TD height="26" class="table_body">球镜范围</TD>
			               <TD class="table_none">
                            <input class="text_input80" type="text" clean=clean maxlength="10" id="minSphyx" name="minSphyx" value="${requestScope.minSphyx}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入球镜范围！'}]">
                            	至
                            <input class="text_input80" type="text" clean=clean maxlength="10" id="maxSphyx" name="maxSphyx" value="${requestScope.maxSphyx}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入球镜范围！'}]">
			               </TD>
						   <TD height="26" class="table_body" >柱镜范围</TD>
			               <TD class="table_none">
                            <input class="text_input80" type="text" clean=clean id="minCylyx" maxlength="10" name="minCylyx" value="${requestScope.minCylyx}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入柱镜范围！'}]">
                            	至
                            <input class="text_input80" type="text" clean=clean id="maxCylyx" maxlength="10" name="maxCylyx" value="${requestScope.maxCylyx}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入柱镜范围！'}]">
			               </TD>
                         ...
                         <tr id="yx">
			               ...
			               <TD height="26" class="table_body">抛弃型分类</TD>
							...
							<select id="bgistealthclassyx" clean=clean name="bgistealthclassyx" >
                             ...
                             <option value="8" ${requestScope.bgistealthclassyx == '8' ? 'selected="selected"' : '' }>塑形镜</option>
      	                   </select>
                           </TD>
                        </tr>
                        <c:if test="${ app != 'app'}">
                        ...
                        <tr id="lhj">
			               ...
			               <TD class="table_none">
                            <input class="text_input80" type="text" clean=clean id="minSphlhj" name="minSphlhj" value="${requestScope.minSphlhj}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入老花镜度数！'}]">
                            	至
                            <input class="text_input80" type="text" clean=clean id="maxSphlhj" name="maxSphlhj" value="${requestScope.maxSphlhj}" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新输入老花镜度数！'}]">
			               </TD>
                        	<TD height="26" class="table_body">厂家色号</TD>
			               <TD class="table_none">
		               ...
		               <tr>		
			               ...
			               <TD class="table_none" colspan="5">
	                           ...
	                           <input id="select_retail" name="select_retail" type="hidden" value="${select_retail}">
                           </TD>
                           </c:if>
                           <c:if test="${departmenttype != '1'}">     
                      ...
                      <TBODY>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">                         
                          ...
                          <input type="checkbox" onClick="setSingleValue(this);" id="chk" goodsid="${bgigoodsid}"
                           ...
                           'bgisource':'${bgisource }','bgiwholesaleprice':'${bgiwholesaleprice }','bgiwhichretail':'${bgiwhichretail }','bgirksj':'${bgirksj}','bgiregistrationnum':'${bgiregistrationnum}'}">
                          </TD>
                           <c:choose>
                          	<c:when test="${goodscategoryID == '1' }">
	                          	 <TD>${bgigoodsid}</TD>
