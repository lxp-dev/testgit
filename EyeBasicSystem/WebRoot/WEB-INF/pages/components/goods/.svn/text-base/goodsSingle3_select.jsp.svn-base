...
<script>
	...
	function RorL(obj){
		goodsForm.action="selGoodsSingleWP.action?rl="+$(obj).val();
		goodsForm.submit();
	}
	...
	function search(){
		if('${storebatch }' == '1'||'${storebatch }' == '2'||'${storebatch }' == '3'){
		...
		goodsForm.action = "selGoodsSingleWP.action";
		goodsForm.submit();
		showLoadingBar();
	}
...
<input type="hidden" id='justType' name="justType" value="goodssr" />
                     ...
                     <TBODY>
                      ...
                      <TBODY>
			               ...
			               <TD class="table_none" colspan="5">
                            ...
                            <select id="goodscategoryID" name="goodscategoryID" onchange="category();" ${not empty(categoryID_open) ? 'disabled="disabled"' : '' }>
				               ...
				               <option value="${bgcid}" <c:if test="${not empty goodscategoryID}"> ${goodscategoryID == bgcid ? 'selected="selected"' : '' }</c:if><c:if test="${empty goodscategoryID}">${categoryID_open == bgcid ? 'selected="selected"' : '' }</c:if>>${bgcgoodscategoryname}</option>
      		                  ...
      		                  <c:if test="${storebatch == '3'}">
      		                 <s:iterator value="goodsCategorys">
      		                   <c:if test="${bgcid != '4'&&bgcid != '3' }">
				               <option value="${bgcid}" <c:if test="${not empty goodscategoryID}"> ${goodscategoryID == bgcid ? 'selected="selected"' : '' }</c:if><c:if test="${empty goodscategoryID}">${categoryID_open == bgcid ? 'selected="selected"' : '' }</c:if>>${bgcgoodscategoryname}</option>
	     	                   </c:if>
	     	                 </s:iterator>
      		                 </c:if>
      		                 
      		                 <c:if test="${storebatch != '1'&&storebatch != '2'&&storebatch != '3'}">
      		                 <s:iterator value="goodsCategorys">
				               <option value="${bgcid}" <c:if test="${not empty goodscategoryID}"> ${goodscategoryID == bgcid ? 'selected="selected"' : '' }</c:if><c:if test="${empty goodscategoryID}">${categoryID_open == bgcid ? 'selected="selected"' : '' }</c:if>>${bgcgoodscategoryname}</option>
	     	                 </s:iterator>
	     	                 </c:if>
		               ...
		               <tr>		
                        ...
                        <tr>
                        <TD id="LorR" height="26" class="table_none" colspan="6">
                        	<input type="radio" name="rl" value="R" onclick="RorL(this)" checked=checked ${rl == 'R' ? 'checked=checked':'' }/>右眼镜片&nbsp;&nbsp;&nbsp;
                        	<input type="radio" name="rl" value="L" onclick="RorL(this)" ${rl == 'L' ? 'checked=checked':'' }/>左眼镜片
						</TD>
                        </tr>
                      </TBODY>
                    </TABLE>
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">                         
                          <TD height="26">
                          <input type="checkbox" onClick="setSingleValue(this);" id="chk" goodsid="${bgigoodsid}${rl}"
                           value="{'rl':'${rl}','chkgoodis':'${bgigoodsid }${rl}','bgigoodsid':'${bgigoodsid}','bgigoodsbarcode':'${bgigoodsbarcode}','bgigoodsname':'${bgigoodsname}','bgiunitname':'${bgiunitname}','bgicostprice':'${bgicostprice}','bgiretailprice':'${bgiretailprice}','bgitaxrate':'${bgitaxrate}',
                           ...
                           'bgisource':'${bgisource }','bgiwholesaleprice':'${bgiwholesaleprice }','bgiwhichretail':'${bgiwhichretail }'}">
                          </TD>
                           <c:choose>
                          	<c:when test="${goodscategoryID == '1' }">
	                          	 <TD>${bgigoodsid}</TD>
