...
<script>	
	...
	function update(id,isShowBarcode){
        window.parent.openValues1(id,isShowBarcode);
        parent.hidePopWin();
	}
...
//		document.all.hid.value = id;
		...
		if(is_iPad()){
			showPopWin("allocationApplyForAppDetails.action?hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("allocationApplyForAppDetails.action?hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	$("img").removeAttr("onclick");
		goodsForm.action = "selGoodsSingleAllForApp.action";
		goodsForm.submit();
		showLoadingBar();
	}
	function clean(){
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                     ...
                     <TBODY>
                        ...
                        <c:if test="${systemParameterPo.fspisallocationcategory eq '1'}">
			               ...
			               <TD class="table_none">
                              <select clean=clean name="isWriteoffs" id="isWriteoffs">
                                    <option value="0" ${isWriteoffs == '0'  ? 'selected="selected"' : '' }>未核销</option>
                                    <option value="1" ${isWriteoffs == '1'  ? 'selected="selected"' : '' }>已核销</option>
	                          </select>
			               </TD>
					 ...
					 <table width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH width="20%" height="26" scope=col>申请单号</TH>
