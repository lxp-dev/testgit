...
<script>	
	...
	function details(id,sopecid){
		...
		if(is_iPad()){
			showPopWin("detailEyesCheckBjtr.action?hid="+id+"&sopecid="+sopecid,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("detailEyesCheckBjtr.action?hid="+id+"&sopecid="+sopecid,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function search(optometryID,optometryBasicID,flag,chuyanfuyan,personid,sopoyoneormany){
		...
		if(flag=='0'){
			if($('#personid').val()==personid){
				...
				if(is_iPad()){
					showPopWin("initRefractiveSelectBjtr.action?isfresh="+ document.getElementById("isfresh").value +"&optometryID="+optometryID+"&customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&chuyanfuyan="+sopoyoneormany,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
				}else{
					showPopWin("initRefractiveSelectBjtr.action?isfresh="+ document.getElementById("isfresh").value +"&optometryID="+optometryID+"&customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&chuyanfuyan="+sopoyoneormany,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
				}
				document.getElementById('popupTitle').innerHTML="【验光信息详细】";
			}
		}else{
	...
	function inspectionCopy(){
		...
		showPopWin("","inspectionCopyBjtr.action?oldOptometryID="+'${optometryBasics[0].optometrys[0].sopoyoptometryid}'+"&customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryBasicID="+'${optometryBasics[0].sopoboptometrybasicid}',screen.width-200,screen.height-200, '',null,true);
		selectHidden();
	}
	function insert(chuyanfuyan){
		...
		if(chuyanfuyan=='0'){
			...
			var topCols = top.document.getElementById("btmframe").cols;
			if(is_iPad()){
				showPopWin("initRefractiveInsertBjtr.action?moduleID="+'${moduleID}'+"&customerID="+'${customerInfoPo.smecicustomerid}'+"&chuyanfuyan="+chuyanfuyan+"&optometryID="+optometryID+"&isfresh="+'${customerInfoPo.smecimemberid}',970,screen.height-200,topRows,topCols,returnRefresh(true),true);
			}else{
				showPopWin("initRefractiveInsertBjtr.action?moduleID="+'${moduleID}'+"&customerID="+'${customerInfoPo.smecicustomerid}'+"&chuyanfuyan="+chuyanfuyan+"&optometryID="+optometryID+"&isfresh="+'${customerInfoPo.smecimemberid}',screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
			}
	...
	function del(optometryID,basicID){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initOptometryBasicDeleteBjtr.action?basicID=" + basicID + "&optometryID="+optometryID+"&moduleID="+'${requestScope.moduleID}',400,140,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initOptometryBasicDeleteBjtr.action?basicID=" + basicID + "&optometryID="+optometryID+"&moduleID="+'${requestScope.moduleID}',400,140,topRows,topCols,returnRefresh(true),true);
		}
	...
	function glassHistory(){
		...
		optometryBasicForm.action="selectGlassesHistoryBjtr.action?customerID="+'${customerInfoPo.smecicustomerid }';
		optometryBasicForm.submit();
	}
