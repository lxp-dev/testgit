...
<script> 
	...
	function search(){		
		...
		customerInfoForm.action = "optometryCaseSelBjtr.action";
	...
	function details(optometryID,customerID,flag,chuyanfuyan){
		...
		if(is_iPad()){
			showPopWin("initRefractiveSelectBjtr.action?optometryID="+optometryID+"&customerID="+customerID+"&moduleID="+'${moduleID}',970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initRefractiveSelectBjtr.action?optometryID="+optometryID+"&customerID="+customerID+"&moduleID="+'${moduleID}',screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
...
　　 }
	...
	function search2(optometryID,optometryBasicID,sopoyoneormany,cid){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		
		if(is_iPad()){
			showPopWin("initRefractiveSelectBjtr.action?optometryID="+optometryID+"&customerID="+cid+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&chuyanfuyan="+sopoyoneormany,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initRefractiveSelectBjtr.action?optometryID="+optometryID+"&customerID="+cid+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&chuyanfuyan="+sopoyoneormany,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		document.getElementById('popupTitle').innerHTML="【验光信息详细】";
	}
                      ...
                      <TBODY>
                        ...
                        <TR>
			               ...
			               <TD class="table_none"><select id="sopoydiffusepupil" name="sopoydiffusepupil">
			                 ...
			                 <OPTION value="4" ${sopoydiffusepupil != '10' ? '':'selected=selected'}>复验</OPTION>
			                 </select></TD>
						  ...
						  <TH width="10%" scope=col>状态</TH>
						 	...
						 	<c:forEach var="po" items="${optometryPos}">
		                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
		                          <TD>
	                       			<img src="${ctx }/img/newbtn/search_0.png" btn=btn title='详细' onClick="search2('${po.sopoyoptometryid }','${po.sopoyoptometrybasicid }','${po.sopoyoneormany }','${po.sopoycustomerid }')">
		                          </TD>
		                          ...
		                          <TD>${po.sopoydiffusepupil ne '10' ? '初验':'复验'}</TD>
		                        </TR>
                         	</c:forEach>
