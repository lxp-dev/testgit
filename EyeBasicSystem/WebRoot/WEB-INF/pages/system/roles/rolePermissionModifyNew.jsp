...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  ...
  <TR>
   ...
   <TABLE cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
       <TBODY>	
        <c:forEach items="${moduleList}" var="item" varStatus="lineNum">
	        <c:if test="${item.moduleparentid eq '0'}">	                   
	          ...
	          <TR name="displayID_${item.moduleid}" class=table_body align=middle>
	               <TD align=left width="20%" height="26">栏目名称</TD>
	               <TD align=left colSpan=8 height="26">权限名称</TD>
	          </TR>
	        </c:if>            
			<c:forEach items="${moduleList}" var="itemChild" varStatus="lineNumChild">
			<c:if test="${itemChild.moduleparentid eq item.moduleid}">
	               ...
	               <TD align=left colSpan=8>
	                  <TABLE id=ctl00_PageBody_Module_Main_ctl00_Module_Sub_ctl00_PermissionList 
	                     ...
	                     <TBODY>
	                        <TR>
	                          <TD>					
								<c:forEach items="${permissionList}" var="permissionPo" varStatus="lineNumPermission">
								   <c:if test="${permissionPo.moduleID eq itemChild.moduleid}">
							   			<input type="hidden" name="moduleParents[${lineNum.index }].moduleLowers[${lineNumChild.index }].permissionList[${lineNumPermission.index }].moduleID" value="${permissionPo.moduleID}" />
										<input type="hidden" name="moduleParents[${lineNum.index }].moduleLowers[${lineNumChild.index }].permissionList[${lineNumPermission.index }].pageValue" value="${permissionPo.pageValue }" />
										<input type="checkbox" clean=clean id='${item.moduleid}' name="moduleParents[${lineNum.index }].moduleLowers[${lineNumChild.index }].permissionList[${lineNumPermission.index }].pageKey" value="1" ${(permissionPo.pageKey=='1') ? 'checked="checked"' : '' }/>${permissionPo.pageName }&nbsp;
								   </c:if>
								</c:forEach>	
	                          </TD>
	                        </TR>
