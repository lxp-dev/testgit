...
<script>
	...
	function addRow(details){
		...
		c1.innerHTML = '<input id="chk" type="checkbox" value="" />';
		...
		c5.innerHTML = '<input type="text" class="text_input100" name="details['+ index +'].frpdprice" value="' + details.frpdprice +'" maxlength="10" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'金额不能为空！\'}, {\'Type\' : Type.Number, \'Expansion\' : {Type : Expansion.DecimalValidation, Params : [2]}, \'Message\' : \'金额格式错误！\'}]" /><label style="color:red;">&nbsp;*</label>';
		...
		c7.innerHTML = '<input type="text" class="text_input100" name="details['+ index +'].frpdamount" value="' + details.frpdamount +'" maxlength="10" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'合计不能为空！\'},{\'Type\' : Type.Number, \'Expansion\' : {\'Type\' : Expansion.DecimalValidation, Params : [2]}, \'Message\' : \'合计格式不正确！\'}]" /><label style="color:red;">&nbsp;*</label>';
    }	
    ...
    function init(){
    <c:forEach var="po" items="${details}">
   	addRow({'frpdprojectname':'${po.frpdprojectname }',
   			'frpdspec':'${po.frpdspec }',
   			'frpdunit':'${po.frpdunit }',
   			'frpdprice':'${po.frpdprice }',
           	'frpdnumber':'${po.frpdnumber }',
           	'frpdamount':'${po.frpdamount }'});
   	</c:forEach>
    }
	...
	function save(){
	 if(checkForm(registeredCategoryForm)){
		...
		registeredCategoryForm.action = "updateRegisteredCategory.action";
                    ...
                    <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
						...
						<TR>
						  ...
						  <TD height="26" class="table_body">收费类型</TD>
                          <TD class="table_none">
						  ...
						  <select id="frcamounttype" name="registeredCategoryPo.frcamounttype">
								...
								<option value="1" ${registeredCategoryPo.frcamounttype eq '1' ? 'selected':''}>验光师</option>
						  </select>
						  </TD>
			            </TR>
                    ...
                    <table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR class=table_title align=middle>
                          <TH width="3%" height="26" scope=col><input type="checkbox" id="chks" onclick="chkAll();">全选</TH>                        
					...
					<TABLE id=ctl00_PageBody_PostButton cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
                      <TBODY>                        
						<TR>
                          ...
                          <!-- <input icon='icon-add-row' type='button' value='添加' onClick="addToSelOne('functionName','sel1','sel2','请保存后再进行添加')"> -->
							<img src="${ctx}/img/newbtn/btn_save_0.png" btn=btn id="submitButton" title='保存' onClick="save()">  
						<!--	<input icon='icon-retry' type='button' value='重置' onClick="reset()"> -->  
                          </TD>
                        </TR>
                      </TBODY>
                    </TABLE>
