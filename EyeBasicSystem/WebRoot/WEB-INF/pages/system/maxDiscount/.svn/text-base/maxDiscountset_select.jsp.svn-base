...
<script>	
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initMaxDiscountSetUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initMaxDiscountSetUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【最大折扣设置修改】";
	}
	
	function search(){
		$("img").removeAttr("onclick");
		maxDiscountFrm.action = "maxDiscountSetSel.action";
	...
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initMaxDiscountSetInsert.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initMaxDiscountSetInsert.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【最大折扣设置新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		showPopWin("initMaxDiscountSetDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,140,topRows,topCols,returnRefresh(true),true);
		document.getElementById('popupTitle').innerHTML="【最大折扣设置删除】";
	}
	...
	function batchUpdate(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initMaxDiscountSetBatchUpdate.action?hid="+billID,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initMaxDiscountSetBatchUpdate.action?hid="+billID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
    ...
    function batchDetele(){    		
		...
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		showPopWin("initMaxDiscountSetBatchDelete.action?hid="+billID,450,140,topRows,topCols,returnRefresh(true),true);
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：最大折扣设置</TD>
            <td align="right" valign="bottom">&nbsp;
            	<c:if test="${permissionPo.keya == 1}">
            		<img src="${ctx }/img/newbtn/btn_zdzkszadd_0.png" btn=btn title="最大折扣设置新增" onclick="insert();"/>
            	</c:if>
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          <TH width="4%" scope=col><input id=chks type="checkbox" onclick="chkAll()">全选</TH>
                          ...
                          <c:if test="${systemParameterPo.fspisusegoodslevel eq '0'}">	
                          <TH width="8%" scope=col>最大折扣</TH>
                          </c:if>
						</TR>
