...
<script>
	...
	function del(){
		if(checkForm(memberManagementForm)){
		    ...
		    memberManagementForm.action = "memberManagementUpgrade.action";
...
<input type="hidden" id="hid" name="memberManagementPo.fmmid" value="${hid}">
                      ...
                      <TBODY>
						<TR height="26">       
                          ...
                          <TD width="65%" class="table_none ">
	                          <select name="memberManagementPo.fmmtypeid" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请选择会员卡类型！'}]" style="width:150">
	                             <c:if test="${not empty(list)}">
		                             <s:iterator value="list">
		                                <c:if test="${hid != fmmid && hcount-0 < fmmdown-0 }">
