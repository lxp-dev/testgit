...
<script>
	...
	function indexCreate(){
		...
		exec=1;
			$.ajax({
	            type: "POST",
	            url: "execSqlIndexRecreate.action",
    ...
    setInterval("getAjaxIndexCreateList()",5000);
    function getAjaxIndexCreateList() {
        //alert(exec);
        if(exec==1){
        	$.ajax({
                type: "POST",
                url: "getAjaxIndexCreateList.action",
                success: function(data) {
                    //alert(data);
                    if(data!=""){
                		document.getElementById('context').innerHTML=data;
                		msg_end.click(); 
                		if(data.indexOf("系统索引重建完成")!=-1){
                			exec = 0;
                		}
                    }               	    
                }
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                    ...
                    <TABLE id="title2" cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
                      <TBODY>
                        <TR>
                          <TD>
                          	<img src="${ctx}/img/newbtn/btn_save_0.png" btn=btn id="submitButton" title='执行优化' onclick="indexCreate()">
                          </TD>
                        </TR>
                      </TBODY>
                    </TABLE>
                    ...
                    <TABLE id=ctl00_PageBody_PostButton cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
                      <TBODY>
                        <TR>
                          <TD>
                          	<div id="context"></div>
                          	<a id="msg_end" name="1" href="#1">&nbsp</a></div> 
                          </TD>
                        </TR>
                      </TBODY>
                    </TABLE>
