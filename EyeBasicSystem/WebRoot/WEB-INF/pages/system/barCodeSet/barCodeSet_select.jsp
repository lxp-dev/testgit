...
<script>	
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initBarCodeSetUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initBarCodeSetUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【商品条码批号设置修改】";
	}
	
	function search(){
		$("img").removeAttr("onclick");
		maxDiscountFrm.action = "barCodeSetSel.action";
	...
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initBarCodeSetInsert.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initBarCodeSetInsert.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【商品条码批号设置新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		showPopWin("initBarCodeSetDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,140,topRows,topCols,returnRefresh(true),true);
		document.getElementById('popupTitle').innerHTML="【商品条码批号设置删除】";
	}
	...
	function batchUpdate(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initBarCodeBatchSetUpdate.action?hid="+billID,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initBarCodeBatchSetUpdate.action?hid="+billID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
    ...
    function batchDetele(){    		
		...
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		showPopWin("initBarCodeBatchSetDelete.action?hid="+billID,450,140,topRows,topCols,returnRefresh(true),true);
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：商品条码批号设置 </TD>
            <td align="right" valign="bottom">&nbsp;
            	<c:if test="${permissionPo.keya == 1}">
            		<img src="${ctx }/img/newbtn/btn_zdzkszadd_0.png" btn=btn title="商品条码批号设置新增" onclick="insert();"/>
            	</c:if>
				   ...
				   <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                    ...
                    <table id="title2" cellspacing="2">
						<tr height="10">
							...
							<c:if test="${permissionPo.keyc=='1' && systemParameterPo.fspisusegoodslevel eq '0'}">	
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->
					<c:if test="${not empty(barCodeSetList)}"> 
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH width="15%" scope=col>商品条码批号设置</TH>

						</TR>
						<s:iterator value="barCodeSetList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <td>
                            <input id=chk type="checkbox" value="${fbcsid}">
                          </td>
                          <TD width="4%"  height="26">
		                  	<c:if test="${permissionPo.keyc == 1}">
		                     <img src="${ctx }/img/newbtn/edit_0.png" btn=btn title='修改' onClick="javascript:update('${fbcsid}')">
		                  	</c:if>
		                  </TD>
		                  <TD  width="4%">
		                  	<c:if test="${permissionPo.keyb == 1}">
		                     <img src="${ctx }/img/newbtn/delete_0.png" btn=btn title='删除' onClick="javascript:del('${fbcsid}')">
		                  	</c:if>
		                  </TD>
                          <TD>${fbcsgoodscategoryname }</TD>
                          <TD>${fbcssuppliername }</TD>
                          <TD>${fbcsbrandname }</TD>
                          <Td>${fbcsgoodsid}</Td>
                          <TD>${fbcsgoodsname}</TD>    
                          <c:if test="${fbcsbarcodeflag eq '1'}">                      
                          <TD>已设置</TD>
                          </c:if>
						</TR>
						</s:iterator>
