...
<script>
	...
	function search(){
		...
		var ShopCodeName=document.getElementById("bdpdepartmentname").value;//部门名称初始化值
		if(BeginDate=="" || End==""){
			alert('请选择日期!');
			return false;
		}
        ...
        switch($('input[name=customAmountForm]:checked').val()){
	        case '1':
	        	...
	        	DataURL = "report.action?reportlet=sales_salesPersonManyStoreRpt.cpt&shopCode="+ShopCode+"&begDate="+BeginDate+"&endDate="+End+"&customAmountName="+EncodeUtf8(customAmountName)+"&ShopCodeName="+EncodeUtf8(ShopCodeName)+"&isShow="+isShow;    
	            break;
	        case '2':
	        	customAmountName="订金按应收金额计入补齐日收入,商品金额计入补齐日收入";
	        	DataURL = "report.action?reportlet=sales_salesPersonManyStoreRpt2.cpt&shopCode="+ShopCode+"&begDate="+BeginDate+"&endDate="+End+"&customAmountName="+EncodeUtf8(customAmountName)+"&ShopCodeName="+EncodeUtf8(ShopCodeName)+"&isShow="+isShow;    
	            break;
	        case '3':
	        	customAmountName="订金结款日收入,补齐金额计入补齐日收入,商品金额计入结款日收入";
	        	DataURL = "report.action?reportlet=sales_salesPersonManyStoreRpt3.cpt&shopCode="+ShopCode+"&begDate="+BeginDate+"&endDate="+End+"&customAmountName="+EncodeUtf8(customAmountName)+"&ShopCodeName="+EncodeUtf8(ShopCodeName)+"&isShow="+isShow;    
	            break;  
	        default:
		        return;
        }
		...
		document.getElementById('popupTitle').innerHTML="【营业员工作量统计报表(多店)】";
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：营业员工作量统计表(多店)</TD>
            <TD align=right>&nbsp;</TD>
          </TR>
          <TR>
