...
<script>	
	function search(){
		...
		if(begintime == "" || endtime == ""){
			...
			if ($('#typeID').val() == '1'){
			    DataURL = "report.action?reportlet=treims/ST_StealthCheckRpt.cpt&begindate="+begintime+"&enddate="+endtime+"&supplierID="+supplierID+"&brandID="+brandID+"&batch="+batch;                   
			}else{
			    DataURL = "report.action?reportlet=treims/ST_StealthCheckRpt.cpt&begindate="+begintime+"&enddate="+endtime+"&supplierID="+supplierID+"&brandID="+brandID+"&batch="+batch;                   
			}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%">目前操作功能：医疗器械进货验收记录 </TD>
            <TD align=right></TD>
          </TR>
          <TR>
