...
<script>	
	function search(){
		...
		var customAmountName="";//初始化定金统计方式 
		var ShopCodeName=document.getElementById("bdpdepartmentname").value;
		var DataURL = "";
        switch($('input[name=customAmountForm]:checked').val()){
	        case '1':
	        	customAmountName="订金按应收金额计入结款日收入,商品金额计入结款日收入";
	        	DataURL="report.action?reportlet=sales_CashierManyStoreRpt.cpt&departmentid="+ShopCode+"&startTime="+BeginDate+"&endTime="+End+"&customAmountName="+EncodeUtf8(customAmountName)+"&ShopCodeName="+EncodeUtf8(ShopCodeName)+"&isShow="+isShow;  
	            break;
	        case '2':
	        	customAmountName="订金按应收金额计入补齐日收入,商品金额计入补齐日收入";
	        	DataURL="report.action?reportlet=sales_CashierManyStoreRpt2.cpt&departmentid="+ShopCode+"&startTime="+BeginDate+"&endTime="+End+"&customAmountName="+EncodeUtf8(customAmountName)+"&ShopCodeName="+EncodeUtf8(ShopCodeName)+"&isShow="+isShow;  
	            break;
	        case '3':
	        	customAmountName="订金结款日收入,补齐金额计入补齐日收入,商品金额计入结款日收入";
	        	DataURL="report.action?reportlet=sales_CashierManyStoreRpt3.cpt&departmentid="+ShopCode+"&startTime="+BeginDate+"&endTime="+End+"&customAmountName="+EncodeUtf8(customAmountName)+"&ShopCodeName="+EncodeUtf8(ShopCodeName)+"&isShow="+isShow;  
	            break;  
	        default:
		        return;
        }
		...
		document.getElementById('popupTitle').innerHTML="【收银员工作量统计报表(多店)】";
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：收银员工作量统计表(多店)</TD>
            <TD align=right>&nbsp;</TD>
          </TR>
          <TR>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
		               ...
		               <c:if test="${person.departmenttype!=1}">
						   ...
						   <c:if test="${person.departmenttype==1}">
                            ${person.bdpdepartmentname}<input type="hidden" name="departmentID" value="${person.departmentID}" id="departmentID"/>
                            <input class="text_input300" id="bdpdepartmentname" name="personInfoPo.bdpdepartmentname" value="${personInfoPo.bdpdepartmentname }" type="hidden" />
      	                   </c:if>
                        ...
                        <TR>
			               ...
			               <TD class="table_none" colspan="5">
                               <input type="radio" name="customAmountForm" id="customAmount1" value="1" checked="checked">订金按应收金额计入结款日收入,商品金额计入结款日收入<br/>
                               <input type="radio" name="customAmountForm" id="customAmount2" value="2">订金按应收金额计入补齐日收入,商品金额计入补齐日收入<br/>
                               <input type="radio" name="customAmountForm" id="customAmount3" value="3">订金结款日收入,补齐金额计入补齐日收入,商品金额计入结款日收入<br/>
			               </TD>
                        </TR>
                                                <TR>
