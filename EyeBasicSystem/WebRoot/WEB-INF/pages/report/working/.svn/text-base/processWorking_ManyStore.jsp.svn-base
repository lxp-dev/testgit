...
<script>	
	function search(){
		...
		var DataURL = "report.action?reportlet=treims/process_processWorkingManyStoreRpt.cpt&shopCode="+ShopCode+"&bgnDate="+BeginDate+"&endDate="+End+"&shopNames="+EncodeUtf8(shopNames)+"&isShow="+isShow; 
		...
		document.getElementById('popupTitle').innerHTML="【加工人员工作量统计表(多店)】"; 
	}
	function clean(){
		goodsForm.reset();
	}
	...
	function openDepartment(){
		...
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("initDepartmentOpen.action?isclosed=0",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initDepartmentOpen.action?isclosed=0",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：加工人员工作量统计表(多店)</TD>
            <TD align=right>&nbsp;</TD>
          </TR>
          <TR>
