...
<script>
	...
	function search(){	
        ...
        if (goodscategoryID == '1'){
        	goodsCategoryName = '镜架';
        }
        if (goodscategoryID == '2'){
        ...
        if (goodscategoryID == '5'){
        	goodsCategoryName = '护理液';
        }
        if (goodscategoryID == '6'){
        	goodsCategoryName = '太阳镜';
        }
        if (goodscategoryID == '7'){
        	goodsCategoryName = '耗材';
        }
        if (goodscategoryID == '8'){
        	goodsCategoryName = '老花镜';
        }
        if (goodscategoryID == '9'){
		...
		$("input[id=isShow]").each(function() {
		  ...
		  if(typeID=='1'){
			  if(videoType=='1'){
				  var DataURL = "report.action?reportlet=storage_storageTakeSelNum1.cpt&__bypagesize__=false&goodsCategoryID="+goodscategoryID+"&storeID="+storeID+"&supplierID="+supplierID+"&brandID="+brandID+"&beginDate="+beginDate+"&endDate="+endDate+"&taketype="+taketype+"&brandName="+EncodeUtf8(brandName)+"&storeName="+EncodeUtf8(storeName)+"&supplierName="+EncodeUtf8(supplierName)+'&isShow='+isShow+"&goodsCategoryName="+EncodeUtf8(goodsCategoryName);
			  }else{
				  var DataURL = "report.action?reportlet=storage_storageTakeSelNum2.cpt&goodsCategoryID="+goodscategoryID+"&storeID="+storeID+"&supplierID="+supplierID+"&brandID="+brandID+"&beginDate="+beginDate+"&endDate="+endDate+"&taketype="+taketype+"&brandName="+EncodeUtf8(brandName)+"&storeName="+EncodeUtf8(storeName)+"&supplierName="+EncodeUtf8(supplierName)+'&isShow='+isShow+"&goodsCategoryName="+EncodeUtf8(goodsCategoryName);
			  }			  
			  document.getElementById('popupTitle').innerHTML="【产品领用数量任意时段统计】";
		  }else{
			  if(videoType=='1'){
				  var DataURL = "report.action?reportlet=storage_storageTakeSelPrice1.cpt&__bypagesize__=false&goodsCategoryID="+goodscategoryID+"&storeID="+storeID+"&supplierID="+supplierID+"&brandID="+brandID+"&beginDate="+beginDate+"&endDate="+endDate+"&taketype="+taketype+"&brandName="+EncodeUtf8(brandName)+"&storeName="+EncodeUtf8(storeName)+"&supplierName="+EncodeUtf8(supplierName)+'&isShow='+isShow+"&goodsCategoryName="+EncodeUtf8(goodsCategoryName);			   
			  }else{
				  var DataURL = "report.action?reportlet=storage_storageTakeSelPrice2.cpt&goodsCategoryID="+goodscategoryID+"&storeID="+storeID+"&supplierID="+supplierID+"&brandID="+brandID+"&beginDate="+beginDate+"&endDate="+endDate+"&taketype="+taketype+"&brandName="+EncodeUtf8(brandName)+"&storeName="+EncodeUtf8(storeName)+"&supplierName="+EncodeUtf8(supplierName)+'&isShow='+isShow+"&goodsCategoryName="+EncodeUtf8(goodsCategoryName);			   
			  }			  
			  document.getElementById('popupTitle').innerHTML="【产品领用金额任意时段统计】";
		  }
	...
	function clean(){
		...
		document.getElementById("taketype").value = "1";
	...
	function openDepartment(){
		...
		if($("#taketype").val() == '1'){
			if(is_iPad()){
				showPopWin("selDepartmentOpen.action",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
			}else{
				showPopWin("selDepartmentOpen.action",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
			}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：产品领用数量金额任意时段统计</TD>
            <TD>&nbsp;</TD>
          </TR>
          <TR>
                      ...
                      <TBODY>
					  	<TR>
					  	   <TD width="10%" height="30" class="table_body">
					  	   		<select id="taketype" name="taketype">
