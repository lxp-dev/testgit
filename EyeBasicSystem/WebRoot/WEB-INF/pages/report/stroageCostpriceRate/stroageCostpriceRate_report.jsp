...
<script>
    ...
    function getSelectText(selectId) {
        ...
        if(userSelectIndex == 0 && !selectObj.value) {
            return selectText;
        }
	...
	function search(){		
		...
		var isShow = "";
		var goodsCategoryName = getSelectText("goodscategoryID");
		var costTypeName = getSelectText("costtype");
		var payTypeName = getSelectText("payType");
		...
		if($("#payType").val() == 1) {
			DataURL = "report.action?reportlet=stroage_stroageCostpriceRateForPayTypeRpt.cpt&__bypagesize__=false&categoryid="+goodscategoryID+"&costtype="+costtype+'&isShow='+isShow+"&goodsCategoryName="+goodsCategoryName+"&costTypeName="+costTypeName
					+"&payTypeName="+payTypeName;
		} else {
			DataURL = "report.action?reportlet=stroage_stroageCostpriceRate.cpt&__bypagesize__=false&categoryid="+goodscategoryID
					+"&costtype="+costtype+'&isShow='+isShow+"&goodsCategoryName="+goodsCategoryName+"&costTypeName="+costTypeName
					+"&payTypeName="+payTypeName;
		}
		...
		document.getElementById('popupTitle').innerHTML="【 库存产品成本资金占用率统计分析 】";
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：库存产品成本资金占用率统计分析 </TD>
            <TD>&nbsp;</TD>
          </TR>
          <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
                           <TD height="26" class="table_body" width="9%">商品类别</TD>
			               <TD class="table_none" width="15%">
	      	               ...
	      	               <TD height="26" class="table_body" width="9%">成本类型</TD>
			               <TD class="table_none" width="15%">
							   <select id="costtype" name="costtype">
					             <option value="1">含税成本</option>
					             <option value="2">不含税成本</option>
					             <option value="3">加权平均成本</option>
	      	                   </select>
	      	               </TD>
	      	                <TD height="26" class="table_body" width="9%">是否显示查询条件</TD>
			               <TD class="table_none" width="15%">
                        ...
                        <TR>
                           <TD height="26" class="table_body" width="9%">统计方式</TD>
			               <TD class="table_none" width="15%" colspan="5">
							   <select id="payType" name="payType">
	      		                 <option value="">----请选择----</option>
					             <option value="0">不采用采购结算方式查看</option>
					             <option value="1">采用采购结算方式查看</option>
	      	                   </select>
	      	               </TD>
                        </TR>
                      </TBODY>
					...
					<table width="100%">
						<tr height="50px">
							<div class="reportHelp">
								1.用&nbsp;&nbsp;&nbsp;途：根据查询条件对仓位中库存商品占总库存的比率进行分析统计。<br/>
								2.查询条件：商品类别（按类别查询商品，如镜架、镜片等）
								</div>
						</tr>
					</table>
                  </DIV>
