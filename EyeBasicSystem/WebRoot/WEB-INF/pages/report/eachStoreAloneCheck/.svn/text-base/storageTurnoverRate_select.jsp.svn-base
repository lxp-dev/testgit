...
<html>
	...
	function search(){
		if(checkForm(goodsForm)){
			...
			+"&supplierID="+supplierID+"&brandID="+brandID+"&supplierName="+EncodeUtf8(supplierName)+"&brandName="+EncodeUtf8(brandName)+'&isShow='+isShow+"&bgnretailamount="+$('#bgnretailamount2').val()+"&endretailamount="+$('#endretailamount2').val()+"&showCompanyName="+showCompanyName+"&totalPriceType="+totalPriceType+"&isShowCus="+isShowCus+"&companyID="+companyID; 
	
	        switch(searchtype){
	        case '1':
				reportName = 'L_StorageTurnoverRateForType_Storage.cpt';
				formAction = 'zzl1';
	            break;
	        case '2':
				reportName = 'L_StorageTurnoverRateForSupplier_Storage.cpt';
				formAction = 'zzl2';
	            break;
	        case '3':
				reportName = 'L_StorageTurnoverRateForBrand_Storage.cpt';
				formAction = 'zzl3';
	            break;  
	        case '4':
				reportName = 'L_StorageTurnoverRateForGoods_Storage.cpt';
				formAction = 'zzl4';
	            break; 
	        default:
		        return;
              ...
              <TR><!--?Start-->
                <TD>
                    <c:if test="${permissionPo.keya eq '1'}">      
	                    ...
	                    <TD width="80" align="center" class=tab id=tabLabel__0 background=${ctx}/img/pic/tab_active_bg.gif>按库存查询</TD>
					...
					<c:if test="${permissionPo.keyc eq '1'}">      
                        ...
                        <TD width="80" align="center" class=tab id=tabLabel__0 background=${ctx}/img/pic/tab_unactive_bg.gif onclick="JavaScript:window.location.href='initProtationRateSum.action?moduleID=${moduleID}';">按单据查询</TD>
          ...
          <TD bgColor=#ffffff>
                        ...
                        <TR>
			               ...
			               <TD class="table_none" colspan=5>
			                   <c:if test="${permissionPo.keyf eq '1'}">    
	                               <input type="radio" id="totalPriceType" name="totalPriceType" value="1" checked="checked"/>当前基础信息中最新的不含税成本
	                               <input type="radio" id="totalPriceType" name="totalPriceType" value="2"/>当前基础信息中最新的含税成本
	                               <input type="radio" id="totalPriceType" name="totalPriceType" value="3"/>当前账期的加权平均成本（如果当前账期未进行成本计算，则默认按零统计）
			...
			<c:if test="${systemParameterPo.fspreporthelpshow eq '1'}">		
					...
					<div class="reportHelp">												
                      如果需要重新汇总数据，可以进入【定时任务维护】模块，通过【商品库存周转率（按库存）】这个定时任务按照当前最新的基础信息中的成本重新汇总数据。<br/>
					</div>														
			</c:if>		
                  </DIV>
                </DIV>
