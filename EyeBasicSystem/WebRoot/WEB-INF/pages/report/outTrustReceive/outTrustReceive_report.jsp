...
$(document).ready(function() { 
	...
	function search(){
		if(checkForm(document.all.goodsForm)){
			...
			url+="eims_reporting/storage_outTrustReceiveRpt&begindate="+begindate+"&enddate="+enddate+
			...
			window.open (url, 'outTrustReceiveRpt', 'fullscreen=no, top=150,left=150, toolbar=no, menubar=no, scrollbars=yes, location=no, status=no,resizable=yes');
	...
	function openBrand(){
	    ...
	    var cstpgoodscategory = document.getElementById('goodscategoryid').value;
	    
	    if(cstpgoodscategory == ''){
	      alert('请选择采购类型');
	      return false;
	    }	
		//showPopWin("","selBrandOpen.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + document.getElementById('supplierID').value ,screen.width-200,screen.height-200, '', null, true);
		//selectHidden();
		...
		if(is_iPad()){
			showPopWin("selBrandOpen.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + document.getElementById('supplierID').value,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selBrandOpen.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + document.getElementById('supplierID').value,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：委外收货汇总表</TD>
            <TD>&nbsp;</TD>
          </TR>
          <TR>
