	...
	function search(){		
		...
		if(linkType == '1'){
			if('${person.departmenttype }' == '1' || '${person.departmenttype }' == '2'){
				reportname = "storage_stroageStoreNumOrPrice1.cpt";
			}else{
				reportname = "storage_stroageStoreNumOrPrice2.cpt";
			}
		}else{
			if('${person.departmenttype }' == '1' || '${person.departmenttype }' == '2'){
				reportname = "storage_stroageStoreNumOrPriceS1.cpt";
			}else{
				reportname = "storage_stroageStoreNumOrPriceS2.cpt";
			}
		}
		
		var DataURL = "report.action?reportlet="+reportname+"&__bypagesize__=false&begindate="+BeginDate+"&enddate="+End+"&categoryid="+goodscategoryID+"&stockid="+ShopCode+"&supplierid="+supplierID+"&brandid="+brandID+
		  "&brandName="+EncodeUtf8(brandName)+"&storeName="+EncodeUtf8(storeName)+"&supplierName="+EncodeUtf8(supplierName)+'&isShow='+isShow;
		
		document.getElementById('popupTitle').innerHTML="【加盟店调货数量及金额统计】";
  ...
  <TR>
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            <TD class=menubar_title width="15%"><img border='0' src='${ctx }/img/pic/module.gif' align='absmiddle' hspace='3' vspace='3'>重庆精益报表</TD>
            <TD width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：加盟店调货数量及金额统计</TD>
            <TD>&nbsp;</TD>
          </TR>
          <TR>
