...
<script>
	...
	function save(){
		if(checkForm(document.all.mallTypeSmallForm)){	
			...
			mallTypeSmallForm.action = "insertMallTypeSmallPo.action";
                    ...
                    <table width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>   
                        <TR>
			             ...
			             <TD class="table_none" width="20%">
						  	<select id="mtslargeid" name="mallTypeSmallPo.mtslargeid" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '商品类型不能为空！'}]">
								...
								<option value="${mtlid}">${mtlname}</option>
	     	               		</s:iterator>
							</select>
			             ...
			             <TD class="table_none" width="20%">
						  	<select id="mtsbrandid" name="mallTypeSmallPo.mtsbrandid" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '商品品种不能为空！'}]">
								...
								<option value="${mbid}">${mbname}</option>
	     	               		</s:iterator>
							</select>
							<label style="color:red;">&nbsp;*&nbsp;</label>
						 </TD>
			             ...
			             <TD class="table_none">
						  	<select id="mtsareaid" name="mallTypeSmallPo.mtsareaid">
								...
								<option value="${maid}">${maname}</option>
	     	               		</s:iterator>
							</select>
						 </TD>
						</TR>   
                        <TR>
                          ...
                          <TD class="table_none"><input class="text_input100" id="mtspriceold" name="mallTypeSmallPo.mtspriceold" value="${mallTypeSmallPo.mtspriceold }" onKeyUp="value=value.replace(/[^\d\.]/g,'')" maxlength="10" onblur="if(!isNaN(this.value)) { if($.trim(this.value)!='')this.value = new Number(this.value).toFixed(2);}" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '金额不能为空！'}, {'Type' : Type.Number, 'Formula' : '', 'Expansion' : {Type : Expansion.DecimalValidation, Params : [2]}, 'Message' : '金额格式错误！'}]"><label style="color:red;">&nbsp;*&nbsp;</label></TD>
                          <TD class="table_body" height="26">商品现价</TD>
                          <TD class="table_none" colspan="3"><input class="text_input100" id="mtspricenew" name="mallTypeSmallPo.mtspricenew" value="${mallTypeSmallPo.mtspricenew }" onKeyUp="value=value.replace(/[^\d\.]/g,'')" maxlength="10" onblur="if(!isNaN(this.value)) { if($.trim(this.value)!='')this.value = new Number(this.value).toFixed(2);}" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '金额不能为空！'}, {'Type' : Type.Number, 'Formula' : '', 'Expansion' : {Type : Expansion.DecimalValidation, Params : [2]}, 'Message' : '金额格式错误！'}]"><label style="color:red;">&nbsp;*&nbsp;</label></TD>
                        </TR> 
                        <TR>
                        ...
                        <TR>
						  ...
						  <td class="table_none" colspan="5">
                            <input type="text" class="text_input300" name="mallTypeSmallPo.mtspicurl" id="mtspicurl" value="${mallTypeSmallPo.mtspicurl}" readonly="readonly"/>
