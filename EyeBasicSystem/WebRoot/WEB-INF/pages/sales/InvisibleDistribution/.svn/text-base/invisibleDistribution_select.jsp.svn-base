...
<script>	
	...
	function search(){
		if ($('#ssesblocation').find('option').size() == 0){
            alert('暂时没有需要隐形配送的配镜单!');
            return;
		}
		$("img").removeAttr("onclick");
		invisibleDistributionForm.action = "selectInvisibleDistribution.action";
	...
	function fineReportEvent(reportFileName,showtype){
		if(checkForm(document.all.invisibleDistributionForm)){
		...
		case "1":
			...
			document.getElementById('popupTitle').innerHTML="【隐形订做配送单】";
		  break;
		case "2":
	...
	function openPersonScan(id){
		...
		var salesids = ""; 
		var ssesblocation = $("select[name=ssesblocation]").val();
		...
		if(is_iPad()){
			showPopWin("initPensonScanInvisible.action?moduleID=${moduleID}&ssesblocation="+ssesblocation,400,70,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initPensonScanInvisible.action?moduleID=${moduleID}&ssesblocation="+ssesblocation,400,70,topRows,topCols,returnRefresh(true),false);
		}
	...
	function doorStoreDelivery_JD(){
		$("img").removeAttr("onclick");
		invisibleDistributionForm.action = "initSelectedDoorStoreDelivery.action?type=3&moduleID="+$('#moduleID').val();
		invisibleDistributionForm.submit();
	}
</script>
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：隐形订做配送 </TD>
          </TR>
          <TR>
                    ...
                    <table id="searchBar" cellspacing="2" width="100%">
						<tr height="10">
						    ...
						    <TD height="26" valign="bottom" width="50%" align="right">
						    	...
						    	<img id="submitButton" src="${ctx }/img/newbtn/btn_yxsend_0.png" btn=btn title='隐形配送并调拨至门店' onClick="openPersonScan('')" >
						    </TD>
						</tr>
					</table>
					</c:if>
