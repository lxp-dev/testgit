...
　　 }
	...
	function save(){
		if(checkForm(toMailForm)) {
			...
			var regPhone = /^((1[3|5|8|7][0-9]\d{4,8})|((0{1,1}[1-9]{2,3}-)?[1-9]{1,1}[0-9]{6,7}))$/
			...
			if($('#ssetmmaiilaudit').is(":checked")){
				printReport($('#ssetmuuid').val());
			}
			</c:if>
			
			$("img").removeAttr("onclick");
			toMailForm.action = "toMailUpdate.action";
			toMailForm.submit();
		}
	}
	...
	function show(){
		...
		if($("#ssetmmailcompanyname").val()==''){
			if ($("#ssetmtomaillistid").find("option:selected").val() != ''){
...
<input type="hidden" id="ssetmuuid" name="toMailPo.ssetmuuid" value="${toMailPo.ssetmuuid }">
                ...
                vAlign=top><DIV id=tabContent__1>
                      ...
                      <TBODY>
                          ...
                          <TD width="8%" height="26" class="table_body">配镜单号</TD>
					  	  <TD width="24%" class="table_none">
					  	  	<input type="text" class="text_input160" readonly="readonly" style="background-color: #E0E0E0" id="ssetmlinksalesid" name="toMailPo.ssetmlinksalesid" value="${toMailPo.ssetmlinksalesid }"/>
					  	  </TD>
					  	  <TD width="8%" height="26" class="table_body">会员卡号</TD>
					  	  <TD width="24" class="table_none"><input type="text" readonly="readonly" class="text_input160" style="background-color: #E0E0E0" id="ssetmmemberid" name="toMailPo.ssetmmemberid" value="${toMailPo.ssetmmemberid }"/></TD>
					  	  <TD width="8%" height="26" class="table_body">顾客姓名</TD>
                    	  <TD class="table_none"><input class="text_input160" type="text"  id="ssetmcustomername" name="toMailPo.ssetmcustomername" value="${toMailPo.ssetmcustomername }" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '顾客姓名不能为空!'}]" maxlength="20"/><label style="color:red;">&nbsp;*&nbsp;</label> </TD>
					  	</TR>
                    	<TR>
                    	...
                    	<TR>
                    	  ...
                    	  <TD class="table_none" colspan="5">
                    	   <select clean=clean id="ssetmtomaillistid" name="toMailPo.ssetmtomaillistid" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '邮寄单不能为空!'}]">
                    	           ...
                    	           <OPTION value="${btmlid}" ${toMailPo.ssetmtomaillistid!= btmlid  ? '' : 'selected="selected"' } >${btmlname}</OPTION>
                    	          </s:iterator>
      	                    </select>
      	                    <label style="color:red;">&nbsp;*&nbsp;</label> 
                    	  </TD>
                  ...
                  <c:if test="${systemParameterPo.fspisbaojiaflag == '1'}">          						  
                        <TR>
                    	  ...
                    	  <TD class="table_none" colspan="5">
                    	    <input type="checkbox"  id="ssetmissupport" name="toMailPo.ssetmissupport" value="1" onclick="isshowbjv(this)" ${toMailPo.ssetmissupport == '1' ? "checked='checked'":"" }/>&nbsp;&nbsp;
                    	    <div id="bjv" ${toMailPo.ssetmissupport == '1' ? "":"style='display: none;'" }>
                    	    <input class="text_input80" type="text"  id="ssetmsupportvalue" name="toMailPo.ssetmsupportvalue" value="${toMailPo.ssetmsupportvalue }" maxlength="20" onKeyUp="value=value.replace(/[^\d\.]/g,'')" onblur="if(!isNaN(this.value)) { if(trim(this.value)!='')this.value = new Number(this.value).toFixed(2);}"/>&nbsp;元
                    	    </div>
                    	  </TD>
                  	  </TR>	
                  </c:if>
                  ...
                  <c:if test="${systemParameterPo.fspisbaojiaflag == '2'}">          						  
                        <TR>
                    	  <TD height="26" class="table_body">保价</TD>
                    	  <TD class="table_none" colspan="5">&nbsp;&nbsp;${toMailPo.ssetmsupportvalue }&nbsp;元
                    	    <input type="hidden" id="ssetmissupport" name="toMailPo.ssetmissupport" value="1" readonly="readonly"/>
                    	    <input type="hidden" id="ssetmsupportvalue" name="toMailPo.ssetmsupportvalue" value="${toMailPo.ssetmsupportvalue }" readonly="readonly"/>
                    	  </TD>
                  	  </TR>	
                  </c:if>
                    ...
                    <TABLE id="title2" cellspacing="2">
	                   	<TR>
	                        <TD align="left"><input type="checkbox" id="ssetmmaiilaudit" name="toMailPo.ssetmmaiilaudit" value="1" onclick="show()">保存并邮寄</TD>
					  	</TR>
						<tr height="10">
							<td>
