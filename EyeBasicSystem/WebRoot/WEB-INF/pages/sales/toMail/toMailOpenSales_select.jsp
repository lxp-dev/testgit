...
<script>	
	...
	/*查询*/
	function search(){
		salesBasicForm.action = "toMailOpenSalesSel.action";
		salesBasicForm.submit();
		showLoadingBar();
	}
	/*传参*/
	...
	function selectCustomer(ssesbsalesid ,ssesbpersonName, ssesbphone ,ssesbMemberId,ssetmcustomerid,ssesbsalesvalue){
		window.parent.setCustomer(ssesbsalesid ,ssesbpersonName, ssesbphone ,ssesbMemberId,ssetmcustomerid,ssesbsalesvalue);
		parent.hidePopWin();
		//
	}
                        ...
                        <TR>
			               ...
			               <TD class="table_none" colspan="3">
			               	  <c:if test="${person.departmenttype != '1'}">
                    	           ...
                    	           <OPTION value="${bdpdepartmentid}" ${shopcode != bdpdepartmentid  ? '' : 'selected="selected"' } >(${bdpdepartmentid})${bdpdepartmentname}</OPTION>
                    	          </s:iterator>
                    	        </select>
                    	      </c:if>
                    	      <c:if test="${person.departmenttype == '1'}">
                    	      	   (${person.departmentID})${person.bdpdepartmentname}
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TR height="26" class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">                         
                          <TD>
                             <img src="${ctx }/img/newbtn/select_0.png" btn=btn title='选择' onClick="javascript:selectCustomer('${ssesbsalesid }','${ssesbpersonName }','${ssesbphone }','${ssesbMemberId }','${ssesbcustomerid }','${ssesbsalesvalue}')">
                          </TD>
                          <TD>${ssesbsalesid}</TD>
