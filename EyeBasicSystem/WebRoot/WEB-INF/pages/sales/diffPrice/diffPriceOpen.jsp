...
<script>
	...
	*/
	function discount1(){
		showPopWin("","initPersonDiscountSelect.action",screen.width-600,screen.height-470, '', null, true);
		selectHidden();
	}
	/*
	整单打折开窗回调
	...
	*/
	function discount2(item){
		showPopWin("","initPersonDiscountSelect.action?arg0=1&rownumber="+$(item).parent().find(".rownumber").val(),screen.width-600,screen.height-470, '', null, true);
		selectHidden();
	}
	
	function setDiscount2(discount2,rownumber){
		$('tr[id=goodsrow]').find('.yssum')[rownumber].value=accMul($('tr[id=goodsrow]').find('.pricesums')[rownumber].value,discount2).toFixed(1);
		$('tr[id=goodsrow]').find('.zksum')[rownumber].value=accAdd($('tr[id=goodsrow]').find('.pricesums')[rownumber].value,'-'+$('tr[id=goodsrow]').find('.yssum')[rownumber].value).toFixed(1);
		$('tr[id=goodsrow]').find('.discount')[rownumber].value=discount2;
		$('tr[id=goodsrow]').find('.postvalue')[rownumber].value=accAdd($('tr[id=goodsrow]').find('.oldDetailPost')[rownumber].value,'-'+accMul(discount2,$('tr[id=goodsrow]').find('.pricesums')[rownumber].value)).toFixed(1);
	...
	$(document).ready(function(){
		...
		$('#ssespbnewpsalsvalue').keyup(function(){
			if(isNaN($(this).val())){
				$(this).val('');
			}
		});
		
		$('#ssespbnewpsalsvalue').blur(function(){
			if(!isNaN($(this).val())){
				if($(this).val()==''){
					$('#ssje').text('￥0.00');
					$(this).val('0.00');
	...
	function diffPrice(){
		...
		if($('#ssespbpostvalue').text()==''||parseFloat($('#ssespbpostvalue').text())<=0){
			alert('退差价金额有误!');
			return;
		}
		
		$('#ssespbsalesvalue').val(accAdd('${salesBasicPo.ssesbpricesum }','-'+$('#ssespbnewdiscountnum').text()));
		
		$('input[name=ssespbnewdiscountnum]')[0].value=$('#ssespbnewdiscountnum').text();
		
		$('input[name=ssespbpostvalue]')[0].value=$('#ssespbpostvalue').text();
		
		$('input[name=ssespbnewarrearsvalue]')[0].value=parseData(accAdd($('#ssespbsalesvalue').val(),'-'+$('#ssespbnewpsalsvalue').val()));
		$("img").removeAttr("onclick");
		diffPriceForm.action="diffPrice.action?ssespbpostvalue="+$('#ssespbpostvalue').text();
		diffPriceForm.submit();
	}
</script>
<BODY bgColor=#ffffff topMargin=5 ${sessionScope.systemParameterPo.fsprightshowurl eq '1' ? 'onkeydown="KeyDown()" oncontextmenu="event.returnValue=false" onhelp="Showhelp();return false;"' : '' }>
<form method="post" name="diffPriceForm"  action="">
<input type=hidden name="ssespbnewdiscountnum"/>
<input type=hidden name="ssespbpostvalue"/>
<input type=hidden name="ssespbnewarrearsvalue"/>
<input type=hidden name="ssespbsalesvalue" id="ssespbsalesvalue"/>
                  ...
                  <DIV>
                      ...
                      <TBODY>
                        <TR>
                          ...
                          <TD width="35%" class="table_none"><input type="hidden" name="newsalesid" value="${newsalesID }"/>
                          <input type="hidden" name="salesID" value="${salesID }"/>${newsalesID }</TD>
                        ...
                        <TR >
                                                  ...
                                                  <input type="hidden" name="zhekou" value="${salesBasicPo.ssesbdiscountnum }">
                                                  </TD>
                        </TR>
                        <TR>
                        ...
                        <TR >
                                                  <TD class="table_body">调整后实缴金额</TD>
                                                  <TD class="table_none" ><input type="text" value="${salesBasicPo.ssesbpsalsvalue }" id="ssespbnewpsalsvalue" name="ssespbnewpsalsvalue"/></TD>
                                                  <TD class="table_body" height="30">调整后折扣金额</TD>
                                                  <TD class="table_none"><span id="ssespbnewdiscountnum">${salesBasicPo.ssesbarrearsvalue}</span></TD>
						                            
                        </TR>
                        <TR >
                                                  <TD class="table_body"><font color="red">退差价金额</font></TD>
                                                  <TD class="table_none" ><span id="ssespbpostvalue"><font color="red"></font></span></TD>
                                                  <TD class="table_body" height="30">&nbsp;</TD>
					    ...
					    <TBODY>
                        <TR>
                                              <TD><input id="saveButton" name="button3223" icon="icon-add-row" type='button' value='缴费' onclick="diffPrice()" align="left"></TD>
                                              <td>整单打折:<input size="8" id="ssespbnewdiscountrate" name="ssespbnewdiscountrate"  class="text_input"  type="text"  onclick="discount1()" ></td>
                        </TR>
                        </TBODY>
                    </TABLE>
						  <table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                              ...
                              <table width="100%" border="0" cellpadding="1" cellspacing="1" class="Privateborder privateTable">
                                ...
                                <tr id="goodsrow">
                                  <td class="Privateborder"><input style="width:100%;background:transparent;border:0px;" name="salesPostDetailPo.ssespdsalesitemid" class="text_input100" value="${ssesdsalesitemid }" readOnly="readOnly"><input type="hidden" name="salesPostDetailPo.ssespdid" value="${ssesdid}"/></td>
                                  ...
                                  <td class="Privateborder"><input style="width:100%" onmouseover="salesover(this)" onmouseout="salesout(this)" name="salesPostDetailPo.ssespddiscountrate" class="text_input100 salesout discount" value="${ssesddiscountrate }" onClick="discount2(this)" readOnly="readOnly"/><input type="hidden"  value="${index.index}" class="rownumber"/> </td>
