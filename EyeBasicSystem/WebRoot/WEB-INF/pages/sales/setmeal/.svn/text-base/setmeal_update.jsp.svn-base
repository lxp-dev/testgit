...
<script>
	$(document).ready(function() {
        ...
        $('select[hidn=hidn]').each(function(){
   ...
   function chkAll2(){  
    ...
    function chkAll3(){  
     ...
     function addRow(){
		...
		if ($('#ssmsmform').val() == '1' || $('#ssmsmform').val() == '2'){
		...
		if ($('#ssmsmform').val() != '3'){
		   ...
		   c7.innerHTML = '<input class="text_input60" maxlength="15" id="specialoffer2'+index+'" type="text" name="salesGoodsArray.ssmsgspecialoffer" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'填写金额！\'},{\'Type\' : Type.String, \'Formula\' : Formula.UFloat, \'Message\' : \'填写的金额不正确！\'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value).toFixed(2);}else{this.value = \'0.00\';}checkCostPrice(this,'+index+');" value="0.00">';
	    }		
     ...
     function addRow2(){
        ...
        c4.innerHTML = '<input class="text_input60" style="width:50" maxlength="15" id="ssmsgbeginAmount'+index+'" type="text" name="creditGoodsArray.ssmsgbeginAmount" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.FloatORNULL, \'Message\' : \'填写的金额不正确！\'}]" onblur="validationGoodsCostPriceMin(this,'+index+');" zp6="zp'+index+'" onKeyPress="zpEnterDown('+index+',\'7\',\'\')">至<input maxlength="15" id="ssmsgendAmount'+index+'" style="width:50" class="text_input60" type="text" name="creditGoodsArray.ssmsgendAmount" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.FloatORNULL, \'Message\' : \'填写的金额不正确！\'}]" onblur="validationGoodsCostPriceMax(this,'+index+');" zp7="zp'+index+'" onKeyPress="zpEnterDown('+index+',\'8\',\'\')">';
     ...
     function addRow3(){
         ...
         c3.innerHTML = '<span id="spanminCostPriceAmount'+index+'"></span><input class="text_input60 noneInput" id="minCostPriceAmount'+index+'" type="text" name="creditGoodsArray.ssmsgmincostPrice" readonly="readonly">至<span id="spanmaxCostPriceAmount'+index+'"></span><input id="maxCostPriceAmount'+index+'" class="text_input60 noneInput" type="text" readonly="readonly" name="creditGoodsArray.ssmsgmaxcostPrice" >';
    ...
    function getUrl(index){
        ...
        path = path + '&retailbeginprice='+$('#ssmsgbeginAmount'+index).val()+ '&retailendprice='+$('#ssmsgendAmount'+index).val();     
        return path;
    }
	 ...
	 */ 
     ...
     function computeCostPrice(obj,index){
         var min = $('#minCostPrice'+index).val();
         var max = $('#maxCostPrice'+index).val();
         
         $('#minCostPriceAmount'+index).val(parseFloat(accMul(obj.value,min)).toFixed(2));
         $('#maxCostPriceAmount'+index).val(parseFloat(accMul(obj.value,max)).toFixed(2));
         $('#specialoffer'+index).val(parseFloat(accMul(obj.value,max)).toFixed(2));
         
         $('#ssmsgbeginAmount'+index).val(parseFloat(accMul(obj.value,min)).toFixed(2));
         $('#ssmsgendAmount'+index).val(parseFloat(accMul(obj.value,max)).toFixed(2));
         if ($('#ssmsgexpensecredit'+index) != null){
             $('#ssmsgexpensecredit'+index).val(parseFloat(accMul(obj.value,min)).toFixed(2));
         }
         amoumt();
     }
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                    ...
                    <table width="100%"  border=0 align=center cellpadding=0 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
			              ...
			              <TD width="24%" class="table_none">                    	    
                      	        <c:if test="${setMealPo.ssmsmform eq '1'}">商品满送</c:if>
	                      	    <c:if test="${setMealPo.ssmsmform eq '2'}">商品满减</c:if>
	                      	    <c:if test="${setMealPo.ssmsmform eq '3'}">商品特价</c:if>
                      	        <input type="hidden" class="text_input200" id="ssmsmform" name="setMealPo.ssmsmform"  value="${setMealPo.ssmsmform}">                      	    
			              </TD>
			            ...
			            <TR>
			              ...
			              <input id="ssmsmcreatedate" clean="clean" 
					       ...
					       type="text" class="text_input100"  value="${setMealPo.ssmsmeffectivedate}" 
					       ...
					       />&nbsp;<span class="STYLE1">*</span>
                        	...
                        	<TD height="26" class="table_none" >
                        	<select clean="clean" id="ssmsmclassify" name="setMealPo.ssmsmclassify" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请选择套餐分类！'}]" onchange="changeClassif()">
							    <option value="" ${setMealPo.ssmsmclassify eq '' ? 'selected="selected"' : '' }>----请选择----</option>
	                            <option value="1" ${setMealPo.ssmsmclassify eq '1' ? 'selected="selected"' : '' }>镜架</option>
	                            <option value="3" ${setMealPo.ssmsmclassify eq '3' ? 'selected="selected"' : '' }>镜片</option>
	                            <option value="4" ${setMealPo.ssmsmclassify eq '4' ? 'selected="selected"' : '' }>隐形类</option>
	                            <option value="6" ${setMealPo.ssmsmclassify eq '6' ? 'selected="selected"' : '' }>成镜类</option>
			        ...
			        <c:if test="${setMealPo.ssmsmform eq '3'}">      
                    ...
                    <c:if test="${setMealPo.ssmsmform eq '1' || setMealPo.ssmsmform eq '2'}">
                          <TD height="26" class="table_body">套餐金额</TD>
                          <TD height="26" class="table_none">
                        	 <input type="text" readonly="readonly" class="text_input100 noneInput" id="ssmsmendbgnAmount" name="setMealPo.ssmsmendbgnAmount" value="${setMealPo.ssmsmendbgnAmount}"> &nbsp;
                          </TD>          
                          <TD height="26" class="table_body">是否允许累加</TD>
							<TD class="table_none">
							  <input id="x15" name='setMealPo.ssmsmissum' type="radio" ${setMealPo.ssmsmissum eq '1' ? 'checked="checked"' : '' } value="1" >允许
			            ...
			            <TR>                             
                            ...
                            <input type="checkbox" id="chks" name="chks" onclick="chkAll()" ${chks_checked eq '1' ? 'checked="checked"' : '' }>全选
                            </TD>
					                  ...
					                  <c:forEach items="${departmentList}" var="departmentInfo" varStatus="status">
					                    <c:if test="${status.index >= currentIndex && currentCount <= columnCount }" >
						                 <td width="180" height="26" >
						                   <input type="checkbox" id=chk name='bdpdepartmentid' value="${ departmentInfo.bdpdepartmentid}" ${departmentInfo.bdpwizardflag eq '1' ? 'checked="checked"' : '' }>${ departmentInfo.bdpdepartmentname }
						                 </td>
			       ...
			       <!-- 
			            <TR>
							...
							<TD class="table_none">
	                          <input id="x13" name='setMealPo.ssmsmisdiscount' type="radio" ${setMealPo.ssmsmisdiscount eq '1' ? 'checked="checked"' : '' } value="1" >允许
			       ...
			       -->             
			              <TR>
							...
							<TD class="table_none" colspan="5">
							    <textarea clean="clean" id="ssmsmremark" name="setMealPo.ssmsmremark" validate="[{'Type' : Type.String, 'Formula' : '', 'Expansion' : {Type : Expansion.LessThanLengthORNULL, Params : [1001]}, 'Message' : '备注不能大于1000字！'}]">${setMealPo.ssmsmremark }</textarea>
                             </TD>
			              </TR>
                    </TABLE>
                <div id="shangpinmansong">    
                 ...
                 <c:if test="${setMealPo.ssmsmform eq '1' || setMealPo.ssmsmform eq '2'}">    
					...
					<TABLE id="addTablebangding" width="100%"  border=0 align=center cellspacing=1 class="privateBorder">
                      <TBODY>
                       ...
                       <c:if test="${ssmsgflag eq '1'}">
                         <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
						  <Td height="26"><input type="checkbox" id="chk1" name="chk1"></td>
                          <Td>                          
                          <c:if test="${setMealPo.ssmsmclassify eq '1'}">
                          <li class="horizontal_onlyRight" >
                            <select clean=clean id="goodscategoryID${idx.index }" xiaofei=xiaofei name="goodscategoryID" onchange="change3(this,'${idx.index }')" xf0="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','1','6')">
        		             <option value="1_0" ${ssmsggoodscategory eq '1' ? 'selected="selected"' : '' }>镜架</option>
      		                 <option value="2_1" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      		                 <option value="3" ${ssmsggoodscategory eq '3' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>镜片</option>
      		                 <option value="3_0" ${ssmsggoodscategory eq '3' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>成品片</option>
      		                 <option value="3_D" ${ssmsggoodscategory eq '3' ? (ssmsgiscustomize eq 'D' ? 'selected="selected"' : '') : '' }>订做片</option>
      	                    </select>
      	                    </li>
      	                  </c:if>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                          <li class="horizontal_onlyRight" >
                            <select clean=clean id="goodscategoryID${idx.index }" xiaofei=xiaofei name="goodscategoryID" onchange="change4(this,'${idx.index }')"  xf0="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','1','6')">
                             <option value="4" ${ssmsggoodscategory eq '4' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>隐形镜片</option>      		                 
      		                 <option value="4_0" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>隐形成品片</option>
      		                 <option value="4_D" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq 'D' ? 'selected="selected"' : '') : '' }>隐形订做片</option>
      		                 <option value="5_0" ${ssmsggoodscategory eq '5' ? 'selected="selected"' : '' }>隐形护理液</option>
      		                 <option value="2_2" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      	                    </select>
      	                    </li>
      	                  </c:if>  
                          <c:if test="${setMealPo.ssmsmclassify eq '6'}">
                          <li class="horizontal_onlyRight">
                            <select clean=clean id="goodscategoryID${idx.index }" xiaofei=xiaofei name="goodscategoryID"  xf0="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">
      		                 <option value="6_6" ${ssmsggoodscategory eq '6' ? 'selected="selected"' : '' }>太阳镜</option>
      		                 <option value="6_8" ${ssmsggoodscategory eq '8' ? 'selected="selected"' : '' }>老花镜</option>
      		                </select>
      		                </li>
      		              </c:if>
      		              
                            <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                              <c:if test="${ssmsggoodscategory eq '4'}">
                              <li class="horizontal_onlyRight" id="ligoodscategoryID401${idx.index }">
                                 <select clean=clean id="goodscategoryID401${idx.index }" name="goodscategoryID401" ${ssmsggoodscategory == '4' ? '' : 'hidn=hidn' } onchange="change41(this,'${idx.index }')" xf1="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','2','6')">      
      		                       <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
      		                       <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>使用类型</option>
      		                       <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>抛弃型分类</option>
      	                         </select>
      	                         </li>   
                               </c:if>                            
                            </c:if>
                            <c:if test="${setMealPo.ssmsmclassify eq '1' || setMealPo.ssmsmclassify eq '3'}">
                            <li class="horizontal_onlyRight" id="ligoodscategoryID306${idx.index }">
									 <select clean=clean id="goodscategoryID306${idx.index }" name="goodscategoryID306" ${ssmsggoodscategory == '1' ? '' : 'hidn="hidn"' } onchange="change32(this,'${idx.index }')" xf1="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','2','6')">
			       		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>                          
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材质</option>	            
			      	                 </select>
			      	                 </li> 
			      	        <li class="horizontal_onlyRight" id="ligoodscategoryID301${idx.index }">              
		                             <select clean=clean id="goodscategoryID301${idx.index }" name="goodscategoryID301" ${ssmsggoodscategory == '3' ? '' : 'hidn="hidn"' } onchange="change31(this,'${idx.index }')" xf1="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','2','6')">
			      		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材料分类</option>
			                      		 <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>折射率</option>     
			      		                 <option value="3" ${ssmsgbigclass eq '3' ? 'selected="selected"' : '' }>光度分类</option> 	
			      		                 <option value="4" ${ssmsgbigclass eq '4' ? 'selected="selected"' : '' }>镜片功能</option> 			            
		      	                     </select>
		             	      </li>                        	                              	                              
                              </c:if>
                           <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                              <c:if test="${ssmsggoodscategory eq '4'}">
                              <li class="horizontal_onlyRight" id="ligoodscategoryID402${idx.index }">
									 <select clean=clean id="goodscategoryID402${idx.index }" name="goodscategoryID402" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">  
			      		               <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>常带型</option>
			      		               <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>抛弃型</option>
									<option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>塑形镜</option>
									 </select>
							</li>		 
							 <li class="horizontal_onlyRight" id="ligoodscategoryID403${idx.index }">		 							 
		                             <select clean=clean id="goodscategoryID403${idx.index }" name="goodscategoryID403" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">       
			      		                 <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>日抛</option>
			      		                 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>周抛</option>
			      		                 <option value="9" ${ssmsgsmallclass eq '9' ? 'selected="selected"' : '' }>双周抛</option>
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>月抛</option>
			      		                 <option value="4" ${ssmsgsmallclass eq '4' ? 'selected="selected"' : '' }>季抛</option>
			      		                 <option value="5" ${ssmsgsmallclass eq '5' ? 'selected="selected"' : '' }>半年抛</option>
			      		                 <option value="6" ${ssmsgsmallclass eq '6' ? 'selected="selected"' : '' }>年抛</option>
			      		                 <option value="7" ${ssmsgsmallclass eq '7' ? 'selected="selected"' : '' }>RGP</option>  		                 
		      	                     </select>
		      	              </li>       
                              </c:if>                            
                          </c:if>
                         <c:if test="${setMealPo.ssmsmclassify eq '1' || setMealPo.ssmsmclassify eq '3'}">
                          <li class="horizontal_onlyRight" id="ligoodscategoryID307${idx.index }">
		                             <select clean=clean id="goodscategoryID307${idx.index }" name="goodscategoryID307" ${(ssmsggoodscategory == '1' && ssmsgbigclass == '1') ? '' : 'hidn="hidn"' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">
	      		                 	    <s:iterator value="frameMateriallist"> 
										   <option value="${bfmid}" ${ssmsgsmallclass eq bfmid ? 'selected="selected"' : '' }>${bfmname}</option> 
										</s:iterator>     		                 			      		                 			      		                 		            
		      	                     </select>
		      	                     </li>
		      	           <li class="horizontal_onlyRight" id="ligoodscategoryID302${idx.index }">          
		                             <select clean=clean id="goodscategoryID302${idx.index }" name="goodscategoryID302" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '1') ? '' : 'hidn="hidn"' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>树脂</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>玻璃</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>PC</option> 			            
		      	                     </select>
		      	                     </li>
		      	           <li class="horizontal_onlyRight" id="ligoodscategoryID303${idx.index }">                  
									<select clean=clean id="goodscategoryID303${idx.index }" name="goodscategoryID303" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '2') ? '' : 'hidn="hidn"' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')"> 
			                             <s:iterator value="refractiveSetList">
			                             <option value="${brfname}" ${ssmsgsmallclass eq brfname ? 'selected="selected"' : '' }>${brfname}</option>
			                             </s:iterator>                      			            
									</select>
									</li>
						   <li class="horizontal_onlyRight" id="ligoodscategoryID304${idx.index }">							
									<select clean=clean id="goodscategoryID304${idx.index }" name="goodscategoryID304" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '3') ? '' : 'hidn="hidn"' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">
			                             <option value="0" ${ssmsgsmallclass eq '0' ? 'selected="selected"' : '' }>单光</option>
			                      		 <option value="m" ${ssmsgsmallclass eq 'm' ? 'selected="selected"' : '' }>多光</option>     
			      		                 <option value="j_1" ${ssmsgsmallclass eq 'j_1' ? 'selected="selected"' : '' }>青少年渐进</option>
			                      		 <option value="j_2" ${ssmsgsmallclass eq 'j_2' ? 'selected="selected"' : '' }>成人渐进</option> 
			      		                 <option value="k" ${ssmsgsmallclass eq 'k' ? 'selected="selected"' : '' }>抗疲劳</option> 	
			      		                 <option value="q" ${ssmsgsmallclass eq 'q' ? 'selected="selected"' : '' }>其他</option> 			            
									</select>
									</li>
							<li class="horizontal_onlyRight" id="ligoodscategoryID305${idx.index }">													
									<select clean=clean id="goodscategoryID305${idx.index }" name="goodscategoryID305" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '4') ? '' : 'hidn="hidn"' } xf2="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>白色片</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>变色片</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>偏光片</option>
			      		                 <option value="4" ${ssmsgsmallclass eq '4' ? 'selected="selected"' : '' }>变色偏光片</option> 	
			      		                 <option value="5" ${ssmsgsmallclass eq '5' ? 'selected="selected"' : '' }>染色片</option> 	
			      		                 <option value="6" ${ssmsgsmallclass eq '6' ? 'selected="selected"' : '' }>抗疲劳片</option>
			      		                 <option value="7" ${ssmsgsmallclass eq '7' ? 'selected="selected"' : '' }>抗疲劳变色片</option>
			      		                 <option value="8" ${ssmsgsmallclass eq '8' ? 'selected="selected"' : '' }>偏光抗疲劳片</option> 			            
									</select>  
									</li>    	                              	                              	                              
                              </c:if>
                          <li class="horizontal_onlyRight"><input id="goodsName${idx.index }" value="${ssmsggoodsname}" name="salesGoodsArray.ssmsggoodsname" class="text_input120" type="text" readonly="readonly"></li>
                          <li class="horizontal_onlyRight"><img src="${ctx }/img/newbtn/btn_change_0.png" title="选择" onclick="openGoods('${idx.index }');" btn=btn></li>
                          <input id="supplier${idx.index }" value="${ssmsgsupplier}"  class="text_input60" name="salesGoodsArray.ssmsgsupplier" type="hidden"><input id="brand${idx.index }" value="${ssmsgbrand }" class="text_input60" name="salesGoodsArray.ssmsgbrand" type="hidden"><input id="iscustomize${idx.index }" value="${ssmsgiscustomize }" name="salesGoodsArray.ssmsgiscustomize" class="text_input60" type="hidden"><input id="goods${idx.index }" value="${ssmsggoodsid }" name="salesGoodsArray.ssmsggoodsid" class="text_input60" type="hidden">
                          <input id="goodscategory2id${idx.index }" value="${ssmsggoodscategory }" name="salesGoodsArray.ssmsggoodscategory" class="text_input60" type="hidden"><input id="minCostPrice${idx.index }" value="${ssmsgmincostPrice }" class="text_input60" type="hidden"><input id="maxCostPrice${idx.index }" value="${ssmsgmaxcostPrice }" class="text_input60" type="hidden"><input id="bigClass${idx.index }" value="${ssmsgbigclass }" name="salesGoodsArray.ssmsgbigclass" class="text_input60" type="hidden"><input id="smallClass${idx.index }" value="${ssmsgsmallclass }" name="salesGoodsArray.ssmsgsmallclass" class="text_input60" type="hidden">
                                   
                          </Td>
                          <Td><span id="spanminCostPriceAmount${idx.index }">${ssmsgmincostPrice}</span><input class="text_input60 noneInput" value="${ssmsgmincostPrice}" id="minCostPriceAmount${idx.index }" type="hidden" name="salesGoodsArray.ssmsgmincostPrice" readonly="readonly">至<span id="spanmaxCostPriceAmount${idx.index }">${ssmsgmaxcostPrice}</span><input value="${ssmsgmaxcostPrice}" id="maxCostPriceAmount${idx.index }" class="text_input60 noneInput" type="hidden" readonly="readonly" name="salesGoodsArray.ssmsgmaxcostPrice" ></Td>
                          <Td><input class="text_input60" style="width: 50" maxlength="15" value="${ssmsgbeginAmount}" id="ssmsgbeginAmount${idx.index }" type="text" name="salesGoodsArray.ssmsgbeginAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMin(this,'${idx.index }');" xf6="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','7','')">至<input style="width: 50" maxlength="15" value="${ssmsgendAmount}" id="ssmsgendAmount${idx.index }" class="text_input60" type="text" name="salesGoodsArray.ssmsgendAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMax(this,'${idx.index }');" xf7="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','8','')"></Td>
                          <Td><input class="text_input60" style="width: 50" maxlength="15" value="${ssmsggoodsquantity}" id="goodsquantity${idx.index }" type="text" name="salesGoodsArray.ssmsggoodsquantity" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写销售数量！'},{'Type' : Type.String, 'Formula' : Formula.UINT, 'Message' : '填写的销售数量不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number($.trim(this.value) == '' ? 1 : this.value).toFixed(0);}else{this.value = '1';}getSetMealAmount3(this);" xf8="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','9','')"></Td>
						  <Td><input class="text_input60" style="width: 50" maxlength="15" value="${ssmsgretailPrice}" id="ssmsgretailPrice${idx.index }" type="text" name="salesGoodsArray.ssmsgretailPrice" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的零售价不正确！'}]" onblur="getSetMealAmount(this)" xf9="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','10','')"></Td>
						  <Td><input class="text_input60" style="width: 50" maxlength="15" value="${ssmsgspecialoffer}" id="specialoffer${idx.index }" type="text" name="salesGoodsArray.ssmsgspecialoffer" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写金额！'},{'Type' : Type.String, 'Formula' : Formula.UFloat, 'Message' : '填写的金额不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value).toFixed(2);}else{this.value = '0.00';}" xf10="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','11','12')"></Td>
                          <Td>
                       <c:if test="${setMealPo.ssmsmform eq '2'}">   
                          <input class="text_input60" style="width: 50" maxlength="15" value="${ssmsgexpensecredit}" id="ssmsgexpensecredit${idx.index }" type="text" name="salesGoodsArray.ssmsgexpensecredit" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写金额！'},{'Type' : Type.String, 'Formula' : Formula.UFloat, 'Message' : '填写的金额不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value).toFixed(2);}else{this.value = '0.00';}" xf11="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }','12','')">
                       </c:if>   
                          </Td>
                          
                        <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgsphul${idx.index }" type="text" name="salesGoodsArray.ssmsgsphul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]"  xf12="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }',13,'')" onblur="checkData(this)" value="${ssmsgsphul}">至<input maxlength="20" id="ssmsgsphup${idx.index }" class="text_input60" style="width:50" type="text" name="salesGoodsArray.ssmsgsphup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]" xf13="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }',14,'')" onblur="checkData(this)" value="${ssmsgsphup}"></Td>
                        <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgcylul${idx.index }" type="text" name="salesGoodsArray.ssmsgcylul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]"  xf14="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }',15,'')" onblur="checkDataz(this)" value="${ssmsgcylul}">至<input maxlength="20" id="ssmsgcylup${idx.index }" class="text_input60" style="width:50" type="text" name="salesGoodsArray.ssmsgcylup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]" xf15="xf${idx.index }" onKeyPress="xfEnterDown('${idx.index }',0,'')" onblur="checkDataz(this)" value="${ssmsgcylup}"></Td>
			    ...
			    <c:if test="${setMealPo.ssmsmform eq '1'}">   			
					<fieldset>
					...
					<TABLE id="addTablesong" width="100%"  border=0 align=center cellspacing=1 class="privateBorder">
                     ...
                     <TR class=table_title>
                          ...
                          <TH width="30%" scope=col>商品信息</TH>
                          <TH width="12%" scope=col>商品原价区间</TH>
                     ...
                     <s:iterator value="salesGoodsArrayList" status="idx">
                       <c:if test="${ssmsgflag eq '2'}">
                         <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
						  <Td height="26"><input type="checkbox" id="chk2" name="chk2"></td>
                          <Td>
                          <li class="horizontal_onlyRight">
                          <c:if test="${setMealPo.ssmsmclassify eq '1'}">
                            <select clean=clean id="goodscategoryID${idx.index }" name="goodscategoryID" onchange="change3(this,'${idx.index }')" zp0="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','1','6')">
        		             <option value="1_0" ${ssmsggoodscategory eq '1' ? 'selected="selected"' : '' }>镜架</option>
      		                 <option value="2_1" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      		                 <option value="3" ${ssmsggoodscategory eq '3' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>镜片</option>
      		                 <option value="3_0" ${ssmsggoodscategory eq '3' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>成品片</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                            <select clean=clean id="goodscategoryID${idx.index }" name="goodscategoryID" onchange="change4(this,'${idx.index }')" zp0="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','1','6')">
                             <option value="4" ${ssmsggoodscategory eq '4' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>隐形镜片</option>     
      		                 <option value="4_0" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>隐形成品片</option>
      		                 <option value="4_D" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq 'D' ? 'selected="selected"' : '') : '' }>隐形订做片</option>
      		                 <option value="5_0" ${ssmsggoodscategory eq '5' ? 'selected="selected"' : '' }>隐形护理液</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '6'}">
                            <select clean=clean id="goodscategoryID${idx.index }" name="goodscategoryID" zp0="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">
      		                 <option value="6_6" ${ssmsggoodscategory eq '6' ? 'selected="selected"' : '' }>太阳镜</option>
      		                 <option value="6_8" ${ssmsggoodscategory eq '8' ? 'selected="selected"' : '' }>老花镜</option>
      		                </select>
      		              </c:if>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                              <c:if test="${ssmsggoodscategory eq '4'}">
                                <li class="horizontal_onlyRight" id="ligoodscategoryID401${idx.index }">
                                 <select clean=clean id="goodscategoryID401${idx.index }" name="goodscategoryID401" ${ssmsggoodscategory == '4' ? '' : 'hidn=hidn' } onchange="change41(this,'${idx.index }')" zp1="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','2','6')">      
      		                       <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
      		                       <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>使用类型</option>
      		                       <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>抛弃型分类</option>
      	                         </select>
      	                           ...
      	                           <li class="horizontal_onlyRight" id="ligoodscategoryID402${idx.index }">
									 <select clean=clean id="goodscategoryID402${idx.index }" name="goodscategoryID402" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">  
			      		               <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>常带型</option>
			      		               <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>抛弃型</option>
									 </select>
									 </li>
									   <li class="horizontal_onlyRight" id="ligoodscategoryID403${idx.index }">
		                             <select clean=clean id="goodscategoryID403${idx.index }" name="goodscategoryID403" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">       
			      		                 <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>日抛</option>
			      		                 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>周抛</option>
			      		                 <option value="9" ${ssmsgsmallclass eq '9' ? 'selected="selected"' : '' }>双周抛</option>
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>月抛</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '1' || setMealPo.ssmsmclassify eq '3'}">
                            <li class="horizontal_onlyRight" id="ligoodscategoryID306${idx.index }">
									 <select clean=clean id="goodscategoryID306${idx.index }" name="goodscategoryID306" ${ssmsggoodscategory == '1' ? '' : 'hidn=hidn' } onchange="change32(this,'${idx.index }')" zp1="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','2','6')">
			       		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>                          
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材质</option>	            
			      	                 </select>
			      	                 </li>
			      	                   <li class="horizontal_onlyRight" id="ligoodscategoryID307${idx.index }">
		                             <select clean=clean id="goodscategoryID307${idx.index }" name="goodscategoryID307" ${(ssmsggoodscategory == '1' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">
	      		                 	    <s:iterator value="frameMateriallist"> 
										   <option value="${bfmid}" ${ssmsgsmallclass eq bfmid ? 'selected="selected"' : '' }>${bfmname}</option> 
										</s:iterator>    
		      	                     </select>
		      	                       ...
		      	                       <li class="horizontal_onlyRight" id="ligoodscategoryID301${idx.index }">
		                             <select clean=clean id="goodscategoryID301${idx.index }" name="goodscategoryID301" ${ssmsggoodscategory == '3' ? '' : 'hidn=hidn' } onchange="change31(this,'${idx.index }')" zp1="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','2','6')">
			      		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材料分类</option>
			                      		 <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>折射率</option>     
			      		                 <option value="3" ${ssmsgbigclass eq '3' ? 'selected="selected"' : '' }>光度分类</option> 	
		      	                       ...
		      	                       <li class="horizontal_onlyRight" id="ligoodscategoryID302${idx.index }">
		                             <select clean=clean id="goodscategoryID302${idx.index }" name="goodscategoryID302" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>树脂</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>玻璃</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>PC</option> 			            
		      	                     </select>
		      	                       ...
		      	                       <li class="horizontal_onlyRight" id="ligoodscategoryID303${idx.index }">
									<select clean=clean id="goodscategoryID303${idx.index }" name="goodscategoryID303" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')"> 
			                             <s:iterator value="refractiveSetList">
			                             <option value="${brfname}" ${ssmsgsmallclass eq brfname ? 'selected="selected"' : '' }>${brfname}</option>
			                             </s:iterator>                      			            
									</select>
									  ...
									  <li class="horizontal_onlyRight" id="ligoodscategoryID304${idx.index }">
									<select clean=clean id="goodscategoryID304${idx.index }" name="goodscategoryID304" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '3') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">
			                             <option value="0" ${ssmsgsmallclass eq '0' ? 'selected="selected"' : '' }>单光</option>
			                      		 <option value="m" ${ssmsgsmallclass eq 'm' ? 'selected="selected"' : '' }>多光</option>     
			      		                 <option value="j_1" ${ssmsgsmallclass eq 'j_1' ? 'selected="selected"' : '' }>青少年渐进</option>
			                      		 <option value="j_2" ${ssmsgsmallclass eq 'j_2' ? 'selected="selected"' : '' }>成人渐进</option> 
									  ...
									  <li class="horizontal_onlyRight" id="ligoodscategoryID305${idx.index }">
									<select clean=clean id="goodscategoryID305${idx.index }" name="goodscategoryID305" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '4') ? '' : 'hidn=hidn' } zp2="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>白色片</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>变色片</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>偏光片</option>
			      		                 <option value="4" ${ssmsgsmallclass eq '4' ? 'selected="selected"' : '' }>变色偏光片</option> 	
                          ...
                          <li class="horizontal_onlyRight"><input id="goodsName${idx.index }" value="${ssmsggoodsname}" name="creditGoodsArray.ssmsggoodsname" class="text_input120" type="text" readonly="readonly"></li>
                          <li class="horizontal_onlyRight"><img src="${ctx }/img/newbtn/btn_change_0.png" title="选择" onclick="openGoods('${idx.index }');" btn=btn></li>
                          <input id="supplier${idx.index }" value="${ssmsgsupplier}"  class="text_input60" name="creditGoodsArray.ssmsgsupplier" type="hidden"><input id="brand${idx.index }" value="${ssmsgbrand }" class="text_input60" name="creditGoodsArray.ssmsgbrand" type="hidden"><input id="iscustomize${idx.index }" value="${ssmsgiscustomize }" name="creditGoodsArray.ssmsgiscustomize" class="text_input60" type="hidden"><input id="goods${idx.index }" value="${ssmsggoodsid }" name="creditGoodsArray.ssmsggoodsid" class="text_input60" type="hidden">
                          <input id="goodscategory2id${idx.index }" value="${ssmsggoodscategory }" name="creditGoodsArray.ssmsggoodscategory" class="text_input60" type="hidden"><input id="minCostPrice${idx.index }" value="${ssmsgmincostPrice }" class="text_input60" type="hidden"><input id="maxCostPrice${idx.index }" value="${ssmsgmaxcostPrice }" class="text_input60" type="hidden"><input id="bigClass${idx.index }" value="${ssmsgbigclass }" name="creditGoodsArray.ssmsgbigclass" class="text_input60" type="hidden"><input id="smallClass${idx.index }" value="${ssmsgsmallclass }" name="creditGoodsArray.ssmsgsmallclass" class="text_input60" type="hidden">
                                   
                          </Td>
                          <Td><span id="spanminCostPriceAmount${idx.index }">${ssmsgmincostPrice}</span><input class="text_input60 noneInput" value="${ssmsgmincostPrice}" id="minCostPriceAmount${idx.index }" type="hidden" name="creditGoodsArray.ssmsgmincostPrice" readonly="readonly">至<span id="spanmaxCostPriceAmount${idx.index }">${ssmsgmaxcostPrice}</span><input value="${ssmsgmaxcostPrice}" id="maxCostPriceAmount${idx.index }" class="text_input60 noneInput" type="hidden" readonly="readonly" name="creditGoodsArray.ssmsgmaxcostPrice" ></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsgbeginAmount}" id="ssmsgbeginAmount${idx.index }" type="text" name="creditGoodsArray.ssmsgbeginAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMin(this,'${idx.index }');" zp6="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','7','')">至<input style="width:50" maxlength="15" value="${ssmsgendAmount}" id="ssmsgendAmount${idx.index }" class="text_input60" type="text" name="creditGoodsArray.ssmsgendAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMax(this,'${idx.index }');" zp7="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','8','')"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsggoodsquantity}" id="goodsquantity${idx.index }" type="text" name="creditGoodsArray.ssmsggoodsquantity" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写赠送数量！'},{'Type' : Type.String, 'Formula' : Formula.UINT, 'Message' : '填写的赠送数量不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number($.trim(this.value) == '' ? 1 : this.value).toFixed(0);}else{this.value = '1';}getSetMealAmount3(this);" zp8="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','13','')"></td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" id="ssmsgretailPrice${idx.index }" type="text" name="creditGoodsArray.ssmsgretailPrice" value="${ssmsgretailPrice}" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写赠送单价！'},{'Type' : Type.String, 'Formula' : Formula.UFloat, 'Message' : '填写的赠送单价不正确！'}]" onblur="getSetMealAmount(this)" zp13="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','9','')"></Td>
						  <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgsphul${idx.index }" type="text" name="creditGoodsArray.ssmsgsphul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]"  zp9="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','10','')" onblur="checkData(this)" value="${ssmsgsphul}">至<input maxlength="20" id="ssmsgsphup${idx.index }" class="text_input60" style="width:50" type="text" name="creditGoodsArray.ssmsgsphup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]" zp10="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','11','')" onblur="checkData(this)" value="${ssmsgsphup}"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgcylul${idx.index }" type="text" name="creditGoodsArray.ssmsgcylul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]"  zp11="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','12','')" onblur="checkDataz(this)" value="${ssmsgcylul}">至<input maxlength="20" id="ssmsgcylup${idx.index }" class="text_input60" style="width:50" type="text" name="creditGoodsArray.ssmsgcylup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]" zp12="zp${idx.index }" onKeyPress="zpEnterDown('${idx.index }','0','')" onblur="checkDataz(this)" value="${ssmsgcylup}"></Td>
					...
					<TABLE id="addTablesong2" width="100%"  border=0 align=center cellspacing=1 class="privateBorder">
                     ...
                     <s:iterator value="salesGoodsArrayList" status="idx">
                       <c:if test="${ssmsgflag eq '2'}">
                         <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
						  <Td height="26"><input type="checkbox" id="chk3" name="chk3"></td>
                          <Td>
                          <li class="horizontal_onlyRight">
                          <c:if test="${setMealPo.ssmsmclassify eq '1'}">
                            <select clean=clean id="goodscategoryID${idx.index }" name="goodscategoryID" onchange="change3(this,'${idx.index }')" dk0="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','1','6')">
        		             <option value="1_0" ${ssmsggoodscategory eq '1' ? 'selected="selected"' : '' }>镜架</option>
      		                 <option value="2_1" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      		                  <option value="3" ${ssmsggoodscategory eq '3' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>镜片</option>
      		                 <option value="3_0" ${ssmsggoodscategory eq '3' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>成品片</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                            <select clean=clean id="goodscategoryID${idx.index }" name="goodscategoryID" onchange="change4(this,'${idx.index }')" dk0="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','1','6')">     
      		                 <option value="2_2" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      		                 <option value="4" ${ssmsggoodscategory eq '4' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>隐形镜片</option>
      		                 <option value="4_0" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>隐形成品片</option>
      		                 <option value="4_D" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq 'D' ? 'selected="selected"' : '') : '' }>隐形订做片</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '6'}">
                            <select clean=clean id="goodscategoryID${idx.index }" name="goodscategoryID" dk0="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">
      		                 <option value="6_6" ${ssmsggoodscategory eq '6' ? 'selected="selected"' : '' }>太阳镜</option>
      		                 <option value="6_8" ${ssmsggoodscategory eq '8' ? 'selected="selected"' : '' }>老花镜</option>
      		                </select>
      		              </c:if>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                              <c:if test="${ssmsggoodscategory eq '4'}">
                               <li class="horizontal_onlyRight" id="ligoodscategoryID401${idx.index }">
                                 <select clean=clean id="goodscategoryID401${idx.index }" name="goodscategoryID401" ${ssmsggoodscategory == '4' ? '' : 'hidn=hidn' } onchange="change41(this,'${idx.index }')" dk1="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','2','6')">      
      		                       <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
      		                       <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>使用类型</option>
      		                       <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>抛弃型分类</option>
      	                         </select>
      	                        ...
      	                        <li class="horizontal_onlyRight" id="ligoodscategoryID402${idx.index }">  
									 <select clean=clean id="goodscategoryID402${idx.index }" name="goodscategoryID402" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">  
			      		               <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>常带型</option>
			      		               <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>抛弃型</option>
									 </select>
							</li>		 
							 <li class="horizontal_onlyRight" id="ligoodscategoryID403${idx.index }">		 
		                             <select clean=clean id="goodscategoryID403${idx.index }" name="goodscategoryID403" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">       
			      		                 <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>日抛</option>
			      		                 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>周抛</option>
			      		                 <option value="9" ${ssmsgsmallclass eq '9' ? 'selected="selected"' : '' }>双周抛</option>
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>月抛</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '1' || setMealPo.ssmsmclassify eq '3'}">
                                 <li class="horizontal_onlyRight" id="ligoodscategoryID306${idx.index }">
									 <select clean=clean id="goodscategoryID306${idx.index }" name="goodscategoryID306" ${ssmsggoodscategory == '1' ? '' : 'hidn=hidn' } onchange="change32(this,'${idx.index }')" dk1="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','2','6')">
			       		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>                          
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材质</option>	            
			      	                 </select>
                                 </li>				      	                 
			      	             <li class="horizontal_onlyRight" id="ligoodscategoryID307${idx.index }">     
		                             <select clean=clean id="goodscategoryID307${idx.index }" name="goodscategoryID307" ${(ssmsggoodscategory == '1' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">
	      		                 	    <s:iterator value="frameMateriallist"> 
										   <option value="${bfmid}" ${ssmsgsmallclass eq bfmid ? 'selected="selected"' : '' }>${bfmname}</option> 
										</s:iterator>    		      		                 	            
		      	                     </select>
		      	                 ...
		      	                 <li class="horizontal_onlyRight" id="ligoodscategoryID301${idx.index }">    
		                             <select clean=clean id="goodscategoryID301${idx.index }" name="goodscategoryID301" ${ssmsggoodscategory == '3' ? '' : 'hidn=hidn' } onchange="change31(this,'${idx.index }')" dk1="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','2','6')">
			      		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材料分类</option>
			                      		 <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>折射率</option>     
			      		                 <option value="3" ${ssmsgbigclass eq '3' ? 'selected="selected"' : '' }>光度分类</option> 	
                                 ...
                                 <li class="horizontal_onlyRight" id="ligoodscategoryID302${idx.index }">		      	                     
		                             <select clean=clean id="goodscategoryID302${idx.index }" name="goodscategoryID302" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>树脂</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>玻璃</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>PC</option> 			            
		      	                     </select>
                                 ...
                                 <li class="horizontal_onlyRight" id="ligoodscategoryID303${idx.index }">		      	                     
									<select clean=clean id="goodscategoryID303${idx.index }" name="goodscategoryID303" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')"> 
			                             <s:iterator value="refractiveSetList">
			                             <option value="${brfname}" ${ssmsgsmallclass eq brfname ? 'selected="selected"' : '' }>${brfname}</option>
			                             </s:iterator>                      			            
									</select>
                                 ...
                                 <li class="horizontal_onlyRight" id="ligoodscategoryID304${idx.index }">									
									<select clean=clean id="goodscategoryID304${idx.index }" name="goodscategoryID304" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '3') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">
			                             <option value="0" ${ssmsgsmallclass eq '0' ? 'selected="selected"' : '' }>单光</option>
			                      		 <option value="m" ${ssmsgsmallclass eq 'm' ? 'selected="selected"' : '' }>多光</option>     
			      		                 <option value="j_1" ${ssmsgsmallclass eq 'j_1' ? 'selected="selected"' : '' }>青少年渐进</option>
			                      		 <option value="j_2" ${ssmsgsmallclass eq 'j_2' ? 'selected="selected"' : '' }>成人渐进</option> 
                                 ...
                                 <li class="horizontal_onlyRight" id="ligoodscategoryID305${idx.index }">									
									<select clean=clean id="goodscategoryID305${idx.index }" name="goodscategoryID305" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '4') ? '' : 'hidn=hidn' } dk2="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>白色片</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>变色片</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>偏光片</option>
			      		                 <option value="4" ${ssmsgsmallclass eq '4' ? 'selected="selected"' : '' }>变色偏光片</option> 	
                          ...
                          <Td><span id="spanminCostPriceAmount${idx.index }">${ssmsgmincostPrice}</span><input class="text_input60 noneInput" value="${ssmsgmincostPrice}" id="minCostPriceAmount${idx.index }" type="hidden" name="creditGoodsArray.ssmsgmincostPrice" readonly="readonly">至<span id="spanmaxCostPriceAmount${idx.index }">${ssmsgmaxcostPrice}</span><input value="${ssmsgmaxcostPrice}" id="maxCostPriceAmount${idx.index }" class="text_input60 noneInput" type="hidden" readonly="readonly" name="creditGoodsArray.ssmsgmaxcostPrice" ></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsgbeginAmount}" id="ssmsgbeginAmount${idx.index }" type="text" name="creditGoodsArray.ssmsgbeginAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMin(this,'${idx.index }');" dk6="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','7','')">至<input style="width:50" maxlength="15" value="${ssmsgendAmount}" id="ssmsgendAmount${idx.index }" class="text_input60" type="text" name="creditGoodsArray.ssmsgendAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMax(this,'${idx.index }');" dk7="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','9','')"></Td>
                          <Td>1<input class="text_input60" style="width:50" maxlength="15" value="1" id="goodsquantity${idx.index }" type="hidden" name="creditGoodsArray.ssmsggoodsquantity" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写消费抵扣数量！'},{'Type' : Type.String, 'Formula' : Formula.UINT, 'Message' : '填写的消费抵扣数量不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value == '' ? 1 : this.value).toFixed(0);}else{this.value = '1';}"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsgretailPrice}" id="ssmsgretailPrice${idx.index }" type="text" name="creditGoodsArray.ssmsgretailPrice" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的零售价不正确！'}]" onblur="getSetMealAmount(this)" dk9="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','10','')"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsgexpensecredit}" id="ssmsgexpensecredit${idx.index }" type="text" name="creditGoodsArray.ssmsgexpensecredit" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写金额！'},{'Type' : Type.String, 'Formula' : Formula.UFloat, 'Message' : '填写的金额不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value).toFixed(2);}else{this.value = '0.00';}" dk10="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','11','')"></Td>						  
                          <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgsphul${idx.index }" type="text" name="creditGoodsArray.ssmsgsphul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]"  dk11="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','12','')" onblur="checkData(this)" value="${ssmsgsphul}">至<input maxlength="20" id="ssmsgsphup${idx.index }" class="text_input60" style="width:50" type="text" name="creditGoodsArray.ssmsgsphup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]" dk12="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','13','')" onblur="checkData(this)" value="${ssmsgsphup}"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgcylul${idx.index }" type="text" name="creditGoodsArray.ssmsgcylul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]"  dk13="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','14','')" onblur="checkDataz(this)" value="${ssmsgcylul}">至<input maxlength="20" id="ssmsgcylup${idx.index }" class="text_input60" style="width:50" type="text" name="creditGoodsArray.ssmsgcylup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]" dk14="dk${idx.index }" onKeyPress="dkEnterDown('${idx.index }','0','')" onblur="checkDataz(this)" value="${ssmsgcylup}"></Td>
					...
					<TABLE id=ctl00_PageBody_PostButton cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
                      <TR><td align="left" width="18%">
                          <img src="${ctx }/img/newbtn/btn_tjtjsp_0.png" btn=btn title="添加特价商品" onclick="addRow()">
                          <img src="${ctx }/img/newbtn/btn_delete_0.png" btn=btn title='删除' onclick="del();" >
					...
					<TABLE id="addTablebangding" width="100%"  border=0 align=center cellspacing=1 class="privateBorder">
                      <TBODY>
                         <TR class=table_title align=middle>
						  <TH width="4%" height="26" scope=col>全选<input type="checkbox" id="chks1" name="chks1" onclick="chkAll1()"></TH>
                          <TH width="33%" scope=col>商品信息</TH>
                          <TH width="13%" scope=col>商品原价区间</TH>
                          <TH width="8%" scope=col>套餐价格区间</TH>
                          <TH width="5%" scope=col>购买数量</TH>
                          <TH width="8%" scope=col>球镜区间(上\下)</TH>
						  <TH width="8%" scope=col>柱镜区间(上\下)</TH> 
						  <TH width="8%" scope=col>特价价格</TH>
                         </TR>
 						 <TR class=table_title align=middle>
						  <TH height="26" colspan="7">现价合计：</TH>
                          <TH>
						  <input class="text_input60" style="width:50" id="adjustmentPrice" name="adjustmentPrice" type="text" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value).toFixed(2);}else{this.value = '0.00';}" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请填写特价金额！'}]">
						  </TH>
                         </TR>                   
                       <s:iterator value="salesGoodsArrayList" status="idx">
                       <c:if test="${ssmsgflag eq '1'}">
                         <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          ...
                          <li class="horizontal_onlyRight">
                          <c:if test="${setMealPo.ssmsmclassify eq '1'}">
                            <select clean=clean id="goodscategoryID${idx.index }" xiaofei=xiaofei name="goodscategoryID" onchange="change3(this,'${idx.index }')" tj0="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','1','6')">
        		             <option value="1_0" ${ssmsggoodscategory eq '1' ? 'selected="selected"' : '' }>镜架</option>
      		                 <option value="2_1" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      		                 <option value="3" ${ssmsggoodscategory eq '3' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>镜片</option>
      		                 <option value="3_0" ${ssmsggoodscategory eq '3' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>成品片</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                            <select clean=clean id="goodscategoryID${idx.index }" xiaofei=xiaofei name="goodscategoryID" onchange="change4(this,'${idx.index }')" tj0="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','1','6')">     
      		                 <option value="2_2" ${ssmsggoodscategory eq '2' ? 'selected="selected"' : '' }>配件</option>
      		                 <option value="4" ${ssmsggoodscategory eq '4' ? ((ssmsgiscustomize!='0' && ssmsgiscustomize!='D') ? 'selected="selected"' : '') : '' }>隐形镜片</option>
      		                 <option value="4_0" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq '0' ? 'selected="selected"' : '') : '' }>隐形成品片</option>
      		                 <option value="4_D" ${ssmsggoodscategory eq '4' ? (ssmsgiscustomize eq 'D' ? 'selected="selected"' : '') : '' }>隐形订做片</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '6'}">
                            <select clean=clean id="goodscategoryID${idx.index }" xiaofei=xiaofei name="goodscategoryID" tj0="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">
      		                 <option value="6_6" ${ssmsggoodscategory eq '6' ? 'selected="selected"' : '' }>太阳镜</option>
      		                 <option value="6_8" ${ssmsggoodscategory eq '8' ? 'selected="selected"' : '' }>老花镜</option>
      		                </select>
      		              </c:if>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '4'}">
                              <c:if test="${ssmsggoodscategory eq '4'}">
                              <li class="horizontal_onlyRight" id="ligoodscategoryID401${idx.index }">
                                 <select clean=clean id="goodscategoryID401${idx.index }" name="goodscategoryID401" ${ssmsggoodscategory == '4' ? '' : 'hidn=hidn' } onchange="change41(this,'${idx.index }')" tj1="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','2','6')">      
      		                       <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
      		                       <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>使用类型</option>
      		                       <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>抛弃型分类</option>
      	                         </select>
      	                     ...
      	                     <li class="horizontal_onlyRight" id="ligoodscategoryID402${idx.index }">    
									 <select clean=clean id="goodscategoryID402${idx.index }" name="goodscategoryID402" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">  
			      		               <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>常带型</option>
			      		               <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>抛弃型</option>
									 </select>
							</li>		 
							 <li class="horizontal_onlyRight" id="ligoodscategoryID403${idx.index }">	 
		                             <select clean=clean id="goodscategoryID403${idx.index }" name="goodscategoryID403" ${(ssmsggoodscategory == '4' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">       
			      		                 <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>日抛</option>
			      		                 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>周抛</option>
			      		                 <option value="9" ${ssmsgsmallclass eq '9' ? 'selected="selected"' : '' }>双周抛</option> 
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>月抛</option>
                          ...
                          <c:if test="${setMealPo.ssmsmclassify eq '1' || setMealPo.ssmsmclassify eq '3'}">
                               <li class="horizontal_onlyRight" id="ligoodscategoryID306${idx.index }">
									 <select clean=clean id="goodscategoryID306${idx.index }" name="goodscategoryID306" ${ssmsggoodscategory == '1' ? '' : 'hidn=hidn' } onchange="change32(this,'${idx.index }')" tj1="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','2','6')">
			       		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>                          
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材质</option>	            
			      	                 </select>
			      	            </li>     
			      	            <li class="horizontal_onlyRight" id="ligoodscategoryID307${idx.index }">     
		                             <select clean=clean id="goodscategoryID307${idx.index }" name="goodscategoryID307" ${(ssmsggoodscategory == '1' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">
	      		                 	    <s:iterator value="frameMateriallist"> 
										   <option value="${bfmid}" ${ssmsgsmallclass eq bfmid ? 'selected="selected"' : '' }>${bfmname}</option> 
										</s:iterator>    		      		                 	            
		      	                     </select>
		      	                 ...
		      	                 <li class="horizontal_onlyRight" id="ligoodscategoryID301${idx.index }">     
		                             <select clean=clean id="goodscategoryID301${idx.index }" name="goodscategoryID301" ${ssmsggoodscategory == '3' ? '' : 'hidn=hidn' } onchange="change31(this,'${idx.index }')" tj1="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','2','6')">
			      		                 <option value="" ${ssmsgbigclass eq '' ? 'selected="selected"' : '' }>----请选择----</option>
			                             <option value="1" ${ssmsgbigclass eq '1' ? 'selected="selected"' : '' }>材料分类</option>
			                      		 <option value="2" ${ssmsgbigclass eq '2' ? 'selected="selected"' : '' }>折射率</option>     
			      		                 <option value="3" ${ssmsgbigclass eq '3' ? 'selected="selected"' : '' }>光度分类</option> 	
		      	                 ...
		      	                 <li class="horizontal_onlyRight" id="ligoodscategoryID302${idx.index }">    
		                             <select clean=clean id="goodscategoryID302${idx.index }" name="goodscategoryID302" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '1') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>树脂</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>玻璃</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>PC</option> 			            
		      	                     </select>
		      	                ...
		      	                <li class="horizontal_onlyRight" id="ligoodscategoryID303${idx.index }">     
									<select clean=clean id="goodscategoryID303${idx.index }" name="goodscategoryID303" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '2') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')"> 
			                             <s:iterator value="refractiveSetList">
			                             <option value="${brfname}" ${ssmsgsmallclass eq brfname ? 'selected="selected"' : '' }>${brfname}</option>
			                             </s:iterator>                      			            
									</select>
								 ...
								 <li class="horizontal_onlyRight" id="ligoodscategoryID304${idx.index }">	
									<select clean=clean id="goodscategoryID304${idx.index }" name="goodscategoryID304" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '3') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">
			                             <option value="0" ${ssmsgsmallclass eq '0' ? 'selected="selected"' : '' }>单光</option>
			                      		 <option value="m" ${ssmsgsmallclass eq 'm' ? 'selected="selected"' : '' }>多光</option>     
			      		                 <option value="j_1" ${ssmsgsmallclass eq 'j_1' ? 'selected="selected"' : '' }>青少年渐进</option>
			                      		 <option value="j_2" ${ssmsgsmallclass eq 'j_2' ? 'selected="selected"' : '' }>成人渐进</option> 
								 ...
								 <li class="horizontal_onlyRight" id="ligoodscategoryID305${idx.index }">	
									<select clean=clean id="goodscategoryID305${idx.index }" name="goodscategoryID305" ${(ssmsggoodscategory == '3' && ssmsgbigclass == '4') ? '' : 'hidn=hidn' } tj2="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','6','')">
			                             <option value="1" ${ssmsgsmallclass eq '1' ? 'selected="selected"' : '' }>白色片</option>
			                      		 <option value="2" ${ssmsgsmallclass eq '2' ? 'selected="selected"' : '' }>变色片</option>     
			      		                 <option value="3" ${ssmsgsmallclass eq '3' ? 'selected="selected"' : '' }>偏光片</option>
			      		                 <option value="4" ${ssmsgsmallclass eq '4' ? 'selected="selected"' : '' }>变色偏光片</option> 	
                          ...
                          <Td><span id="spanminCostPriceAmount${idx.index }">${ssmsgmincostPrice}</span><input class="text_input60 noneInput" value="${ssmsgmincostPrice}" id="minCostPriceAmount${idx.index }" type="hidden" name="salesGoodsArray.ssmsgmincostPrice" readonly="readonly" >至<span id="spanmaxCostPriceAmount${idx.index }">${ssmsgmaxcostPrice}</span><input value="${ssmsgmaxcostPrice}" id="maxCostPriceAmount${idx.index }" class="text_input60 noneInput" type="hidden" readonly="readonly" name="salesGoodsArray.ssmsgmaxcostPrice" ></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsgbeginAmount}" id="ssmsgbeginAmount${idx.index }" type="text" name="salesGoodsArray.ssmsgbeginAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMin(this,'${idx.index }');" tj6="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','7','')">至<input style="width:50" maxlength="15" value="${ssmsgendAmount}" id="ssmsgendAmount${idx.index }" class="text_input60" type="text" name="salesGoodsArray.ssmsgendAmount" validate="[{'Type' : Type.String, 'Formula' : Formula.FloatORNULL, 'Message' : '填写的金额不正确！'}]" onblur="validationGoodsCostPriceMax(this,'${idx.index }');" tj7="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','8','')"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsggoodsquantity}" id="goodsquantity${idx.index }" type="text" name="salesGoodsArray.ssmsggoodsquantity" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写特价商品数量！'},{'Type' : Type.String, 'Formula' : Formula.UINT, 'Message' : '填写的特价商品数量不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value == '' ? 1 : this.value).toFixed(0);}else{this.value = '1';}amoumt();" tj8="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','9','')"></Td>
						  <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgsphul${idx.index }" type="text" name="salesGoodsArray.ssmsgsphul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]"  tj9="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','10','')" onblur="checkData(this)" value="${ssmsgsphul}">至<input maxlength="20" id="ssmsgsphup${idx.index }" class="text_input60" style="width:50" type="text" name="salesGoodsArray.ssmsgsphup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写球镜下限有误,请重新填写！'}]" tj10="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','11','')" onblur="checkData(this)" value="${ssmsgsphup}"></Td>
                          <Td><input class="text_input60" style="width:50" maxlength="20" id="ssmsgcylul${idx.index }" type="text" name="salesGoodsArray.ssmsgcylul" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]"  tj11="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','12','')" onblur="checkDataz(this)" value="${ssmsgcylul}">至<input maxlength="20" id="ssmsgcylup${idx.index }" class="text_input60" style="width:50" type="text" name="salesGoodsArray.ssmsgcylup" validate="[{'Type' : Type.String,'Formula' : Formula.SphCylOrNull, 'Message' : '填写柱镜下限有误,请重新填写！'}]" tj12="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','13','')" onblur="checkDataz(this)" value="${ssmsgcylup}"></Td>
        
						  <Td><input class="text_input60" style="width:50" maxlength="15" value="${ssmsgspecialoffer}" id="specialoffer2${idx.index }" type="text" name="salesGoodsArray.ssmsgspecialoffer" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '填写金额！'},{'Type' : Type.String, 'Formula' : Formula.UFloat, 'Message' : '填写的金额不正确！'}]" onblur="if(!isNaN(this.value)) {this.value = new Number(this.value).toFixed(2);}else{this.value = '0.00';}checkCostPrice(this,'${idx.index }');" tj13="tj${idx.index }" onKeyPress="tjEnterDown('${idx.index }','0','')"></Td>
                         </TR>
                         </c:if>
                         </s:iterator>
					  </TBODY>
