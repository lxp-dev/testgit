...
　　 }
	function save(){
		if(checkForm(document.all.glassesFrameForm)){ 
			...
			glassesFrameForm.action = "updateCashCoupon.action";
                      ...
                      <TBODY>
                        <TR>
			              ...
			              <input id="bccbegindate"  
					       ...
					       onFocus="WdatePicker({readOnly:true,maxDate:'#F{$dp.$D(\'bccenddate\')}'})" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请选择使用开始日期！'}]" 
					       value="${cashCouponPo.bccbegindate}"/>至
					       <input id="bccenddate" 
					       name="cashCouponPo.bccenddate" 
					       type="text" class="text_input100" value="${cashCouponPo.bccenddate}" 
					       onfocus="WdatePicker({readOnly:true,minDate:'#F{$dp.$D(\'bccbegindate\')}'})" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请选择使用截止日期！'}]" 
					       />
                            <label style="color:red;">&nbsp;*</label>
                            </TD>			                
			             </TR>
