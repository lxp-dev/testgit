...
<script>
	...
	function getBrandList(preDepId) {
		...
		preSalesForm.action = 'initUpdateSetPreSales.action';
		preSalesForm.submit();
	}
	
	function delDepartment(id) {
		var size = $("img[delete=delete]").size();
		if(parseFloat(size) <= 1){
			alert("当前部门数量不足，无法继续删除！");
			return;
		}
	...
	function batchUpdate() {
		...
		if($("input[id=chk]").length <= 0) {
			...
			$("input[id=chk]").each(function() {
				if($(this).attr("checked") == true) {
					totalCheckedNum = totalCheckedNum + 1;
				}
			});
			if(totalCheckedNum <= 0) {
				alert("请选择品种,后进行修改!");
				return;
			}
		}
		if(!val) {
			...
			quantityArray = $("input[id=quantity]")
			$("input[id=chk]").each(function(index) {
				if($(this).attr("checked") == true) {
					quantityArray[index].value = val;
				}
			});
		}
                      ...
                      <TBODY>
                        <TR height="26">
                          ...
                          <li class="horizontal_onlyRight">
                            <input type="text" class="text_input160" id="planName" name="prePlanPo.planName" value="${requestScope.prePlanPo.planName }" readonly="readonly"/>
                          </li>
                          	...
                          	<s:iterator value="preDepPoList" status="indexFlag">
                          		...
                          		<td>
                          	      <li class="horizontal_onlyRight">
                          			...
                          			<img id="${preDepId }" delete=delete btn=btn src="${ctx }/img/newbtn/delete_0.png" title="删除" onclick="delDepartment('${preDepId }')">
                          		  </li>
                          		</td>
                        ...
                        <TR class=table_title align=middle> 
				   		...
				   		<s:iterator value="preBrandPoList">
				   		  <TR class="row" height="26" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')" >
				   		  	<TD>
				   		  	  <input type="checkbox" id="chk" value="${supplierAndBrandId }">
				   		  	</TD>
				   		  	...
				   		  	<TD><input id="quantity" name="preBrandPo.salesQuantitys" type="text" class="text_input60" value="${salesQuantity }" onblur="amount()"/></TD>
				   		  </TR>
				   		</s:iterator>
                      </TBODY>
                    </TABLE>
