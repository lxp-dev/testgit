...
<script>	
	...
	function update(id){
		...
		var url = "initPreShopSalesUpdate.action?hid="+id+"&moduleID="+$("#moduleID").val();
		if(is_iPad()){
		...
		document.getElementById('popupTitle').innerHTML="【门店计划销售金额更新】";
	}

	function del(id){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		var url = "initPreShopSalesDelete.action?hid="+id+"&moduleID="+$("#moduleID").val();
		showPopWin(url,400,140,topRows,topCols,returnRefresh(true),false);
		document.getElementById('popupTitle').innerHTML="【门店计划销售金额删除】";
	}

	function insert(){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		var url = "initPreShopSalesInsert.action?moduleID="+$("#moduleID").val();
		if(is_iPad()){
			showPopWin(url,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin(url,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		document.getElementById('popupTitle').innerHTML="【门店计划销售金额新增】";
	}

	function search(){
		if(checkForm(goodsForm)){
			$("img").removeAttr("onclick");
			goodsForm.action = "preShopSalesSel.action";
			goodsForm.submit();		
			showLoadingBar();
	    }
	}	
          ...
          <TD bgColor=#ffffff>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR>
				...
				<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->
					<c:if test="${not empty(preShopSalesList)}">
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                              <TBODY>
                                <TR>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                       ...
                       <s:iterator value="preShopSalesList">
