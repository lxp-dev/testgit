...
<script>
	function save(){
	    if(checkForm(document.all.preSalesForm)){ 
			...
			preSalesForm.action = "preShopSalesInsert.action";
			preSalesForm.submit();
		}
	}
	...
	function addRow(goodInfo){
		...
		for(i = 0; i < chk.length; i++){
			if (chk[i].value == (goodInfo.bdpdepartmentid + predate)) return;
		}
		...
		row.className = 'row';
		row.height="26";
		c1.innerHTML = '<input id="chk" name="chk" type="checkbox" value="' + (goodInfo.bdpdepartmentid + predate) + '" >';
        c2.innerHTML = goodInfo.bdpdepartmentname + '<input type="hidden" id="ssepsshopcode" name="preShopSalesTempPo.ssepsshopcode" value="' + goodInfo.bdpdepartmentid +'" />';
        c3.innerHTML = $('#startTime').val() + '<input type="hidden" id="ssepspredate" name="preShopSalesTempPo.ssepsprebgndate" value="' + $('#startTime').val() +'" />';
        c4.innerHTML = $('#endTime').val() + '<input type="hidden" id="ssepspredate" name="preShopSalesTempPo.ssepspreenddate" value="' + $('#endTime').val() +'" />';
        c5.innerHTML = '<input  type="text" id="quantity" onblur="amount()" name="preShopSalesTempPo.ssepssalesprice" class="text_input60" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写计划销售金额!!\'},{\'Type\' : Type.String, \'Formula\' : Formula.UFloat, \'Message\' : \'请重新填写计划销售金额!!\'}]" onblur="toFix(this)" />';	
    }
	...
	function openDepartment(){
		...
		if(is_iPad()){
			showPopWin("selPreShopSalesDepartmentOpen.action?departmentType=1&isclosed=0",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selPreShopSalesDepartmentOpen.action?departmentType=1&isclosed=0",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
                      ...
                      <TBODY>
                        <TR height="26">
			               ...
			               <TD class="table_none" width="20%">
							   ...
							   <li class="horizontal_onlyRight">	
      	                          <img src="${ctx}/img/newbtn/btn_tjmd_0.png" btn=btn title="添加门店" onClick="javascript:openDepartment();">
      	                       </li>
						   </TD>
						</TR>
                      </TBODY>
