...
<script>	
	...
	/*会员详细*/
   // function detailsCustomerInfo(){
	//	showPopWin("","initDetailsCustomerInfo.action?hid="+'${customerID }',850,650, '',null,true);
	//	selectHidden();
	//}

	/*病历信息*/
//	function optometryInformation(){
	//	venditionInformationForm.action = "selectOptometryInformation.action?customerID=${customerID }";
	//	venditionInformationForm.submit();
	//}
</script>
				  ...
				  <c:if test="${not empty(salesBasicList)}">
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      ...
                      <TR class=table_title align=middle>
                      	 ...
                      	 <TH width="8%" scope=col height="28" style="color:black;">销售类型</Th>
                      	 <TH width="8%" scope=col height="28" style="color:black;">处方类型</Th>
                      	 <TH width="8%" scope=col height="28" style="color:black;">销售人员</TH>
                      	 <TH width="8%" scope=col height="28" style="color:black;">原价合计</TH>
                      	 <TH width="8%" scope=col height="28" style="color:black;">折扣金额</TH>
                      	 <TH width="8%" scope=col height="28" style="color:black;">应收金额</TH>
                      	 <TH width="8%" scope=col height="28" style="color:black;">退货状态</TH>
                      	 <TH width="14%" scope=col height="28" style="color:black;">收款日期</TH>
                      </tr>
                       <c:forEach var="basicPo" items="${salesBasicList}" varStatus="basicIndex">
                       
                        <TR class="row" align=middle>
                          <td height="28" ><div onclick="javascript:winPopUp('${basicPo.ssesbsalesid}')" style="cursor:hand"><u>${basicPo.ssesbsalesid }</u></div></td>
                          <td height="28" >${basicPo.ssesbshopcode }</td>
                          <td height="28" >${basicPo.ssesboptometryid }</td>
                          <td height="28" >
                           <c:choose>
			                    ...
			                    <c:when test="${basicPo.ssesborderstype == 5 }">
			                       	   辅料
			                    </c:when>     
			              </c:choose>   
			              </td>
                         ...
                         <c:choose>
                          ...
                          <td height="28" >${basicPo.ssesbsalerid }</td>
                          <td height="28" ><fmt:formatNumber value="${basicPo.ssesbpricesum }" pattern="0.00"/></td>
                          <td height="28" ><fmt:formatNumber value="${basicPo.ssesbdiscountnum }" pattern="0.00"/></td>
                          <td height="28" ><fmt:formatNumber value="${basicPo.ssesbsalesvalue }" pattern="0.00"/></td>
                          <c:if test="${basicPo.ssesbwithdrawflag=='已退货' }">
                          <td height="28" style="color: red;">${basicPo.ssesbwithdrawflag }</td>
                          </c:if>
                          <c:if test="${basicPo.ssesbwithdrawflag=='未退货' }">
                          <td height="28">${basicPo.ssesbwithdrawflag }</td>
                          </c:if>
						  <td height="28">${fn:substring(basicPo.ssesbposdatetime, 0, 10) }</td>                          
                        </TR>                        
					</c:forEach>
                      </TBODY>
                    </TABLE>
