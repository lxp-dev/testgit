...
<script>	
	...
	function selCzk(){
		if(event.keyCode==13){
			var isuse = "";
			window.parent.$("input[czkid=czkid]").each(function (){
				if($(this).val() == $("#czkid").val()&& window.parent.$("input[czkid=czkid]").eq('${indexid }').val() != $("#czkid").val()){
					isuse = "1"
				}
			});

			if(isuse == "1"){
				alert("储值卡号："+$("#czkid").val()+"已被使用！");
				return false;
			}else{
			var salseValue = $('input[name=salseValue]').val();
			$("img").removeAttr("onclick");
			guitarMangermentForm.action = "selCzkPo.action?salseValue="+salseValue+"&czkcardid="+$("#cstczkcardid").val();
	...
	function setValue(){
		...
		var czkid = parseFloat($('input[name=czkid]').val());
		
		if($('input[name=ffje]').val() < 0){
			alert('金额输入错误！');
			$('input[name=ffje]').select();
			return false;
	    }
	...
	$(document).ready(function() { 
	    ...
	    if('${chuzhikaPo.cstczkid}'!=''){ 
	    	document.getElementById('ffje').focus();
	    }
    }); 
                ...
                vAlign=top><DIV id=tabContent__1>
                  <DIV>
				   <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      ...
                      <c:if test="${chuzhikaPo.cstczkid!=null}"> 
                      <TR>
                    ...
                    <table width="100%" id="title2" cellspacing="2">
						<tr height="10" >
							<td align="center">
                        	<img src="${ctx }/img/newbtn/btn_pay_0.png" btn=btn id="jfButton" name="jfButton" title='缴费' onClick="setValue()">
			            	</TD>
							<td >
                        	</TD>
						</tr>
