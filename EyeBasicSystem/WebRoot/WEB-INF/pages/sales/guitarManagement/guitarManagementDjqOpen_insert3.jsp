...
<script>	
	...
	function selCzk(){
		if(event.keyCode==13){
			...
			$("input[id=djqSourceID]").each(function (){
				if($(this).val() == $("#djqid").val()){
					tflag = "1"
				}
			});
			if(tflag != "1"){
				alert("代金券号："+$("#djqid").val()+"需要与结款时使用的代金券相同！");
				return;
			}
			
			$("img").removeAttr("onclick");
			guitarMangermentForm.action = "selDjqPo3.action?djqid="+$("#djqid").val()+"&customerid="+$("#customerid").val();
                  ...
                  <DIV>
				   <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      ...
                      <TR>
                          ...
                          <input type="hidden" id="customerid"  name="customerid" value="${requestScope.customerid}">
                      <c:forEach var="po" items="${cashCouponList}">
                          <input type="hidden" id="djqSourceID"  name="djqSourceID" value="${po.bcccard}">
                      </c:forEach> 
                        </TD>
                      </TR>
                      ...
                      <TR>
                      	...
                      	<TD width="%65" class="table_none" colspan="3">
                            <input class="text_input100" type="text" id="ffje" maxlength="6" name="ffje" value="-${cashCouponPo.bccuseamount}" readonly="readonly">
                        </td>
                      </TR>
                      </c:if>
                      </TBODY>
