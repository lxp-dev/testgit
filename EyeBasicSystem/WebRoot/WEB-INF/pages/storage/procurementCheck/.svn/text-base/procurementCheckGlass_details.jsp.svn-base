...
//		c5.innerHTML = '<input type="text" class="text_input60" onBlur="needAmount()" maxlength="18" name="needNumber" value="' + goodInfo.bgigoodsquantity + '"  validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'商品数量应为整数！\'}]"/>';
    ...
    //页面内扫码，并将条码付到对应的条码框中
  	...
  	function check(obj,type){
  		...
  		//判断收货数量是否大于订货数量
  		if(type == '1'){
	  		if(parseFloat($('#'+index+'cstpoeordergoodsquantity').val()) < parseFloat(obj.val())){
	  			alert("收货数量不能大于订货数量!");
	  			obj.val('0');
	  			obj.focus();
	  		}
  	...
  	function rowamount(obj){
  		...
  		//计算不合格率
  		$('#'+index+'cstpoeunqualifiedrate').val((((parseFloat(shnumber)-parseFloat(bhgnumber))/parseFloat(shnumber))*100).toFixed(2)+'%');
  	}
...
//		alert(StatusID);
			<s:iterator value="procurementCheckPos">
		     var json = {'goodsid':'${cstpoegoodsid}',
		     				...
		     				'dnumber':'${cstpoeordergoodsquantity}','rnumber':'${cstpoegoodsquantity}','bc':'${cstpoebluminosity}','sc':'${cstpoesluminosity}','orderNums':'${cstpcordergoodsquantity}','receiveNums':'${cstpcgoodsquantity}',
		     				'hd':'${cstpoethickness}','bz':'${cstpoepack}','wg':'${cstpoeapp}','hg':'${cstpoequalifiedquantity}','bhg':'${cstpoeunqualifiedquantity}','hgl':'${cstpoeunqualifiedrate}'
		     			};
...
//				window.close();hidePopWin();selectShow();
		...
		row.height="26";
		c1.height="26";
        c1.innerHTML = goodInfo.goodsid+'<input type="hidden" id="'+index+'cstpoegoodsid" name="procurementCheckEntryPo.cstpoegoodsids"  value="' + goodInfo.goodsid +'" />';
        c2.innerHTML = goodInfo.goodsname;	
        if(show==0){
        	c3.innerHTML = goodInfo.dnumber+'<input type="hidden" isshow=isshow class="text_input60 number" id="'+index+'cstpoeordergoodsquantity" readonly="readonly" name="procurementCheckEntryPo.cstpoeordergoodsquantitys" value="' + goodInfo.dnumber +'" onblur="amount(this);"/>';
			c4.innerHTML = goodInfo.rnumber+'<input type="hidden" class="text_input60 number" isshow=isshow maxlength="18" id="'+index+'cstpoegoodsquantity" name="procurementCheckEntryPo.cstpoegoodsquantitys" value="' + goodInfo.rnumber +'" onblur="check($(this),1);rowamount($(this));amount(this);" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'商品数量应为整数！\'}]"/>';
        }else{
        	c3.innerHTML = '<input type="hidden" class="text_input60 number" id="'+index+'cstpoeordergoodsquantity" readonly="readonly" name="procurementCheckEntryPo.cstpoeordergoodsquantitys" value="" onblur="amount(this);"/>';
        	c4.innerHTML = '<input type="hidden" class="text_input60 number" maxlength="18" id="'+index+'cstpoegoodsquantity" name="procurementCheckEntryPo.cstpoegoodsquantitys" value="" />';
        }
		c5.innerHTML = goodInfo.bc + '<input type="hidden" class="text_input100 number" id="'+index+'cstpoebluminosity" name="procurementCheckEntryPo.cstpoebluminositys" value="' + goodInfo.bc +'"/>';
		c6.innerHTML = goodInfo.sc + '<input type="hidden" class="text_input100 number" id="'+index+'cstpoesluminosity" name="procurementCheckEntryPo.cstpoesluminositys" value="' + goodInfo.sc +'"/>';
		c7.innerHTML = goodInfo.hd + '<input type="hidden" class="text_input60 number" id="'+index+'cstpoethickness" name="procurementCheckEntryPo.cstpoethicknesss" value="' + goodInfo.hd +'"/>';
		if(goodInfo.bz=='1')
		{
			c8.innerHTML = '√';
		}else{
		...
		c10.innerHTML = goodInfo.hg +'<input type="hidden" class="text_input60 number" id="'+index+'hg" style="background-color: #E1E3E6;" readonly="readonly" name="hg" value="' + goodInfo.hg +'" onblur="amount(this);"/>';
		c11.innerHTML = goodInfo.bhg +'<input type="hidden" class="text_input60 number" id="'+index+'cstpoeunqualifiedquantity" name="procurementCheckEntryPo.cstpoeunqualifiedquantitys" onblur="check($(this),2);rowamount($(this));amount(this);" value="' + goodInfo.bhg +'"/>';
		c12.innerHTML = goodInfo.hgl +'<input type="hidden" class="text_input60 number" style="background-color: #E1E3E6;" readonly="readonly" id="'+index+'cstpoeunqualifiedrate" name="procurementCheckEntryPo.cstpoeunqualifiedrates" value="' + goodInfo.hgl +'"/>';
    }
                  ...
                  border=0  align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR>
                          ...
                          <TD width="24%" class="table_none">&nbsp;${procurementCheckPos[0].cstpoebillid}</TD>
                          <TD class="table_body" height="26" width="9%">制单人</TD>
                          <TD class="table_none" width="24%">&nbsp;${procurementCheckPos[0].cstpoecheckername }</TD>
                          <TD class="table_body" width="9%" height="26">单据日期</TD>
                          <TD  class="table_none">&nbsp;${fn:substring(procurementCheckPos[0].cstpoebilldate,0,16) }
                          </TD>
                         </TR>
                        <TR>
                         <TD class="table_body" width="8%" height="26">制造商</TD>
                         <TD  class="table_none" width="25%">&nbsp;${procurementCheckPos[0].cstpoesuppliername }
                         </TD>					       
                         <TD class="table_body" width="8%" height="26">收入仓位</TD>
                         <TD  class="table_none" width="25%">&nbsp;${procurementCheckPos[0].cstpoeinstockname }
                          <input type="hidden" name="procurementCheckPo.cstpoeinstockid" value="${procurementCheckPos[0].cstpoeinstockid }">
                         </TD>
                         <TD class="table_body" width="8%" height="26">收货单号</TD>
                         <TD  class="table_none" width="25%">&nbsp;${procurementCheckPos[0].cstpoebillsource }
                          <input type="hidden" name="procurementCheckPo.cstpoebillsource" value="${procurementCheckPos[0].cstpoebillsource }">
                         </TD>					       
                        </TR>
                      </TBODY>
                    </TABLE>
					...
					<table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        ...
                        <TR class=table_title align=middle> 
					    	...
					    	<TH scope=col >${procurementCheckPos[0].cstpoeordergoodsquantity }</TH>
					    	<TH scope=col >${procurementCheckPos[0].cstpoegoodsquantity }</TH>  
