...
<script>
	...
	function save(){
		var i = 0;
		$('input[name=procurementCheckEntryPo.cstpoesluminositys]').each(function(){
			if($(this).val()==''){
				$(this).focus();
				i = i + 1;
			}
		...
		if(!checkBrightness()) {
			alert("实测光度格式错误,请参照标称光度!");
			return;
		}
		$('input[name=procurementCheckEntryPo.cstpoethicknesss]').each(function(){
			if($(this).val()==''){
				$(this).focus();
		...
		if(checkForm(document.all.allocationForm)){
			...
			allocationForm.action = "procurementCheckGlassInsert.action";
			allocationForm.submit();
		}
	}

	function checkBrightness() {
		var regBri = /^[+-]?\d+\.\d{2,2}[+-]{1,1}\d+\.\d{2,2}$/;
		var flag = true;
		$("input[name=procurementCheckEntryPo.cstpoesluminositys]").each(function() {
			if(regBri.exec($(this).val()) == null) {
				flag = false;
				$(this).select();
			}
		});
    ...
    //页面内扫码，并将条码付到对应的条码框中
  	...
  	function amount1(item){
  		//订购数量合计
  		var ordergoodsquantityamount = 0;
		$('input[name=procurementCheckEntryPo.cstpoeordergoodsquantitys][isshow=isshow]').each(function (){
			ordergoodsquantityamount = parseFloat(ordergoodsquantityamount) + parseFloat($(this).val());
		});
		$('#goodsquantityTotal1').text(ordergoodsquantityamount);
		...
		$('input[name=procurementCheckEntryPo.cstpoegoodsquantitys][isshow=isshow]').each(function (){
  	...
  	function checkReceiveNum(index) {
  	  	var quantity = $("#" + index + "cstpoegoodsquantity").val();
  	  	if(!quantity || isNaN(quantity) || quantity.indexOf(".") >= 0) {
	  	  	$("#" + index + "cstpoegoodsquantity").val(0);
  	  	}
  	}
  	...
  	function check(obj,type){
  		...
  		//判断数量为空时处理
  		if(obj.val() == '' || obj.val() == null || isNaN(obj.val()) || obj.val().indexOf(".") >= 0){
  			obj.val('0');
  		}
  	...
  	function rowamount(obj){
  		...
  		//计算不合格商品数量
  		$('#'+index+'hg').val(1-parseFloat(bhgnumber));
  		
  		//计算不合格率
  		$('#'+index+'cstpoeunqualifiedrate').val((((parseFloat(bhgnumber))/1)*100).toFixed(2)+'%');
  	}
	...
	function openValues2(objValue,chaasupplier,bspsuppliername,did){
		allocationForm.action="procurementCheckReceiptBillChange.action?billID="+objValue+"&did="+did;
		allocationForm.submit();
	}
	...
	function openValues3(objValue){
		...
		var StatusID='${StatusID}';
			<s:iterator value="procurementCheckPos">
		     ...
		     var count = Math.ceil(json.rnumber*0.05);
			...
			if(StatusID==1)
			{
				document.getElementById('inputBarCodeSelect').value="2";
				isInputBarCode(document.getElementById('inputBarCodeSelect').value);
			}
		...
		function addRow1(goodInfo,show){
		var table = document.getElementById('addTable');
		var index = table.rows.length - 1;
		// 商品id去重
		var chk=document.getElementsByName("chk");
        ...
        if(show==0){
        	c4.innerHTML = goodInfo.dnumber+'<input type="hidden" isshow=isshow class="text_input60 number" id="'+index+'cstpoeordergoodsquantity" readonly="readonly" name="procurementCheckEntryPo.cstpoeordergoodsquantitys" value="' + goodInfo.dnumber +'" onblur="amount(this);"/>';
			c5.innerHTML = '<input type="text" class="text_input60 number" isshow=isshow maxlength="18" id="'+index+'cstpoegoodsquantity" name="procurementCheckEntryPo.cstpoegoodsquantitys" value="' + goodInfo.rnumber +'" onblur="checkReceiveNum('+index+');check($(this),1);rowamount($(this));amount(this);" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'商品数量应为整数！\'}]"/>';
        }else{
        	c4.innerHTML = '<input type="hidden" class="text_input60 number" id="'+index+'cstpoeordergoodsquantity" readonly="readonly" name="procurementCheckEntryPo.cstpoeordergoodsquantitys" value="" onblur="amount(this);"/>';
        	c5.innerHTML = '<input type="hidden" class="text_input60 number" maxlength="18" id="'+index+'cstpoegoodsquantity" name="procurementCheckEntryPo.cstpoegoodsquantitys" value="" onblur="checkReceiveNum('+index+')"/>';
        }
        
		c6.innerHTML = '<input type="text" class="text_input100 number" id="'+index+'cstpoebluminosity" name="procurementCheckEntryPo.cstpoebluminositys" readonly="readonly" value="' + goodInfo.bc +'"/>';
		c7.innerHTML = '<input type="text" class="text_input100 number" id="'+index+'cstpoesluminosity" name="procurementCheckEntryPo.cstpoesluminositys" value="' + goodInfo.bc +'" Enter="'+index+'" onkeydown="OnKeyDownEnter(this)" maxLength="18"/>';
		
		var cd = "";
		if(parseFloat(goodInfo.sph) > 0){
			cd = 1.2;
		}else{
			cd = 1.1;
		}
		c8.innerHTML = '<input type="text" class="text_input60 number" id="'+index+'cstpoethickness" name="procurementCheckEntryPo.cstpoethicknesss" value="' + cd +'" maxLength="7"/>';
		c9.innerHTML = '<select id="'+index+'cstpoepack" name="procurementCheckEntryPo.cstpoepacks"><option value="1">√</option><option value="0">×</option></select>';
		c10.innerHTML = '<select id="'+index+'cstpoeapp" name="procurementCheckEntryPo.cstpoeapps"><option value="1">√</option><option value="0">×</option></select>';
		c11.innerHTML = '<input type="text" class="text_input60 number" id="'+index+'hg" style="background-color: #E1E3E6;" readonly="readonly" name="hg" value="1" onblur="amount(this);"/>';
		c12.innerHTML = '<input type="text" class="text_input60 number" id="'+index+'cstpoeunqualifiedquantity" name="procurementCheckEntryPo.cstpoeunqualifiedquantitys" onblur="check($(this),2);rowamount($(this));amount(this);" value="0"/>';
		c13.innerHTML = '<input type="text" class="text_input60 number" style="background-color: #E1E3E6;" readonly="readonly" id="'+index+'cstpoeunqualifiedrate" name="procurementCheckEntryPo.cstpoeunqualifiedrates" value="0.00%"/>';
    }
...
<input type="hidden" id="cstpoebilltype" name="procurementCheckPo.cstpoebilltype" value="1">
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
                  <DIV>
                    ...
                    <TABLE cellSpacing=1 cellPadding=3 width="100%" align=center border=0  align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR>
                        ...
                        <TR>
                          ...
                          <TD  class="table_none">
	                          &nbsp;${procurementCheckPos[0].cstpoesuppliername }
                          ...
                          <TD  class="table_none" >
                          	  &nbsp;${procurementCheckPos[0].cstpoeinstockname }
                          ...
                          <TD  class="table_none" >
                          	  &nbsp;${procurementCheckPos[0].cstpoebillsource }
                      ...
                      <TBODY>
                        <TR>
                          <TD align="left" width="60%">
                           <img src="${ctx }/img/newbtn/btn_ashdtjsp_0.png" btn=btn title="按收货单添加商品" onClick="javascript:openGoodReceipt('j');">				  
					...
					<table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR class=table_title align=middle height="26">  
                          ...
                          <TH width="8%" scope=col>标称光度</TH>
                          <TH width="8%" scope=col>实测光度</TH>
                          <TH width="5%" scope=col>中心厚度</TH>
                    ...
                    <TABLE id=ctl00_PageBody_PostButton cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
					   <TR>
						  <TD align="left"><img id="button1" src="${ctx}/img/newbtn/btn_save_0.png" btn=btn title='保存' onclick="save();">
                          		</TD>
                        </TR>                     
                    </TABLE>
                  </DIV>
