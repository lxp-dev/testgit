...
if($("#title1").is(":visible"))
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initApplyAllocationUpdate.action?hid="+id+"&moduleID="+moduleID,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initApplyAllocationUpdate.action?hid="+id+"&moduleID="+moduleID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【采购申请信息更新】";
	}
	function search(){
		$("img").removeAttr("onclick");
		allocationForm.action = "selApplyAllocation.action";
		allocationForm.submit();
		showLoadingBar();
	}	
	function details(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("allocationApplyDetails.action?hid="+id+"&moduleID="+moduleID,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("allocationApplyDetails.action?hid="+id+"&moduleID="+moduleID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		document.getElementById('popupTitle').innerHTML="【采购申请信息详细】";
	}
	function insert(){
		var moduleID = document.getElementById('moduleID').value;
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initApplyAllocationInsert.action?moduleID="+moduleID,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initApplyAllocationInsert.action?moduleID="+moduleID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【采购申请信息新增】";
	}
	function del(id){
		var moduleID = document.getElementById('moduleID').value;
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initApplyAllocationDelete.action?hid="+id+"&moduleID="+moduleID,400,140,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initApplyAllocationDelete.action?hid="+id+"&moduleID="+moduleID,400,140,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【采购申请信息删除】";
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：采购申请管理</TD>
            <td align="right" valign="bottom">&nbsp;
             <c:if test="${permissionPo.keyb=='1'}">
            	<img src="${ctx }/img/newbtn/btn_allocationapplyinsert_0.png" btn=btn title="采购申请新增" onClick="insert();">
             </c:if>
                    	...
                    	<TR>
			               ...
			               <TD class="table_none">
	                           <li class="horizontal_onlyRight">
							   		<input id="bspsuppliername" class="text_input160" name="bspsuppliername" readonly="readonly" value="${bspsuppliername }"/>
							   		<input type="hidden" id="chaasupplier" name="chaasupplier" value="${chaasupplier }" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '制造商不能为空！'}]"/>
							   </li>
						       <li class="horizontal_onlyRight">
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
						  ...
						  <TH width="8%" scope=col>商品类型</TH>
						  <TH width="5%" scope=col>制造商</TH>
