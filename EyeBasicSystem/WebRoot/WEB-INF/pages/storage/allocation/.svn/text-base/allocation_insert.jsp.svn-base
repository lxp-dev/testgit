...
<script>
	...
	if(checkForm(document.all.allocationForm)){ 
       ...
       var cshaaoutstockid1=document.getElementById('cshaaoutstockid').value;
       if(cshaaoutstockid1 == ''){
	       	alert("请选择调出仓位！ ");
		...
		allocationForm.action = "insertAllocation.action";
	...
	*/
    ...
    function addRow(goodInfo){
		...
		<c:if test="${systemParameterPo.fspbarcodetype==3}"> 
		    c7str = '<input type="text" class="text_input60 number" id="quantity" style="text-align:center;" goodsquantity="goodsquantity'+ index +'" ondblclick="goodsquantityAdd(this)" maxlength="18" name="goodsInfoTempPo.goodsquantity" onkeydown="OnKeyDownEnter(this)" value="" dbs="dbs'+index+'" onblur="jshj('+index+');amount1();blackToRed(this);" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'商品数量应为整数！\'}]"/>'+
    ...
    function addRowUpdateNumber(goodInfo){
		...
		c4.innerHTML = goodInfo.bgispec + '<input type="hidden" id="bgispec" name="goodsInfoTempPo.spec" value="' + goodInfo.bgispec +'" />'; 
		c5.innerHTML = '<span name="requirenumSpan">' + requirequantity + '</span><input type="hidden" id="requirequantity" name="goodsInfoTempPo.cshaaerequirequantity" value="' + requirequantity +'" />'; 
		c6.innerHTML = '<a href=\'javascript:getCallAbleNum("'+goodInfo.bgigoodsid.toUpperCase()+'","'+goodInfo.bgigoodsbarcode+'");\'><span name="maxquantitySpan">' + goodInfo.cshaaemaxquantity+'</span></a><input type="hidden" id="maxquantity" name="goodsInfoTempPo.cshaaemaxquantity" value="' + goodInfo.cshaaemaxquantity +'" />';
		var htmlStr = '';
		var goodsnumber = '';
		var goodsprice = '';
        var c7str = '';
		...
		<c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}"> 
			...
			c7str = '<input type="text" class="text_input60 number" maxlength="18" dbs="dbs'+index+'" id="quantity" style="text-align:center;" goodsquantity="goodsquantity'+ index +'" ondblclick="goodsquantityAdd(this)" maxlength="18" id="quantity" name="goodsInfoTempPo.goodsquantity" onkeydown="OnKeyDownEnter(this)" value="'+goodsnumber+'" onblur="jshj('+index+');amount1();blackToRed(this);" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'请填写正确的商品数量！\'}]"/>';
		</c:if>
		
		<c:if test="${systemParameterPo.fspbarcodetype==3}"> 
		    c7str = '<input type="text" class="text_input60 number" maxlength="18" dbs="dbs'+index+'" id="quantity" maxlength="18" style="text-align:center;" goodsquantity="goodsquantity'+ index +'" ondblclick="goodsquantityAdd(this)" name="goodsInfoTempPo.goodsquantity" onkeydown="OnKeyDownEnter(this)" value="" onblur="jshj('+index+');amount1();blackToRed(this);" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'商品数量应为整数！\'}]"/>'+
							'<input type="hidden" id="selectGbc'+index+'" pid="pcbarcode" name="goodsInfoTempPo.goodsbarcode" value="'+goodInfo.bgigoodsbarcode+'"/>';
		</c:if>
    ...
    function showSubMenu(obj,id) {  
    	$('#cshaainstockid').load("getAjaxStock.action?id="+ id);
	...
	function openValues1(objValue,isShowBarcode){
		...
		allocationForm.action="selApplyAllocationBills.action?billID="+objValue+"&isShowBarcode="+isShowBarcode;
		allocationForm.submit();
	}
	/**
	 * 开窗赋值实现方法
	...
	function openValues2(objValue,chaasupplier,bspsuppliername,did){
		document.getElementById('cshaabillassociation').value=objValue;
		allocationForm.action="selReceiptBills.action?billID="+objValue+"&did="+did;
		allocationForm.submit();
	}
	...
	$(document).ready(function(){
		...
		var cshaaindepartmentid=document.getElementById('cshaaindepartmentid').value;
		if(cshaaindepartmentid!=""){
			showSubMenu(document.getElementById('cshaaindepartmentid'),cshaaindepartmentid);
		}
		amount1();
		needAmount();
	...
	function openGoodSingleAllForApp(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initGoodsSingleSelAllForApp.action?isShowBarcode=n&dptID=${person.departmentID}&categoryID_open=" + categoryID_open,970,scopenValues1reen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
	...
	function openGoodReceipt(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initGoodsReceipt.action?categoryID_open=" + categoryID_open + "&supplierID_open=" + supplierID,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initGoodsReceipt.action?categoryID_open=" + categoryID_open + "&supplierID_open=" + supplierID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function openGoodAllocation(flag){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initAllocationGoods.action?categoryID_open=" + categoryID_open+"&allflag="+flag,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initAllocationGoods.action?categoryID_open=" + categoryID_open+"&allflag="+flag,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function openGoodsInfoForSOUT(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initSelGoodsInfoForSOUT.action?app=app&whichretail=1&select_retail=1&isShowBarcode=n&outstockid="+$("#cshaaoutstockid").val()+"&categoryID_open=" + categoryID_open+"&indptid=" + $('#cshaaindepartmentid').val(),970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initSelGoodsInfoForSOUT.action?app=app&whichretail=1&select_retail=1&isShowBarcode=n&outstockid="+$("#cshaaoutstockid").val()+"&categoryID_open=" + categoryID_open+"&indptid=" + $('#cshaaindepartmentid').val(),screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
...
<input type="hidden" id="allactionflag" name="allocationPo.cshaaflag" value="${allocationPo.cshaaflag}">
                  ...
                  border=0>
                      <TBODY>
                        <TR>
                          ...
                          <TD width="9%" class="table_body" height="26">发出部门</TD>
                          <TD class="table_none">
	                           ${allocationPo.cshaaoutdepartmentname} 
	                           <input name="allocationPo.cshaaoutdepartmentname" type="hidden" value="${allocationPo.cshaaoutdepartmentname}"/>
	                      ...
	                      <!--<c:if test="${person.departmenttype eq '3' }" >
	                           <select id="cshaaoutdepartmentid" name="allocationPo.cshaaoutdepartmentid" onchange="showSubMenu1(this,this.options[this.options.selectedIndex].value);" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '发出部门不能为空！'}]">
	      		                 <option value="" retailtype=''>----请选择----</option>
	      		                 <s:iterator value="indepartmentsList">
					               <option value="${bdpdepartmentid}" retailtype='${bdpwhichretail }' companyNature='${bdpcompanynature}' companyID='${bdpcompanysid}'>(${bdpdepartmentid})${bdpdepartmentname}</option>
		     	                 </s:iterator>
	      	                   </select>
	      	              </c:if>-->
      	                  </TD>
                        ...
                        <tr>
							   ...
							   <select id="cshaaoutstockid" name="allocationPo.cshaaoutstockid">
							    <option value="">----请选择----</option>
	      		                 <s:iterator value="outwarehouselist">
	      		                   	<option value="${bwhid}" ${(warehouseConfigurationPo.bwcstockid9== bwhid || allocationPo.cshaaoutstockid== bwhid)  ? 'selected="selected"' : '' }>${bwhwarehouseName}</option>
		     	                 </s:iterator>
	      	                   </select>
						   <!--<c:if test="${person.departmenttype eq '3' }" >
							   <select id="cshaaoutstockid" name="allocationPo.cshaaoutstockid">
                           ...
                           <TD class="table_body" height="26">接收部门</TD>
                           <TD class="table_none">
                            <select id="cshaaindepartmentid" name="allocationPo.cshaaindepartmentid" onchange="showSubMenu(this,this.options[this.options.selectedIndex].value);deleteAllItem(this);" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '接收部门不能为空！'}]">
      		                 <option value="" retailtype=''>----请选择----</option>
      		                 <s:iterator value="indepartmentsList">
                        ...
                        <TR>
                          ...
                          <c:if test="${systemParameterPo.fspisallocationcategory eq '1'}">
			               ...
			               <TD class="table_none">
                            ...
                            <select id="goodscategoryID" name="allocationPo.cshaacategoryid">
      		                 <option value="">----请选择----</option>
      		                 <s:iterator value="goodsCategorys">
				               <option value="${bgcid}" ${allocationPo.cshaacategoryid == bgcid ? 'selected="selected"':'' }>${bgcgoodscategoryname}</option>
	     	                 </s:iterator>
      	                   </select>
			               </TD>	
			               </c:if>				       
                    ...
                    <TABLE id=ctl00_PageBody_PostButton cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
                      <TBODY>
                        <TR>
						  ...
						  <img src="${ctx }/img/newbtn/btn_goodsadd_0.png" btn=btn title="按单品添加商品" onClick="javascript:openGoodSingle();">
						  </c:if>
						  <c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}"> 
						  ...
						  <td align="right">
						  <img src="${ctx}/img/newbtn/btn_delete_0.png" btn=btn id="del" title="删除" onClick="deleteitem();">
						  </td>
                        </TR>
                        <tr>
                        <td align="left" colspan="3"><font color="red" size="2"><b>
                        <c:choose>
					...
					<table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH width="8%" scope=col>调拨数量</TH>
                        <c:if test="${permissionPo.keyi eq '1'}">  
                          <TH width="8%" scope=col id="dj">调入单价</TH>
                          <TH width="8%" scope=col id="hj">调入价</TH> 
                        </c:if>  
                           <c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}">                
