...
<script>	
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initSalesOutDetails.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initSalesOutDetails.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function update(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initSalesOutUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initSalesOutUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【销售出库修改】";
	}
	function search(){
		$("img").removeAttr("onclick");
		salesOutForm.action = "selSalesOut.action";
		salesOutForm.submit();
		showLoadingBar();
	}
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initSalesOutInsert.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initSalesOutInsert.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【销售出库新增】"
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		showPopWin("initSalesOutDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",500,150,topRows,topCols,returnRefresh(true),true);
		document.getElementById('popupTitle').innerHTML="【销售出库删除】"
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：销售出库管理</TD>
             <TD align="right" vAlign=bottom>&nbsp;
            <c:if test="${(permissionPo.keya==1)}">
            	<img src="${ctx }/img/newbtn/btn_xsckadd_0.png" btn=btn title="销售出库新增" onClick="insert()">
           </c:if>
...
<img src="${ctx }/img/newbtn/btn_today_0.png"  btn=btn title="今天" onClick="today()"></li>
			               ...
			               <TD class="table_none">
                            <select id="customerid" name="customerid">
                             	...
                             	<c:if test="${not empty(customerList)}">
                    	           ...
                    	           <OPTION value="${bctid}" ${requestScope.customerid!= bctid  ? '' : 'selected="selected"' } >(${bctid})${bctcustomername}</OPTION>
                    	          </s:iterator>
                    	        </c:if>
      	                   </select>
      	                   </TD>--%>
                    	...
                    	<TR>
			               ...
			               <TD class="table_none">
                            <input class="text_input160" type="text"  id="remark" name="remark" value="${requestScope.remark}" maxlength="20">
			               </TD>
                        </TR>
                        <TR>
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TH width="9%" height="26" scope=col colspan="3">操作</TH>
						  ...
						  <!-- <TH width="13%" scope=col>客户名称</TH> -->						
                          <TH width="20%" scope=col>销售部门</TH>
