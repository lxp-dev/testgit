...
<title>商品批发调价管理</title>
</head>
<script>	
	function save(){
	if(checkForm(document.all.wholesalePriceForm)){ 
	if($('#adjustmentPriceWay').val() == '1' && $('#effecticeTime').val()==''){
		...
		wholesalePriceForm.action = "wholesalePriceUpdate.action";
		wholesalePriceForm.submit();
		}
	}

	function wholesalePrice_open(){
		var supplierID='';
		var categoryID_open='';	
	...
	function openGoodSingle(){
		...
		if(is_iPad()){
			showPopWin("initGoodsSingleWSel.action?categoryID_open=" + categoryID_open + "&supplierID_open=" + supplierID,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initGoodsSingleWSel.action?categoryID_open=" + categoryID_open + "&supplierID_open=" + supplierID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function addRow(goodInfo){
		...
		c6.innerHTML = goodInfo.bgiwholesaleprice+ '<input type="hidden" id="retailprice" name="goodsInfoTempPo.costprice" value="' + goodInfo.bgiwholesaleprice +'" />';
    ...
    function setWholesalePrice(){
        var price=document.getElementById('wholesalePrice').value;
        if (isNaN(price)){
            alert("请输入正确的价格!");
            return;
        } 
	...
	*/
	...
	$(document).ready(function(){
	    ...
	    var json = {'goodsid':'${bgigoodsid}','goodsName':'${bgigoodsname}','brandName':'${bgibrandname}','spec':'${bgispec}','costPrice':'${bgicostprice}','bgiwholesaleprice':'${bgiwholesaleprice}',
	...
	function addtdgoods(){
		wholesalePriceForm.action = "addWholesalePrice.action";
		wholesalePriceForm.submit();
	}
...
//		    obj.style.display = "Block";
		    $('#wholesalePriceWay').val('1');  //石英调价
	    }else if(realTime.checked){
	    	$('tr[id=showtr]').hide();
//		    obj.style.display = "None";
		    $('#wholesalePriceWay').val('0');  //实时调价
	    }		    
	}  
...
<form name="wholesalePriceForm" method="post" action="">
                ...
                vAlign=top><DIV id=tabContent__1>
                  ...
                  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                    ...
                    <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
                          ...
                          <TD width="23%" class="table_none"><input class="text_input160" id="cshaabillid" name="wholesalePricePo.cprwpbillid" value="${wholesalePricePo.cprwpbillid }" readonly="readonly"></TD>
                          <TD width="10%" class="	table_body">制单日期</TD>
                          <TD width="23%" class="table_none">
                         ${wholesalePricePo.cprwpcreatedate}
                        </TR>
                        <TR>
                          <TD width="10%" class="table_body">调价方式</TD>
					
					      <TD width="24%" class="table_none">
					        <input type="hidden" id="wholesalePriceWay" name="wholesalePricePo.cprwpflag" value="${wholesalePricePo.cprwpflag }">
                          	实时调价&nbsp;<input type="radio" id="realTime" name="radioBtn" onClick="isshow()" ${wholesalePricePo.cprwpflag == '0' ? 'checked="checked"' : ''} >&nbsp;&nbsp;&nbsp;
                                                                          按生效日期调价&nbsp;<input type="radio" id="fixedTime" name="radioBtn" onClick="isshow()" ${wholesalePricePo.cprwpflag == '1' ? 'checked="checked"' : ''}>
                          </TD>
    
                          <TD width="9%" class="table_body">制单人</TD>
                          <TD width="41%" class="table_none">${wholesalePricePo.cprwpcreatepersonname }</TD>
                        </TR>
                        <TR id="showtr" ${wholesalePricePo.cprwpflag == '0' ? 'style="display: none"' : ''}>
                          <TD width="9%" class="table_body">生效日期</TD>
                          <TD width="41%" class="table_none" colSpan=6>
                           <input id="effecticeTime"
					       name="wholesalePricePo.cprwpeffectivedate" 
					       value="${wholesalePricePo.cprwpeffectivedate }"
					       type="text" class="text_input200" 
					     onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',minDate:'${effectiveTime}'})" /></TD>
                        </TR>
                        <TR>
                          <TD class="table_body">备注</TD>
                          <TD class="table_none" colSpan=6><textarea  name="wholesalePricePo.cprwpremark" id="textarea2">${wholesalePricePo.cprwpremark }</textarea></TD>
                        </TR>
                      </TBODY>
                    </TABLE>
                    <TABLE id=ctl00_PageBody_PostButton cellSpacing=1 cellPadding=3 width="100%" align=center border=0>
                      <TR>
                        <TD align="left">
							<img name="button22" src="${ctx}/img/newbtn/btn_addgoods_0.png" btn=btn onmouseover="JavaScript:$(this).attr('src','${ctx }/img/newbtn/btn_addgoods_1.png');" onmouseout="JavaScript:$(this).attr('src','${ctx }/img/newbtn/btn_addgoods_0.png');" title="单品添加商品" 
						  onClick="javascript:openGoodSingle();">
                            <img src="${ctx}/img/newbtn/btn_addtwogoods_0.png" btn=btn onmouseover="JavaScript:$(this).attr('src','${ctx }/img/newbtn/btn_addtwogoods_1.png');" onmouseout="JavaScript:$(this).attr('src','${ctx }/img/newbtn/btn_addtwogoods_0.png');" title="二维表添加商品" id="2D"
                      ...
                      <TBODY>
                        ...
                        <TR class=table_title align=middle>
                         <TH width="7%" height="26" scope=col>全选<input type="checkbox" id="chks" onclick="chkAll()"></TH>                        
                          ...
                          <TH width="10%" scope=col>现批发价格</TH>
                          <TH width="10%" scope=col>调整价格</TH>
                          </TR>
                        <c:forEach var="wholesalePriceEntry" items="${wholesalePriceEntryList}">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
						  <TD height="26"><input id="chk" name="chk" type="checkbox" value="${wholesalePriceEntry.cprwpebarcode }" ></TD>
                          <TD>${wholesalePriceEntry.cprwpegoodsid }<input name="goodsInfoTempPo.goodsid" value="${wholesalePriceEntry.cprwpegoodsid }" type="hidden"/></TD>
						  <TD>${wholesalePriceEntry.cprwpegoodsname }<input name="goodsInfoTempPo.goodsname" value="${wholesalePriceEntry.cprwpegoodsname }" type="hidden"/></TD>
                          <TD>${wholesalePriceEntry.cprwpebbdbrandname }<input name="goodsInfoTempPo.goodsbarcode" value="${wholesalePriceEntry.cprwpegoodsbarcode}" type="hidden"/></TD>
                           <TD>${wholesalePriceEntry.cprwpebbdspec }</TD>
                          <TD>${wholesalePriceEntry.cprwpecostprice}<input name="goodsInfoTempPo.costprice" value="${wholesalePriceEntry.cprwpecostprice}" type="hidden"/></TD>
                          <TD><input id='adprice'  type="text" name="goodsInfoTempPo.adprice" value="${wholesalePriceEntry.cprwpeadprice }" class="text_input60" onkeyup="toFixAndNan(this)" onblur="toFix(this)"></TD>
                        ...
                        <TR>
                          <TD align="left" valign="bottom">
                             <li class="horizontal_onlyRight">
                              <input class="text_input100" name="wholesalePrice" id="wholesalePrice" type="text" maxlength="20">&nbsp;
                             </li>
                             <li class="horizontal_onlyRight">
                              <img src="${ctx }/img/newbtn/btn_change_0.png" btn=btn title='选择' onClick="setWholesalePrice();" >
                              </li>
                          </TD>
                        </TR>
                        <TR>
