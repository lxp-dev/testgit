...
//-----------向下复制数量-------------------------
	...
	function save(){
		if(checkForm(document.all.otherReceiptForm)){ 
			...
			otherReceiptForm.action = "insertProcurementReturnStorage.action";
	...
	//加载新建
	function onBlurBarCode1(barCodeInputObj,goodsId){
	...
	function addRow(goodInfo){
		...
		c6.innerHTML = '<a href=\'javascript:getCallAbleNum("'+goodInfo.bgigoodsid+'","'+goodInfo.bgigoodsbarcode.toUpperCase()+'");\'><span name="maxquantitySpan">'+ goodInfo.cshaaemaxquantity + '</span></a><input type="hidden" id="maxquantity" name="goodsInfoTempPo.cshaaemaxquantity" value="' + goodInfo.cshaaemaxquantity +'" />';
	...
	function addRow2(goodInfo){
		...
		c5.innerHTML = goodInfo.bgiunitname;
		c7.innerHTML = '<input type="text" class="text_input40" maxlength="18" name="goodsInfoTempPo.goodsquantity" onkeydown="OnKeyDownEnter(this)" onblur="checkNumber(this);amount();" value="' + goodInfo.bgigoodsquantity +'" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.UINT, \'Message\' : \'请填写正确的商品数量！\'}]"/>';
	...
	*/
	...
	function addDimensionRow(goodInfo){
		...
		if(addtype == "1"){
			loadBar(goodInfo.bgigoodsbarcode);
		}
		...
		if('${systemParameterPo.fspbarcodetype}'=='1'||'${systemParameterPo.fspbarcodetype}'=='2')
		{
			...
			c7.innerHTML = '<input id="selectGbc" name="goodsInfoTempPo.goodsbarcode"  multiple="multiple" class="text_input200 gbc" readonly="readonly" value ="'+goodInfo.pcbarcode+'">'+
						   '<input index="index'+index+'" onkeyup="this.value=this.value.toUpperCase()"   onblur="this.value=this.value.toUpperCase()" maxlength="26" type="hidden" class="text_input160" name="barCodeInput" onfocus="this.select();" onkeydown="onBlurBarCode($(this),\''+goodInfo.goodsid+'\');"/>';
			$('#del'+index).btn().init();
		}else{
			c6.innerHTML = '<input type="text" class="text_input60 number" maxlength="18" name="goodsInfoTempPo.goodsquantity" onkeydown="OnKeyDownEnter(this)" value="'+goodInfo.v+'" onblur="checkNumber(this);amount(this);" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.UINT, \'Message\' : \'请填写正确的商品数量！\'}]"/>'+
	...
	function openValues3(objValue){
		otherReceiptForm.action="selStockGoodsForBrandInsert.action?brandid="+objValue+"&outstockid="+$("#cstioutstockid").val();
		otherReceiptForm.submit();
	}
	...
	function openInvisible(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initOpenInvisible.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + supplierID,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initOpenInvisible.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + supplierID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function openValues21(objValue,isShowBarcode){
		otherReceiptForm.action="reQueryGoodsAllocationBills.action?billID="+objValue+"&isShowBarcode="+isShowBarcode;
		otherReceiptForm.submit();
	}
                  ...
                  border=0>
                      <TBODY>
                        <TR height="26">
                          ...
                          <TD width="24%" class="table_none"><input style="width: 170px;" class="text_input160" id="cstibillid"  name="inventoryPo.cstibillid" value="${inventoryPo.cstibillid}" readonly="readonly"></TD>
                          <TD width="9%" class="table_body">入库类型</TD>
                          <TD width="24%" class="table_none">采购退货单<input type="hidden" name="inventoryPo.cstibilltypeid" id="cstibilltypeid" value="2" /> 
						  </TD>
						...
						<TR height="26">
						  <TD class="table_body">制造商</TD>
						   ...
						   <li class="horizontal_onlyRight">
						  <img src="${ctx }/img/newbtn/btn_change_0.png" title="选 择" btn=btn name=ctl00$PageBody$Button1 onClick="openSupplier();"></li></TD>
                          <TD class="table_body">发出仓位</TD>
                          <TD class="table_none" >
                            <select id="cstioutstockid" name="inventoryPo.cstioutstockid" >
      		                 <s:iterator value="warehouseList">
				               <option value="${bwhid}" ${(bwhid == warehouseConfigurationPo.bwcstockid7 || bwhid == inventoryPo.cstioutstockid) ? 'selected="selected"' : '' }>${bwhwarehouseName}</option>
	     	                 </s:iterator>
      	                   </select>
                          </TD>
					...
					<TABLE id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                       ...
                       <c:if test="${systemParameterPo.fspbarcodetype==3}">
                        ...
                        <input type="text" class="text_input60 number" value="${cstiegoodsquantity}" name="goodsInfoTempPo.goodsquantity" onkeydown="OnKeyDownEnter(this)"  onblur="checkNumber(this);amount(this);" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请填写商品数量！'},{'Type' : Type.String, 'Formula' : Formula.INT, 'Message' : '请填写正确的商品数量！'}]"/>
                       </c:if>
                       <c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}">  
