...
<script>
	...
	*/
	function open2D(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initGoodsOpen_dimensional.action?tdgoodsids="+$('#tdgoodsids').val()+"&tdvs="+$('#tdvs').val()+"&cstpgoodscategory=" + categoryID_open,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initGoodsOpen_dimensional.action?tdgoodsids="+$('#tdgoodsids').val()+"&tdvs="+$('#tdvs').val()+"&cstpgoodscategory=" + categoryID_open,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		document.getElementById('popupTitle').innerHTML="【商品查询】";
	}
	...
	function addDimensionRow(goodInfo){
		...
		c5.innerHTML = '<input type="text" ondblclick="batchCheckQutity(this);" class="text_input60" onBlur="needAmount()" maxlength="18" onkeydown="OnKeyDownEnter(this)" name="needNumber" value="'+goodInfo.v+'" validate="[{\'Type\' : Type.String, \'Formula\' : Formula.notNull, \'Message\' : \'请填写商品数量！\'},{\'Type\' : Type.String, \'Formula\' : Formula.INT, \'Message\' : \'商品数量应为整数！\'}]"/>';
						...
						<tr>
                          <TD class="table_body">单据日期</TD>
			               ...
			               <TD class="table_none">
                            ...
                            <select id="goodscategoryID" name="allocationPo.goodscategoryid">
      		                 <option value="">----请选择----</option>
