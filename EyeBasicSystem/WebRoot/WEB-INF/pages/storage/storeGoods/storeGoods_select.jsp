...
<script>	
	...
	function update(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initStoreGoodsUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStoreGoodsUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【客户批发调货修改】";
	}
	function search(){
		$("img").removeAttr("onclick");
		salesOutForm.action = "selStoreGoods.action";
		salesOutForm.submit();
		showLoadingBar();
	}
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initStoreGoodsInsert.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStoreGoodsInsert.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【客户批发调货新增】"
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initStoreGoodsDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",500,150,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStoreGoodsDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",500,150,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【客户批发调货删除】"
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：客户批发调货管理</TD>
             <TD align="right" vAlign=bottom>&nbsp;
            <c:if test="${(permissionPo.keya==1)}">
			  <c:if test="${systemParameterPo.fspwholesalepriceset eq '0'}">
            	<img src="${ctx }/img/newbtn/btn_tracinsert_0.png" btn=btn title="客户批发调货新增" onClick="insert()">
              </c:if>
			  <c:if test="${systemParameterPo.fspwholesalepriceset eq '1'}">
            	<img src="${ctx }/img/newbtn/btn_tracinsert_0.png" btn=btn title="客户批发调货新增" onClick="javascript:alert('系统已停用批发价,若要使用请联系管理员启用批发价!')">
              </c:if>
           </c:if>
