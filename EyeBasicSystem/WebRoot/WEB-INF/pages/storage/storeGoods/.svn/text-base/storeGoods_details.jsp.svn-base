...
<script>
    $(document).ready(function(){
         ...
         $("#goodsquantityTotal").text(num);

         num = 0;
         $("input[name=goodsInfoTempPo.costpriceamount]").each(function (){
        	 num = accAdd(num,$(this).val());
         });
         $("#costpriceamountTotal").text(parseFloat(num).toFixed(2));
         ...
         <c:if test="${permissionPo.keyf == 1}">	
	         ...
	         $("#nottaxrateamountTotal").text(parseFloat(num).toFixed(2));
         </c:if>
	...
	function tracmould(billid){
		var DataURL = "report.action?reportlet=storage_tracmould.cpt&__bypagesize__=false" + "&billID=" + billid + "&isShowCost=" + '${permissionPo.keyh }';
		...
		document.getElementById('popupTitle').innerHTML="【商品调货模板（非批号） 】";
	}
	
	function tracmouldyx(billid){
		var DataURL = "report.action?reportlet=storage_tracmouldYX.cpt&__bypagesize__=false" + "&billID=" + billid + "&isShowCost=" + '${permissionPo.keyh }';
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;	
		...
		document.getElementById('popupTitle').innerHTML="【商品调货模板（批号） 】";
	}
</script>
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        ...
        <TR>
          <TD align="right" colspan="3">
            <img src="${ctx}/img/newbtn/btn_storegoodsexportyx_0.png" btn=btn onclick="tracmouldyx('${inventoryPo.cstibillid}');"/>
          	<img src="${ctx}/img/newbtn/btn_storegoodsexport_0.png" btn=btn onclick="tracmould('${inventoryPo.cstibillid}');"/>
             <s:action name="getFittingTemplateTypeInfo" executeResult="true">
				<s:param name="actionTypeID">34</s:param>
           		...
           		<s:param name="actionReportTitle">客户批发调货单打印</s:param>
             </s:action>
          </TD>
        </TR>
                  ...
                  <DIV>
                    ...
                    <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                       <TBODY>
                        ...
                        <TR>
                          ...
                          <TD class="table_none">
                           ${fn:substring(inventoryPo.cstiauditdate,0,16)}&nbsp;
                          </TD>
                        </TR>
                         <TR>
					...
					<table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                        <TR class=table_title align=middle>                     
                          ...
                          <TH scope=col width="8%">批发单价</TH>
						  <TH scope=col width="8%">批发金额</TH>
                         <c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}">
                          <TH scope=col width="8%">商品条码</TH></c:if>
                        </TR>
                          <tr class=table_title align=middle> 
                        ...
                        <s:iterator value="inventoryEntryList" status="idx">
	                      ...
	                      <c:if test="${permissionPo.keyf == 1}">	                      
	                        ...
	                        <TD>${cstiecostpriceamount}<input type="hidden" readOnly="readonly" onchange="remove();" style="background-color:#ACA899" style="width:100%" name="goodsInfoTempPo.costpriceamount" value="${cstiecostpriceamount}" />
	                        <input type="hidden" readOnly="readonly" onchange="remove();" style="background-color:#ACA899" style="width:100%" name="goodsInfoTempPo.taxamount" value="${cstietaxamount}" /></TD>
                       <c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}"> 
                        	<TD>${cstiebarcode }
					        </TD>  
					   </c:if>                                                                      
