...
<script>
	...
	if(checkForm(document.all.salesOutForm)){ 
		...
		salesOutForm.action = "insertOtherOut.action";
		salesOutForm.submit();
		}
	...
	**/
	...
	function addRow(goodInfo,stateArray){
		...
		// 添加商品到列表 end	
	    	...
	    	var goodscategoryArray = document.getElementsByName("goodsInfoTempPo.goodscategory");
  ...
  <TR>
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        ...
        <TR>
          <TD class=menubar_function_text height=27><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：其它出库新增</TD>
                    ...
                    <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
			               ...
			               <TD class="table_none">                            
                          	<select id="cstioutstockid" name="inventoryPo.cstioutstockid" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '发出仓位不能为空！'}]">
                             	<c:if test="${not empty(warehouseList)}">
                    	           ...
                    	           <OPTION value="${bwhid}" ${'yybqtclc'!= bwhid  ? '' : 'selected="selected"' } >${bwhwarehouseName}</OPTION>
                    	          </s:iterator>
                    	        </c:if>
      	                   </select>
			               </TD>
                        ...
                        <TR>
			               ...
			               <TD class="table_none" colspan="3">${person.personName }<input class="text_input100" type="hidden" name="inventoryPo.csticreateperson" value="${person.id }"></TD>
                        </TR>
                         <TR>
