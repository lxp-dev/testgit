...
<script>
	...
	function batPrintGoodsBarCode(){
		if(checkPrintNumber()) {
			if(confirm("条码打印确认！")){
				var barCodes = $("input[name=brandBarcode]");
				var goodsQuantitys = $("input[name=quantity]");
				var brandNames = $("input[name=brandName]");
	...
	function addRow(brandPo){
		...
		c5.innerHTML = brandPo.bbdid.replace(/\./g,"")+'<input type="hidden" name="brandBarcode" value="' + brandPo.bbdid.replace(/\./g,"") +'" />';
