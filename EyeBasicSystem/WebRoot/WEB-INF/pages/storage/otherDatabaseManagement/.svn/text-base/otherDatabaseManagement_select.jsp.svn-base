...
<script>	
	function doList(link,perPage_Select,perPage_Text_Hidden){
     	var objOne = document.all[perPage_Select];
	  	...
	  	otherDatabaseManagementForm.action=link;
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initOtherDatabaseManagementUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initOtherDatabaseManagementUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【其他出库修改】";
	}
	function search(){
		$("img").removeAttr("onclick");
		otherDatabaseManagementForm.action = "selOtherDatabaseManagement.action";
		otherDatabaseManagementForm.submit();
		showLoadingBar();
	}	
	function details(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("returnOtherDatabaseManagementDetails.action?hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("returnOtherDatabaseManagementDetails.action?hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initOtherDatabaseManagementInsert.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initOtherDatabaseManagementInsert.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【其他出库新增】";
	}
	...
	function insertOther(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initOtherReceiptInsert.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initOtherReceiptInsert.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function del(id){
		...
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initOtherDatabaseManagementDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,140, topRows,topCols,returnRefresh(true),false); 
		}else{ 
		showPopWin("initOtherDatabaseManagementDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,140, topRows,topCols,returnRefresh(true),false); 
		} 
		document.getElementById('popupTitle').innerHTML="【其他出库删除】";
	}
	...
	$(document).ready(function (){
		var input = document.getElementById("billID");
		input.focus();
	});
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：其他出库管理  </TD>
            <td align="right" width="40%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		<img src="${ctx }/img/newbtn/btn_qtckadd_0.png" btn=btn title="其他出库新增" onClick="insert()">
            	</c:if>
...
<img src="${ctx }/img/newbtn/btn_today_0.png"  btn=btn title="今天" onClick="today()"></li>
                        ...
                        <TR>
			               ...
			               <TD class="table_none">
                            <input class="text_input160" type="hidden"  id="auditPersonName" name="auditPersonName" value="${requestScope.auditPersonName}">
                            <input type="text"  id="auditPersonID" name="auditPersonID" value="${requestScope.auditPersonID}">
			               </TD>
...
<img src="${ctx }/img/newbtn/btn_today_0.png"  btn=btn title="今天" onClick="today1()"></li>
                    	...
                    	<TR>
			               ...
			               <TD class="table_none" colspan="3">
                            <input class="text_input160" type="hidden"  id="createPersonName" name="createPersonName" value="${requestScope.createPersonName}">
                            <input  type="text"  id="createPersonID" name="createPersonID" value="${requestScope.createPersonID}">
			               </TD>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->	
					<c:if test="${not empty(otherDatabaseManagementList)}"> 
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                         <TH width="9%" height="26" scope=col colspan="3">操作</TH>
						  ...
						  <TH width="13%" scope=col>发出仓位</TH>
						  <TH width="10%" scope=col>领用部门</TH>
						  <TH width="12%" scope=col>单据日期</TH>
						  <TH width="12%" scope=col>出库日期</TH>
						  <TH width="7%" scope=col>审核状态</TH>
						  <TH width="7%" scope=col>制单人</TH>
						  <TH width="7%" scope=col>审核人</TH>
						 
						  </TR>
						<s:iterator value="otherDatabaseManagementList">
