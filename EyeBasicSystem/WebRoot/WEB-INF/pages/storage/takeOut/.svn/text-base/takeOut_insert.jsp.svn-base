...
<script>
	...
	function save(){
		var d1=$("#cstidepartmentid1").val();
		var d2=$("#cstidepartmentid2").val();
		...
		salesOutForm.action = "insertTakeOut.action";
		salesOutForm.submit();
	}
	...
	*/
	...
	function openValues3(objValue){
		document.getElementById('cstisourcebillid').value=objValue;
		salesOutForm.action="selReAllocationBillsOut.action?billID="+objValue;
		salesOutForm.submit();
	}
	function openGoodAllocation(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initGoodsAllocationOut.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + supplierID,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initGoodsAllocationOut.action?categoryID_open=" + cstpgoodscategory + "&supplierID_open=" + supplierID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		document.getElementById('popupTitle').innerHTML="【负调拨单查询】";
	} 
                      ...
                      <TBODY>
                        <TR>
                          ...
                          <input name="inventoryPo.cstibilldate" type="hidden" value="<fmt:formatDate value="${now}" type="both" dateStyle="long" pattern="yyyy-MM-dd" />"/></TD>
			               ...
			               <TD class="table_none">
			               	<input type="radio" name="inventoryPo.cstitaketype" onclick="loadtake(this)" checked value="1"/>部门领用
			               	<c:if test="${person.personcompanytype == '1'}">
			               	<input type="radio" name="inventoryPo.cstitaketype" onclick="loadtake(this)" value="2"/>客户领用
			               	</c:if>
			               </TD>
                        </TR>                        
                        <TR>
                           <TD height="26" class="table_body"><div id="d">领用部门</div><div id="j" style="display: none;">领用客户</div></TD>
			               <TD class="table_none">
                          	<select id="cstidepartmentid1" name="inventoryPo.cstidepartmentid">
      	                   ...
      	                   <select id="cstidepartmentid2" name="inventoryPo.cstidepartmentid">
      		                 	<option value="">----请选择----</option>
					...
					<table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                        <TR class=table_title align=middle>                     
 						...
 						<c:if test="${permissionPo.keyf == 1}">
						  ...
						  <TH scope=col width="6%">领用单价</TH>
						  <TH scope=col width="6%">领用金额</TH> 			  
                        <c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}"> 
					    ...
					    <c:if test="${permissionPo.keyf == 1}">
					        ...
					        <th scope=col>&nbsp;</th>
					    	<th scope=col id="takepriceamountTotal">&nbsp;</th>
					    	<c:if test="${systemParameterPo.fspbarcodetype==1||systemParameterPo.fspbarcodetype==2}"> 
					    	<th scope=col>&nbsp;</th>
					    	</c:if>
				   		</tr>
