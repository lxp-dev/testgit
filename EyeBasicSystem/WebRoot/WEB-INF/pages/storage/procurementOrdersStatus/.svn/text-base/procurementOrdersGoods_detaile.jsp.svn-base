              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        ...
                        <TR>
                          ...
                          <c:if test="${procurementOrdersPo.cstpauditstate==6 }">
					...
					<c:if test="${not empty(orderEntryList)}">
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                              <TBODY>
                                <TR>
                                  <TD width="5%"><div align="left"><img src="${ctx }/img/pic/queryCondition.gif" width="86" height="20" ></div></TD>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR class=table_title align=middle>
						  ...
						  <TH width="10%" scope=col>下加</TH>
						  <TH width="10%" scope=col>棱镜</TH>
						  <TH width="10%" scope=col>基底</TH>
						  <TH width="10%" scope=col>订单数量</TH>
                        </TR>
                        <c:forEach var="po" items="${orderEntryList}">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <TD height="28">${po.cstpegoodsname }</TD>
                          <TD>${po.cstpespec }</TD>
                          <TD>${po.cstpecolor }</TD>
                          <TD>${po.cstpeballglass }</TD>
						  <td>${po.cstpepostglass }</td>
                          <TD>${po.cstpeaxis }</TD>
                          <TD>${po.cstpeadd }</TD>
                          <TD>${po.cstpearriseglass }</TD>
                          <TD>${po.cstpebasis }</TD>
                          <TD>${po.cstpeordernumber }</TD>
                        </TR>
                        </c:forEach>
                      </TBODY>
                    </TABLE>
