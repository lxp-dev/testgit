...
<script>	
	...
	function search(){
		brandForm.action = "selectProcurementOrdersStatus.action";
		brandForm.submit();
		showLoadingBar();
	}	
	
	function detaile(id){
		showPopWin("","selectProcurementSend.action?hid="+id, screen.width-200,screen.height-200, '',null,true);
		selectHidden();
	}
    ...
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          ...
          <TR>
            <TD class=menubar_function_text height=27><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：采购订单查询</TD>
          </TR>
          <TR>
				...
				<TD>
                  <TABLE height=22 cellSpacing=0 cellPadding=0 border=0>
                    ...
                    <TR>
                      ...
                      onclick="JavaScript:window.location.href='initProcurementOrdersSel.action?moduleID=${requestScope.moduleID}';"
                      UNSELECTABLE="on">采购订单查询</TD>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TR>
						  ...
						  <select id="cstpauditstate" name="auditstate">
                                ...
                                <option value='6'>制造商备货</option>
                                <option value='7'>制造商发货</option>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->
					<c:if test="${not empty(procurementOrdersStatusList)}">
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                              <TBODY>
                                <TR>
                                  <TD width="5%"><div align="left"><img src="${ctx }/img/pic/queryCondition.gif" width="86" height="20" ></div></TD>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH width="15%" scope=col>订单日期</TH>
                          <TH width="10%" scope=col>订单状态</TH>
						  <th width="10%" scope=col>处理人</th>
                          <TH width="15%" scope=col>处理日期</TH>
						  <TH width="10%" scope=col>详细</TH>
                        </TR>
                        <c:forEach var="po" items="${procurementOrdersStatusList}">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <TD height="28">${fn:substring(po.cstpid,5,31)}</TD>
                          <TD>${po.cstpsuppliername }</TD>
						  <TD>${fn:substring(po.cstpbilldate,0,10)}</TD>
                          
                          <c:if test="${po.cstpauditstate==1 }">
                          <TD>下单</TD>
                          </c:if>
                          <c:if test="${po.cstpauditstate==2 }">
                          <TD>确认</TD>
                          </c:if>
                           <c:if test="${po.cstpauditstate==6 }">
                          <TD>制造商备货</TD>
                          </c:if>
                          <c:if test="${po.cstpauditstate==7 }">
                          <TD>制造商发货</TD>
                          </c:if>
                          <c:if test="${po.cstpauditstate==8 }">
                          <TD>确认收货</TD>
                          </c:if>
                          
                          <td>${po.cstpauditpersonname }</td>
                          <TD>${fn:substring(po.cstpauditdate,0,10)}</TD>
                          <TD>
		                     <input type='button' value='详细' onClick="javascript:detaile('${po.cstpid}')">
		                  </TD>
                        </TR>
                        </c:forEach>
                      </TBODY>
