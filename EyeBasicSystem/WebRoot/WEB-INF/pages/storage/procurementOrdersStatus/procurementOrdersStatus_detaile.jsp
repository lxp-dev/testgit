...
<script>	
	...
	function Ordersdetaile(id){
		showPopWin("","selectProcurementOrdersDetaile.action?hid="+id, screen.width-200,screen.height-200, '',null,true);
		selectHidden();
	}
	
	function sendDetaile(id){
		showPopWin("","selectProcurementSendDetaile.action?hid="+id, screen.width-200,screen.height-200, '',null,true);
		selectHidden();
	}
  ...
  <TR>
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            <TD class=menubar_title><img border='0' src='${ctx }/img/pic/module.gif' align='absmiddle' hspace='3' vspace='3'>采购订单管理</TD>
          ...
          <TR>
            <TD class=menubar_function_text height=27><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：采购订单详细</TD>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
				  ...
				  <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR>
                          ...
                          <TD class="table_none" width="30%">
                          	...
                          	<input type='button' value='采购订单详细' onClick="javascript:Ordersdetaile('${procurementOrdersPo.cstpid}')">
                          </TD>
                        ...
                        <TR>
                          ...
                          <TD class="table_none">
                          	<c:choose>
                    			<c:when test="${procurementOrdersPo.cstpauditstate==1 }">下单</c:when>
                    			<c:when test="${procurementOrdersPo.cstpauditstate==2 }">确认</c:when>
                    			<c:when test="${procurementOrdersPo.cstpauditstate==6 }">制造商备货</c:when>
                    			<c:when test="${procurementOrdersPo.cstpauditstate==7 }">制造商发货</c:when>
                    			<c:when test="${procurementOrdersPo.cstpauditstate==8 }">确认收货</c:when>
                          	</c:choose>
                          </TD>
					...
					<c:if test="${not empty(procurementSendList)}">
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                              <TBODY>
                                <TR>
                                  <TD width="5%"><div align="left"><img src="${ctx }/img/pic/queryCondition.gif" width="86" height="20" ></div></TD>
					...
					<table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        ...
                        <c:forEach var="po" items="${procurementSendList}">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          ...
                          <td>
                          	<c:choose>
                    			<c:when test="${po.cstdauditstate==0 }">制造商备货</c:when>
                    			<c:when test="${po.cstdauditstate==1 }">制造商备货</c:when>
                    			<c:when test="${po.cstdauditstate==2 }">制造商发货</c:when>
                          	</c:choose>
                          </td>
                          <TD>
		                     <input type='button' value='采购发货单详细' onClick="javascript:sendDetaile('${po.cstdid}')">
		                  </TD>
                        </TR>
                        </c:forEach>
                      </TBODY>
