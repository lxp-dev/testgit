...
<script>	
	...
	function update(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initNonconformingUpdateTR.action?hid="+id+"&moduleID="+moduleID,970,screen.height-200,topRows,topCols,returnRefres(true),true);
		}else{
			showPopWin("initNonconformingUpdateTR.action?hid="+id+"&moduleID="+moduleID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【不合格品单更新】";
	}
	function search(){
		document.all.submitButton.disabled="true";
		nonconformingForm.action = "selNonconformingTR.action";
	...
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initNonconformingInsertTR.action?moduleID="+moduleID,970,screen.height-200,topRows,topCols,returnRefres(true),true);
		}else{
			showPopWin("initNonconformingInsertTR.action?moduleID="+moduleID,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
	...
	function openBrand(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("selBrandOpen.action?categoryID_open=1&supplierID_open=" + document.getElementById('supplierID').value,970,screen.height-200,topRows,topCols,returnRefres(true),false);
		}else{
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：不合格品管理TR</TD>
