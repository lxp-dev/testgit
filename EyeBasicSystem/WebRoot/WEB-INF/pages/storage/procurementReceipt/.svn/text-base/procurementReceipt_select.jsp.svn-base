...
<script>
		...
		if(type=='4'||type=='5')
		{
			if(${systemParameterPo.fspbarcodetype}=='1'||${systemParameterPo.fspbarcodetype}=='2')
			{
				if(${systemParameterPo.fspstealtheffective}=='1'||${systemParameterPo.fspstealtheffective}=='2')
				{
					...
					if(is_iPad()){
						showPopWin("initProcurementReceiptUpdateyx.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
					}else{
						showPopWin("initProcurementReceiptUpdateyx.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
					}
					document.getElementById('popupTitle').innerHTML="【采购收货更新】";
				}else
				{
					var topRows = top.document.getElementById("total").rows;
					var topCols = top.document.getElementById("btmframe").cols;
					if(is_iPad()){
						showPopWin("initProcurementReceiptUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
					}else{
						showPopWin("initProcurementReceiptUpdate.action?hid="+id+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
					}
					document.getElementById('popupTitle').innerHTML="【采购收货更新】";
				}
			}else
	...
	function search(){
		...
		procurementReceiptForm.action = "selProcurementReceipt.action";
		procurementReceiptForm.submit();
		showLoadingBar();
	}	
	function details(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("procurementReceiptDetails.action?hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("procurementReceiptDetails.action?hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function del(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initProcurementReceiptDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",500,150,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initProcurementReceiptDelete.action?hid="+id+"&moduleID=${requestScope.moduleID}",500,150,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【采购收货删除】"
	}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：采购收货查询</TD>
                  ...
                  <TABLE height=22 cellSpacing=0 cellPadding=0 border=0>
                    ...
                    <TR>
                      ...
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                       onclick="JavaScript:window.location.href='initProcurementWaitSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
					  	<TR>
			               ...
			               <TD width="9%" height="26" class="table_body">关联订单号</TD>
			               <TD class="table_none" width="22%">
...
<img src="${ctx }/img/newbtn/btn_today_0.png" btn=btn title="今天" onClick="today()">
                        ...
                        <TR>
						  ...
						  <select name="categoryid" id="categoryid">
							...
							<option value="${bgcid}" ${categoryid == bgcid ? 'selected="selected"' : '' } >${bgcgoodscategoryname}</option>
     	               		</s:iterator>
						  </select>
						  </TD> 
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TH width="9%" height="26" scope=col colspan="3">操作</TH>
                          ...
                          <TH width="9%" scope=col>采购类型</TH>
                          <TH width="10%" scope=col>运单单号</TH>
						  <TH width="13%" scope=col>关联订单号</TH>		
