...
<script>
	function disableWhole(id){
		...
		if(is_iPad()){
		showPopWin("initDisableAllWhole.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initDisableAllWhole.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
	...
	function ableWhole(id){
		...
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initAbleAllWhole.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initAbleAllWhole.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initLensFinishedUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensFinishedUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【成品镜片修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initLensFinishedDetails.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initLensFinishedDetails.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【成品镜片详细】";
	}
	
	function search(){
		if(checkForm(lensFinishedForm)){  
			var minSphjp = $("#minSphjp");
			var maxSphjp = $("#maxSphjp");
			var minCyljp = $("#minCyljp");
			var maxCyljp = $("#maxCyljp");
			
			if(parseFloat(minSphjp.val()) > parseFloat(maxSphjp.val())){
				alert("球镜区间填写有误！");
				minSphjp.select();
				return;
			}
			
			if(parseFloat(minCyljp.val()) > parseFloat(maxCyljp.val())){
				alert("柱镜区间填写有误！");
				minCyljp.select();
				return;
			}
			...
			lensFinishedForm.action = "selLensFinished.action";
			lensFinishedForm.submit();		
			showLoadingBar();
		}
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initLensFinishedInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensFinishedInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【成品镜片新增】";
	}
	...
	function insert2d(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initLensFinished2DInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensFinished2DInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【成品镜片按二维表新增】";
	}
	...
	function insertBulk(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initLensFinishedInsertBulk.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensFinishedInsertBulk.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【成品镜片批量新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;	
		if(is_iPad()){ 
		showPopWin("initLensFinishedDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensFinishedDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【成品镜片删除】";
	}
	function disableGlassesAccessories(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initLensFinishedDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensFinishedDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【成品镜片停用】";
	}
	function ableGlassesAccessories(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initLensFinishedAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensFinishedAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【成品镜片启用】";
	}
	function clean(){
		...
		document.getElementById('bgieyeglassmaterialtypejp').value = "";
		document.getElementById('bgirefractivejp').value = "";
		document.getElementById('bgiismutiluminosityjp').value = "";
		document.getElementById('minSphjp').value = "";
		document.getElementById('maxSphjp').value = "";
		document.getElementById('minCyljp').value = "";
		document.getElementById('maxCyljp').value = "";
	...
	*/
	function selectDW(){
		...
		if(is_iPad()){
			showPopWin("selgalessDWOpen.action?categoryID=30&moduleID=${requestScope.moduleID}&brandID=",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selgalessDWOpen.action?categoryID=30&moduleID=${requestScope.moduleID}&brandID=",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="20%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：成品镜片维护</TD>
             <td align="right" width="100%" valign="bottom">&nbsp;
            	...
            	<img src="${ctx }/img/newbtn/btn_cpjpladd_0.png" btn=btn title="成品镜片新增" onClick="insert()">
            	<img src="${ctx }/img/newbtn/btn_cpjpl2dadd_0.png" btn=btn title="成品镜片按二维表新增" onClick="insert2d()">
            	<img src="${ctx }/img/newbtn/btn_cpjplpladd_0.png" btn=btn title="成品镜片批量新增" onClick="insertBulk()">
            	</c:if>
				...
				<TD>
                  <TABLE height=22 cellSpacing=0 cellPadding=0 border=0>
                    ...
                    <TR>
                      ...
                      onclick="JavaScript:window.location.href='initGlassesFrameSel.action?moduleID=${requestScope.moduleID}';" 
                    ...
                    <TD width=3><IMG id=tabImgLeft__1 height=22 
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initGlassesAccessoriesSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    <TD width=3><IMG id=tabImgLeft__1 height=22 
                      ...
                      onclick="JavaScript:window.location.href='initLensFinishedSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initLensCustomSel.action?moduleID=${requestScope.moduleID}';"
                      UNSELECTABLE="on">订做镜片</TD>
                      <TD width=3><IMG id=tabImgRight__1 height=22 
                        src="${ctx }/img/pic/tab_unactive_right.gif" 
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initStealthFinishedSel.action?moduleID=${requestScope.moduleID}';"
                      UNSELECTABLE="on">隐形成品片</TD>
                      <TD width=3><IMG id=tabImgRight__1 height=22 
                        src="${ctx }/img/pic/tab_unactive_right.gif" 
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initStealthCustomLensesSel.action?moduleID=${requestScope.moduleID}';"
                      UNSELECTABLE="on">隐形订做片</TD>
                      <TD width=3><IMG id=tabImgRight__1 height=22 
                        src="${ctx }/img/pic/tab_unactive_right.gif" 
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initStealthAccessoriesSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initGlassesFinishSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initPresbyopicGlassesSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    width=3></TD>
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initOtherGoodsSel.action?moduleID=${requestScope.moduleID}';"
                    ...
                    <TD width=3><IMG id=tabImgLeft__1 height=22 
                      ...
                      <TD class=tab id=tabLabel__1 
                      background=${ctx }/img/pic/tab_unactive_bg.gif 
                      onclick="JavaScript:window.location.href='initVisualOpticsSel.action?moduleID=${requestScope.moduleID}';"
                      UNSELECTABLE="on">视光</TD>
                      <TD width=3><IMG id=tabImgRight__1 height=22 
                        src="${ctx }/img/pic/tab_unactive_right.gif" 
                    width=3></TD>
...
name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
                        ...
                        <tr>
                        	<TD height="26" class="table_body">镜片材料分类</TD>
			                <TD class="table_none">
						    ...
						    <TD height="26" class="table_body">镜片折射率</TD>
			                <TD class="table_none">
			                ...
			                <TD height="26" class="table_body">镜片光度分类</TD>
			                <TD class="table_none">
                        ...
                        <tr>
			               ...
			               <TD class="table_none">
                            <input class="text_input80" type="text" onchange="checkNumberType(this);"  id="minSphjp" name="minSphjp" value="${requestScope.minSphjp}" maxlength="6" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新填写镜片球镜度数！'}]">
                            	至
                            <input class="text_input80" type="text" onchange="checkNumberType(this);" id="maxSphjp" name="maxSphjp" value="${requestScope.maxSphjp}" maxlength="6" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新填写镜片球镜度数！'}]">
			               </TD>
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TH width="3%" scope=col><input type="checkbox" id="chks" name="chks" onclick="chkAll()"></TH>
                          ...
                          <TH width="16%" height="26" scope=col>商品代码</TH>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <TD>
                        	...
                        	<TD width="3%">
			                  ...
			                  <c:if test="${bgiflag==1}">
			                    <img src="${ctx }/img/newbtn/enabled_0.png" title='销售停用' btn=btn onClick="javascript:disableGlassesAccessories('${bgigoodsid}')">	
			                  </c:if>
			                  <c:if test="${bgiflag==0}">
			                  <img src="${ctx }/img/newbtn/unenabled_0.png" title='销售启用' btn=btn onClick="javascript:ableGlassesAccessories('${bgigoodsid}')">	
			                  </c:if>
			                    </c:if>		                     
			                  </TD>
			                  <TD width="3%">
