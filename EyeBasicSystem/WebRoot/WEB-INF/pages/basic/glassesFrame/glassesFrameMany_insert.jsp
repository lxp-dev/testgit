...
　　 }
	...
	function save(){
		...
		if(checkForm(document.all.glassesFrameForm)){  
			...
			if(checkcount==0){
				$("td[returnvalue='1']").each(function(){
					checkcount = accAdd(checkcount,1);
				});
			}
			...
			glassesFrameForm.action = "insertGlassesManyFrame.action";
	...
	function importFile(){
		...
		glassesFrameForm.action = "importGlassesFrameManyExcel.action";
		glassesFrameForm.submit();
		showLoadingBar();
	}
                ...
                vAlign=top><DIV id=tabContent__1>
                  <DIV>
                    ...
                    <table width="99%" border=0 align=center style="PADDING-TOP: 10px;" cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
			               <TD height="26" class="table_body" width="9%">采购年份</TD>
			               <TD class="table_none" colspan="5">
					...
					<table id="addTable" width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                        ...
                        <TR id=cktr class="row" ${bgiisinserted eq '1' ? 'style="background-color: red;"':'' } ${bgiisinserted eq '1' ? 'cktype="1"':'' }>
                          ...
                          <TD ${bgiframematerialtypeck eq '1' ? 'style="background-color: red;"':'' } checkname="bgiframematerialtype" checkvalue="${bgiframematerialtype}" returnvalue="${bgiframematerialtypeck}">
                          	${bgiframematerialtype}
                          	<input type="hidden" name="goodsInfoManyPo.bgiframematerialtypes" value="${bgiframematerialtype }"/>
                          </TD>
                          <TD ${bgichinesecolorck eq '1' ? 'style="background-color: red;"':'' } checkname="bgichinesecolor" checkvalue="${bgichinesecolor}" returnvalue="${bgichinesecolorck}">
                          	${bgichinesecolor}
                          	<input type="hidden" name="goodsInfoManyPo.bgichinesecolors" value="${bgichinesecolor }"/>
                          </TD>
                          <TD ${bgiframestyleck eq '1' ? 'style="background-color: red;"':'' } checkname="bgiframestyle" checkvalue="${bgiframestyle}" returnvalue="${bgiframestyleck}">
                          	${bgiframestyle}
                          ...
                          <TD ${bgiframeprocesscrafttypeck eq '1' ? 'style="background-color: red;"':'' } checkname="bgiframeprocesscrafttype" checkvalue="${bgiframeprocesscrafttype}" returnvalue="${bgiframeprocesscrafttypeck}">
                          	${bgiframeprocesscrafttype}
                          	<input type="hidden" name="goodsInfoManyPo.bgiframeprocesscrafttypes" value="${bgiframeprocesscrafttype }"/>
                          </TD>
                          </TD>
                      ...
                      <TBODY>
						<tr>
						  	<td>
								...
								&nbsp; &nbsp; &nbsp; &nbsp; <font color="#FF0000">材质、颜色、款式:所录内容必须在系统中提前录入</font><br>
...
<script language="javascript">
	$("td[checkname='bgichinesecolor']").each(function(){
		<c:forEach var="colorPo" items="${colorList}"> 
			if("${colorPo.bccolorname}" ==$(this).attr("checkvalue")){
				$(this).css('background-color','#cadee8');
				$(this).attr("returnvalue",'0');
			} 
		</c:forEach>
	});

	$("td[checkname='bgiframematerialtype']").each(function(){
		<c:forEach var="frameMaterialPo" items="${frameMateriallist}"> 
			if("${frameMaterialPo.bfmname}" ==$(this).attr("checkvalue")){
				$(this).css('background-color','#cadee8');
				$(this).attr("returnvalue",'0');
			} 
		</c:forEach>
	...
	$("td[checkname='bgiframeprocesscrafttype']").each(function(){
		<c:forEach var="technologyTypePo" items="${teachnologyTypeList}"> 
			if("${technologyTypePo.fttname}" ==$(this).attr("checkvalue")){
				$(this).css('background-color','#cadee8');
				$(this).attr("returnvalue",'0');
			} 
		</c:forEach>
	...
	$("td[checkname='bgiframestyle']").each(function(){
		<c:forEach var="optionParamPo" items="${frameStyleList1}"> 
			if("${optionParamPo.fopparamname}" ==$(this).attr("checkvalue")){
				$(this).css('background-color','#cadee8');
				$(this).attr("returnvalue",'0');
			} 
		</c:forEach>
