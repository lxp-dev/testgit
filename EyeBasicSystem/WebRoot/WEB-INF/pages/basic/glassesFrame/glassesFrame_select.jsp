...
<script>
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initGlassesFrameUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initGlassesFrameUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【镜架修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initGlassesFrameDetails.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initGlassesFrameDetails.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【镜架详细】";
	}
	
	
	function search(){
	$("img").removeAttr("onclick");
		glassesFrameForm.action = "selGlassesFrame.action";
		glassesFrameForm.submit();		
		showLoadingBar();
	}
	...
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initGlassesFrameInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initGlassesFrameInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【镜架新增】";
	}
	...
	function inserts(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initGlassesFrameManyInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initGlassesFrameManyInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【镜架类批量新增】";
	}
	...
	function del(id){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initGlassesFrameDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initGlassesFrameDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【镜架删除】";
	}
	function disableGlassesFrame(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initGlassesFrameDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initGlassesFrameDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【镜架停用】";
	}
	function ableGlassesFrame(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initGlassesFrameAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initGlassesFrameAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【镜架启用】";
	}
	function clean(){
		...
		document.getElementById('bgitechnologytypeid').value = "";
		document.getElementById('bgiframematerialtype').value = "";		
    ...
    function exportGoods(){
		alert("导出商品信息到Excel的过程可能稍慢，期间请不要做任何操作！");
    	glassesFrameForm.action = "exportFile.action";
    	glassesFrameForm.submit();
    }
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="30%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：镜架维护</TD>
            <td align="right" width="55%" valign="bottom">&nbsp;
            	...
            	<c:if test="${(permissionPo.keyo==1)}">
					<img btn=btn src="${ctx }/img/newbtn/btn_dcsp_0.png" border="0" title='导出所有商品资料' onclick="exportGoods();" > 
				</c:if>
            	<c:if test="${(permissionPo.keya==1)}">
            		<img src="${ctx }/img/newbtn/btn_kjladd_0.png" btn=btn title="镜架类新增" onClick="insert()">
            		<img src="${ctx }/img/newbtn/btn_kjladds_0.png" btn=btn title="镜架类批量新增" onClick="inserts()">
            	</c:if>
              ...
              <TR>
                ...
                vAlign=top><DIV id=tabContent__1>
                  <DIV>
                      ...
                      <TBODY>
                        ...
                        <TR>
						  ...
						  <img src="${ctx }/img/newbtn/btn_change_0.png" btn=btn title='选择' onClick="openBrand();" >
			               ...
			               <div id="div_retail"><input class="text_input100" type="text"  id="bgiretailbeginprice" name="bgiretailbeginprice" value="${requestScope.bgiretailbeginprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"> ~ <input class="text_input100" type="text"  id="bgiretailendprice" name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
			               </TD>
                        ...
                        <tr>
                        	<TD height="26" class="table_body">工艺类型</TD>
			               <TD class="table_none" width="24%">
                        ...
                        <TR> 
				            ...
				            <TD class="table_none" colspan="5">
								   ...
								   <li class="horizontal_onlyRight">
								     <select id="bgipayfeeid" name="bgipayfeeid" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '请选择收费项目！'}]">
	                                   <option value="">---请选择---</option>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->	
					<c:if test="${not empty(glassesFrameList)}"> 
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
						...
						<s:iterator value="glassesFrameList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
		                  ...
		                  <c:if test="${bgiflag==1}">
		                    <img src="${ctx }/img/newbtn/enabled_0.png" title='销售停用' btn=btn onClick="javascript:disableGlassesFrame('${bgigoodsid}')">	
		                  </c:if>
		                  <c:if test="${bgiflag==0}">
		                  <img src="${ctx }/img/newbtn/unenabled_0.png" title='销售启用' btn=btn onClick="javascript:ableGlassesFrame('${bgigoodsid}')">	
		                  </c:if>
		                    </c:if>		                     
		                  </TD>
		                  <TD width="3%">
