...
<script>	
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initLensCustomUpdateAR.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensCustomUpdateAR.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【车房镜片修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initLensCustomDetailsAR.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initLensCustomDetailsAR.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【车房镜片详细】";
	}
	function search(){
		if(checkForm(lensCustomForm)){  
			$("img").removeAttr("onclick");
			lensCustomForm.action = "selLensCustomAR.action";
			lensCustomForm.submit();		
			showLoadingBar();
		}
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initLensCustomInsertAR.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensCustomInsertAR.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【车房镜片新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initLensCustomDeleteAR.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensCustomDeleteAR.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【车房镜片删除】";
	}
	function disableLensCustom(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initLensCustomDisableAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensCustomDisableAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【车房镜片停用】";
	}
	function ableLensCustom(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initLensCustomAbleAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensCustomAbleAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【车房镜片启用】";
	}
	function clean(){
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：车房镜片维护</TD>
             <td align="right" width="40%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		...
            		<img src="${ctx }/img/newbtn/btn_dzjpladd_0.png" btn=btn title="车房镜片新增" onClick="insert()">
