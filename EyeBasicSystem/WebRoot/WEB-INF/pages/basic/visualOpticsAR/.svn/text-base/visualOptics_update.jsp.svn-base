...
<script>
	...
	function save(){
		if(checkForm(document.all.visualOpticsForm)){
			...
			visualOpticsForm.action = "updateVisualOpticsAR.action";
                  ...
                  <DIV>
                    ...
                    <table width="99%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
    	                ...
    	                <c:if test="${systemParameterPo.fspisusegoodslevel ne '1'}">
                        <tr>
                        	<TD class="table_body">其他视光产品类型</TD>
			               <TD class="table_none">
			               		   <c:forEach var="optionParamPoTmp" items="${optionParamPolist}" >
		                              <c:if test="${optionParamPoTmp.fopparentid == 'SGType'}">
		                               ${(goodsInfoPo.bgifinishedglassestype == optionParamPoTmp.fopparamid)? optionParamPoTmp.fopparamname :'' }
		                              </c:if>	                                      	
	                               </c:forEach> 
			               </TD>
