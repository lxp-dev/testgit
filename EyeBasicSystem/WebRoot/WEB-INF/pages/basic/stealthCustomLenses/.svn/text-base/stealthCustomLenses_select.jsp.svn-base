...
<script>	
	...
	function doList(link,perPage_Select,perPage_Text_Hidden){
     	var objOne = document.all[perPage_Select];
	  	...
	  	stealthCustomLensesForm.action=link;
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initStealthCustomLensesUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthCustomLensesUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形订做片修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initStealthCustomLensesDetails.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initStealthCustomLensesDetails.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形订做片详细】";
	}
	function search(){
		if(checkForm(stealthCustomLensesForm)){  
			$("img").removeAttr("onclick");
			stealthCustomLensesForm.action = "selStealthCustomLenses.action";
			stealthCustomLensesForm.submit();		
			showLoadingBar();
		}
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initStealthCustomLensesInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthCustomLensesInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形订做片新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initStealthCustomLensesDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthCustomLensesDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【隐形订做片删除】";
	}
	function disableStealthCustomLenses(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initStealthCustomLensesDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthCustomLensesDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【隐形订做片停用】";
	}
	function ableStealthCustomLenses(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initStealthCustomLensesAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthCustomLensesAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【隐形订做片启用】";
	}
	function clean(){
	...
	*/
	function selectDW(){
		...
		if(is_iPad()){
			showPopWin("selgalessDWOpen.action?categoryID=4D&moduleID=${requestScope.moduleID}&brandID=",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selgalessDWOpen.action?categoryID=4D&moduleID=${requestScope.moduleID}&brandID=",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left"  width="40%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：隐形订做片维护</TD>
            <td align="right" width="40%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		...
            		<img src="${ctx }/img/newbtn/btn_yxdzpladd_0.png" btn=btn title="隐形订做片新增" onClick="insert()">
            	</c:if>
                    ...
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
					  	<TR>
			               ...
			               <TD width="8%" class="table_body">商品名称</TD>
			               <TD width="25%" class="table_none">
...
name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->	
					<c:if test="${not empty(stealthCustomLensesList)}"> 
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TH width="15%" height="26" scope=col colspan="5">操作</TH>
                          ...
                          <TH width="4%" scope=col>曲率1</TH>
                          <TH width="4%" scope=col>曲率2</TH>
                          <TH width="4%" scope=col>轴位</TH>
						...
						<s:iterator value="stealthCustomLensesList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                         ...
                         <TD width="3%">
			                  ...
			                  <c:if test="${bgiflag==1}">
			                    <img src="${ctx }/img/newbtn/enabled_0.png" title='销售停用' btn=btn onClick="javascript:disableStealthCustomLenses('${bgigoodsid}')">	
			                  </c:if>
			                  <c:if test="${bgiflag==0}">
			                  <img src="${ctx }/img/newbtn/unenabled_0.png" title='销售启用' btn=btn onClick="javascript:ableStealthCustomLenses('${bgigoodsid}')">	
			                  </c:if>
			                    </c:if>		                     
			                  </TD>
			              <TD width="3%">
                          ...
                          <TD>${bgicurvature1ul}/${bgicurvature1up}</TD>
                          <TD>${bgicurvature2ul}/${bgicurvature2up}</TD>
                          <TD>${bgiaxisul}/${bgiaxisup}</TD>
                          <TD>${bgidiaul}/${bgidiaup}</TD>
