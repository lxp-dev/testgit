...
<script>	
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initLensCustomUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensCustomUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【订做镜片修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initLensCustomDetails.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initLensCustomDetails.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【订做镜片详细】";
	}
	function search(){
		if(checkForm(lensCustomForm)){  
			$("img").removeAttr("onclick");
			lensCustomForm.action = "selLensCustom.action";
			lensCustomForm.submit();		
			showLoadingBar();
		}
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initLensCustomInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initLensCustomInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【订做镜片新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initLensCustomDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensCustomDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【订做镜片删除】";
	}
	function disableLensCustom(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initLensCustomDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensCustomDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【订做镜片停用】";
	}
	function ableLensCustom(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initLensCustomAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initLensCustomAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【订做镜片启用】";
	}
	function clean(){
	...
	*/
	function selectDW(){
		...
		if(is_iPad()){
			showPopWin("selgalessDWOpen.action?categoryID=3D&moduleID=${requestScope.moduleID}&brandID=",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selgalessDWOpen.action?categoryID=3D&moduleID=${requestScope.moduleID}&brandID=",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：订做镜片维护</TD>
             <td align="right" width="40%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		...
            		<img src="${ctx }/img/newbtn/btn_dzjpladd_0.png" btn=btn title="订做镜片新增" onClick="insert()">
...
name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
                       ...
                       <c:if test="${systemParameterPo.fsphisflag ne '2'}">
                        <tr>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->	
					<c:if test="${not empty(lensCustoms)}"> 
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        ...
                        <TH width="15%" height="26" scope=col colspan="5">操作</TH>
                          ...
                          <TH width="8%" scope=col>下加光</TH>
                          <TH width="5%" scope=col>轴位</TH>
                          <TH width="8%" scope=col>镜片种类</TH>
                        ...
                        <TD width="3%">
			                  ...
			                  <c:if test="${bgiflag==1}">
			                    <img src="${ctx }/img/newbtn/enabled_0.png" title='销售停用' btn=btn  onClick="javascript:disableLensCustom('${bgigoodsid}')">	
			                  </c:if>
			                  <c:if test="${bgiflag==0}">
			                  <img src="${ctx }/img/newbtn/unenabled_0.png" title='销售启用' btn=btn onClick="javascript:ableLensCustom('${bgigoodsid}')">
			                  </c:if>
			                    </c:if>		                     
			                  </TD>
			            <TD width="3%">
                          ...
                          <TD>${bgigoodsname}</TD>
                          <TD>${bgisphup}/${bgisphul}</TD>
                          <TD>${bgicylup}/${bgicylul}</TD>
                          <!--<TD>${not empty(bgibelowplusluminosityul)&&bgibelowplusluminosityul!='0.00'&&bgibelowplusluminosityup!='0.0'?'+':''}${bgibelowplusluminosityul}/${not empty(bgibelowplusluminosityup)&&bgibelowplusluminosityup!='0.00'&&bgibelowplusluminosityup!='0.0'?'+':''}${bgibelowplusluminosityup}</TD>  -->
                          <TD>${bgibelowplusluminosityul}/${bgibelowplusluminosityup}</TD> 
                          <TD>${bgiaxis}</TD>
                          <TD><c:choose>
                          		<c:when test="${bgiismutiluminosity == '0'}">单光</c:when>
                          		<c:when test="${bgiismutiluminosity == 'M'}">多光</c:when>
                          		<c:when test="${bgiismutiluminosity == 'J'}">渐近</c:when>
                          		<c:when test="${bgiismutiluminosity == 'K'}">抗疲劳</c:when>
                          		<c:when test="${bgiismutiluminosity == 'Q'}">其它</c:when>
                          	</c:choose></TD>
                          <TD>${bgiretailprice}</TD>
                          	<c:if test="${(permissionPo.keyf==1)}">
                          <TD>${bgicostprice}</TD></c:if>
                         <TD>
                          <c:if test="${bgiflag==1}">
                           	启用
                           </c:if>
