...
<script>
	...
	function search(){
	    ...
	    correctingErrorsFrm.action = "errorsGoodsBarCodeSel.action";
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：异常条码查询</TD>
          </TR>
          <TR>
					...
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
                        <TR class=table_title align=middle>
                          <TH width="15%" scope=col>变更单号</TH>
                          <TH width="8%" scope=col>变更日期</TH>
                          <TH width="20%" height="26" scope=col>商品条码</TH>
                          <TH width="15%" height="26" scope=col>商品代码</TH>
                          <TH width="8%" height="26" scope=col>相关仓位</TH>                          
                          <TH width="4%" scope=col>库存数量</TH>
                          <TH width="5%" scope=col>条码前18位与代码不符</TH>
                          <TH width="5%" scope=col>条码长度小于26位</TH>
                          <TH width="5%" scope=col>条码长度多于26位</TH>
                          <TH width="4%" scope=col>无条码</TH>
                          <TH width="5%" scope=col>条码中包含非数字或非字母</TH>
						  </TR>
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          ...
                          <TD>${cerrstockname}</TD>
                          <TD>${cerrlognum}</TD>
                          <TD><c:if test="${cerrbarcodeerra eq '0'}">&nbsp;</c:if><c:if test="${cerrbarcodeerra ne '0'}">√</c:if></TD>
                          <TD><c:if test="${cerrbarcodeerrb eq '0'}">&nbsp;</c:if><c:if test="${cerrbarcodeerrb ne '0'}">√</c:if></TD>
                          <TD><c:if test="${cerrbarcodeerrc eq '0'}">&nbsp;</c:if><c:if test="${cerrbarcodeerrc ne '0'}">√</c:if></TD>
                          <TD><c:if test="${cerrbarcodeerrd eq '0'}">&nbsp;</c:if><c:if test="${cerrbarcodeerrd ne '0'}">√</c:if></TD>
                          <TD><c:if test="${cerrbarcodeerre eq '0'}">&nbsp;</c:if><c:if test="${cerrbarcodeerre ne '0'}">√</c:if></TD>
						</TR>
						</s:iterator>
                      </TBODY>
                    </TABLE>
