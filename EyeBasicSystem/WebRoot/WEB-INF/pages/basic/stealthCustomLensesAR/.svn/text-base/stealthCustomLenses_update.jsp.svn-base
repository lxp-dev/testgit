	...
	function save(){
		if(checkForm(document.all.stealthCustomLensesForm)){  
		  ...
		  var bgisphup=parseFloat(document.all.bgisphup.value);
			...
			stealthCustomLensesForm.action = "updateStealthCustomLensesAR.action";
			             ...
			             <tr>
			               ...
			               <TD class="table_none" > 
                                   <c:forEach var="optionParamPoTmp" items="${optionParamPolist}" >
		                              <c:if test="${optionParamPoTmp.fopparentid=='GDType' && goodsInfoPo.bgicontacttypey == optionParamPoTmp.fopparamid}" >
		                              	${optionParamPoTmp.fopparamname }
		                               <input type="hidden" id="bgicontacttypey" value="${optionParamPoTmp.fopparamname }"/>
		                               <input type="hidden" name="goodsInfoPo.bgicontacttypey" value="${goodsInfoPo.bgicontacttypey }"/>
		                              </c:if>	                                      	
	                               </c:forEach> 
			               </TD>
			               ...
			               <TD class="table_none">
									...
									<c:forEach var="x" begin="1" end="209" step="1" varStatus="index">
										...
										<option value="${lens > 0 ? '+' : '' }<fmt:formatNumber value="${lens }" pattern="0.00"/>" ${goodsInfoPo.bgisphup eq lens ? 'selected="selected"' : '' }>${lens > 0 ? '+' : '' }<fmt:formatNumber value="${lens }" pattern="0.00"/></option>
			               			</c:forEach>
      	                   		 </select> -
	                             <select id="bgisphul" name="goodsInfoPo.bgisphul" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '球镜上限不能为空！'}]">
									...
									<c:forEach var="x" begin="1" end="209" step="1" varStatus="index">
			               				<c:set var="lens" value="${-26.00 + (0.25 * (index.index - 1))}" />
										<option value="${lens > 0 ? '+' : '' }<fmt:formatNumber value="${lens }" pattern="0.00"/>" ${goodsInfoPo.bgisphul eq lens ? 'selected="selected"' : '' }>${lens > 0 ? '+' : '' }<fmt:formatNumber value="${lens }" pattern="0.00"/></option>
			               			</c:forEach>
      	                   		 </select>
	                             <label style="color:red;">&nbsp;*</label>
			               </TD>
