...
<script>
	...
	function setBillTemplate(id,type,url){
		window.parent.changeBillTemplateValues(id,type,url);
                  ...
                  <c:forEach items="${fittingTemplateList}" var="fittingTemplateInfo" varStatus="status">
                    <c:if test="${status.index >= currentIndex && currentCount <= columnCount }" >
                   		<td width="220" >
                        	...
                        	<input type="radio" name="bftid" value="${fittingTemplateInfo.bftid}" ${requestScope.id eq fittingTemplateInfo.bftid ? 'checked="checked"' : '' } onclick="setBillTemplate('${fittingTemplateInfo.bftid}','${fittingTemplateInfo.bfttype }','${ctx}${fittingTemplateInfo.bftfileurl}')">
                        	<c:choose>
