...
<script>
	...
	function doList(link,perPage_Select,perPage_Text_Hidden){
     	var objOne = document.all[perPage_Select];
	  	...
	  	stealthFinishedForm.action=link;
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initStealthFinishedUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthFinishedUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形成品片修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initStealthFinishedDetails.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initStealthFinishedDetails.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形成品片详细】";
	}
	function search(){
		if(checkForm(stealthFinishedForm)){  
			var minSphyj = $("#minSphyj");
			var maxSphyj = $("#maxSphyj");
			var minCylyj = $("#minCylyj");
			var maxCylyj = $("#maxCylyj");
			
			if(parseFloat(minSphyj.val()) > parseFloat(maxSphyj.val())){
				alert("球镜区间填写有误！");
				minSphyj.select();
				return;
			}
			
			if(parseFloat(minCylyj.val()) > parseFloat(maxCylyj.val())){
				alert("柱镜区间填写有误！");
				minCylyj.select();
				return;
			}
			...
			stealthFinishedForm.action = "selStealthFinished.action";
			stealthFinishedForm.submit();		
			showLoadingBar();
		}
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initStealthFinishedInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthFinishedInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形成品片新增】";
	}
	function insertBulk(){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initStealthFinishedInsertBulk.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthFinishedInsertBulk.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【隐形成品片批量新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initStealthFinishedDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthFinishedDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【隐形成品片删除】";
	}
	function disableGlassesAccessories(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initStealthFinishedDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthFinishedDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【隐形成品片停用】";
	}
	function ableGlassesAccessories(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initStealthFinishedAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthFinishedAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【隐形成品片启用】";
	}
	function clean(){
		...
		document.getElementById('bgiusetypeyj').value = "";
		document.getElementById('bgistealthclassyj').value = "";
		document.getElementById('minSphyj').value = "";
		document.getElementById('maxSphyj').value = "";
		document.getElementById('minCylyj').value = "";
		document.getElementById('maxCylyj').value = "";
	...
	*/
	function selectDW(){
		...
		if(is_iPad()){
			showPopWin("selgalessDWOpen.action?categoryID=40&moduleID=${requestScope.moduleID}&brandID=",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selgalessDWOpen.action?categoryID=40&moduleID=${requestScope.moduleID}&brandID=",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="30%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：隐形成品片维护</TD>
             <td align="right" width="60%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		...
            		<img src="${ctx }/img/newbtn/btn_yxcppladd_0.png" btn=btn title="隐形成品片新增" onClick="insert()">
            	<img src="${ctx }/img/newbtn/btn_yxcpplpladd_0.png" btn=btn title="隐形成品片批量新增" onClick="insertBulk()">
            	</c:if>
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR>
                <TD width=1 background=${ctx}/img/pic/tab_bg.gif>
                	<IMG height=1 src="${ctx}/img/pic/tab_bg.gif" width=1></TD>
                <TD 
...
name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
                        ...
                        <tr>
                        	<TD height="26" class="table_body">隐形使用类型</TD>
			                <TD class="table_none">
			                ...
			                <TD height="26" class="table_body">隐形球镜范围</TD>
			               <TD class="table_none">
                        ...
                        <tr>
						   <TD height="26" class="table_body" >隐形柱镜范围</TD>
			               <TD class="table_none">
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->	
					<c:if test="${not empty(stealthFinishedList)}"> 
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
						...
						<s:iterator value="stealthFinishedList">
