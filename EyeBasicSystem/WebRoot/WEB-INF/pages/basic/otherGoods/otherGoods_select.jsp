...
<script>	
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initOtherGoodsUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initOtherGoodsUpdate.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【耗材修改】";
	}
	...
	function details(id){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initOtherGoodsDetails.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initOtherGoodsDetails.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【耗材详细】";
	}
	function search(){
		$("img").removeAttr("onclick");
		otherGoodsForm.action = "selOtherGoods.action";
		otherGoodsForm.submit();		
		showLoadingBar();
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initOtherGoodsInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initOtherGoodsInsert.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		document.getElementById('popupTitle').innerHTML="【耗材新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initOtherGoodsDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initOtherGoodsDelete.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【耗材删除】";
	}
	function disableOtherGoods(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initOtherGoodsDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initOtherGoodsDisable.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【耗材停用】";
	}
	function ableOtherGoods(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initOtherGoodsAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initOtherGoodsAble.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【耗材启用】";
	}
	function clean(){
		...
		document.getElementById('bgiothergoodsbigclass').value = "";
	...
	function openBrand(){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("selBrandOpen.action?categoryID_open=7&supplierID_open=" + document.getElementById('supplierID').value,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("selBrandOpen.action?categoryID_open=7&supplierID_open=" + document.getElementById('supplierID').value,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：耗材维护</TD>
            <td align="right" width="40%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		<img src="${ctx }/img/newbtn/btn_hcladd_0.png" btn=btn title="耗材新增" onClick="insert()">
            	</c:if>
...
name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
					...
					<!-- Loading Bar ----------------------------------------------------------------------------------------------------------------->	
					<c:if test="${not empty(otherGoodsList)}"> 
					<table width="100%"   border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
						...
						<s:iterator value="otherGoodsList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                         ...
                         <TD width="3%">
			                  ...
			                  <c:if test="${bgiflag==1}">
			                    <img src="${ctx }/img/newbtn/enabled_0.png" btn=btn title='销售停用' onClick="javascript:disableOtherGoods('${bgigoodsid}')">	
			                  </c:if>
			                  <c:if test="${bgiflag==0}">
			                  <img src="${ctx }/img/newbtn/unenabled_0.png" btn=btn title='销售启用' onClick="javascript:ableOtherGoods('${bgigoodsid}')">	
			                  </c:if>
			                    </c:if>		                     
			                  </TD>
			             <TD width="3%">
