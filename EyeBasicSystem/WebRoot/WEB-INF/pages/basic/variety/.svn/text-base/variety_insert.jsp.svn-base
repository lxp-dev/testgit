...
<script>	
	function save(){
		if(checkForm(brandForm)){
			...
			brandForm.action = "insertVariety.action";
			brandForm.submit();
		}
	}
	 ...
	 */
	function openBrand(){
		showPopWin("","selBrandOpen.action?categoryID_open=" + document.all.bvygcid.value + "&supplierID_open=" + "",screen.width-200,screen.height-200, '', null, true);
	}
  ...
  <TR>
    <TD><!-- ?? Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        ...
        <TR>
          <TD class=menubar_function_text height=27><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：品种新增</TD>
                    ...
                    <table width="100%"  border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder" id="title1">
                      <TBODY>
                        <TR>
			               ...
			               <TD width="40%" class="table_none"><input class="text_input100" name="varietyPo.bvyid" maxlength="2" value="${varietyPo.bvyid }" validate="[{'Type' : Type.String, 'Formula' : Formula.notNull, 'Message' : '品种代码不能为空！'}]"></TD>
						...
						<TR>   					   
						   ...
						   <li class="horizontal_onlyRight">
						   		<input id="selbrandName" class="text_input200" id="selbrandName" name="varietyPo.bbdbrandname" readonly="readonly" value="${varietyPo.bbdbrandname }" />
						  ...
						  <INPUT class=button_bak id=ctl00_PageBody_Button1 icon="icon-zoom" type=button value="选 择" name=ctl00$PageBody$Button1 onClick="openBrand();"></li></TD>
                        	...
                        	<TD height="30" class="table_body">商品类别</TD>
						  	...
						  	<select id="bvygcid" name="varietyPo.bvygcid">
						  		<s:iterator value="goodsCategorys">
								<option value="${bgcid}" ${varietyPo.bvygcid == bgcid ? 'selected="selected"' : '' } >${bgcgoodscategoryname}</option>
	     	               		</s:iterator>
							</select></TD>
                        </TR>
                      </TBODY>
