...
<script>
	...
	function update(id){
		...
		if(is_iPad()){
			showPopWin("initStealthFinishedUpdateAR.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthFinishedUpdateAR.action?moduleID=${requestScope.moduleID}&goodsTree="+goodsTree+"&parent="+parent+"&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品修改】";
	}
	...
	function details(id){
		...
		if(is_iPad()){
			showPopWin("initStealthFinishedDetailsAR.action?moduleID=${requestScope.moduleID}&hid="+id,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initStealthFinishedDetailsAR.action?moduleID=${requestScope.moduleID}&hid="+id,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品详细】";
	}
	function search(){
		if(checkForm(stealthFinishedForm)){  
			...
			stealthFinishedForm.action = "selStealthFinishedAR.action";
			stealthFinishedForm.submit();		
			showLoadingBar();
		}
	}
	function insert(){
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){
			showPopWin("initStealthFinishedInsertAR.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthFinishedInsertAR.action?goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品新增】";
	}
	function insertBulk(){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initStealthFinishedInsertBulkAR.action?moduleID=${requestScope.moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initStealthFinishedInsertBulkAR.action?moduleID=${requestScope.moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
		
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品批量新增】";
	}
	function del(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		...
		var parent=document.getElementById("parent").value;
		if(is_iPad()){ 
		showPopWin("initStealthFinishedDeleteAR.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthFinishedDeleteAR.action?hid="+id+"&goodsTree="+goodsTree+"&parent="+parent+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品删除】";
	}
	function disableGlassesAccessories(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initStealthFinishedDisableAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthFinishedDisableAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品停用】";
	}
	function ableGlassesAccessories(id){
		var topRows = top.document.getElementById("total").rows; 
		var topCols = top.document.getElementById("btmframe").cols; 
		if(is_iPad()){ 
		showPopWin("initStealthFinishedAbleAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		}else{ 
		showPopWin("initStealthFinishedAbleAR.action?hid="+id+"&moduleID=${requestScope.moduleID}",400,160, topRows,topCols,returnRefresh(true),true); 
		} 
		document.getElementById('popupTitle').innerHTML="【软性接触镜成品启用】";
	}
	function clean(){
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
          <TR>
            ...
            <TD align="left" width="30%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：软性接触镜成品维护</TD>
             <td align="right" width="60%" valign="bottom">&nbsp;
            	<c:if test="${(permissionPo.keya==1)}">
            		...
            		<img src="${ctx }/img/newbtn/btn_yxcppladd_0.png" btn=btn title="软性接触镜成品新增" onClick="insert()">
            	<img src="${ctx }/img/newbtn/btn_yxcpplpladd_0.png" btn=btn title="软性接触镜成品批量新增" onClick="insertBulk()">
            	</c:if>
...
name="bgiretailendprice" value="${requestScope.bgiretailendprice}" onKeyUp="value=value.replace(/[^\d\.]/g,'')"></div>
                        ...
                        <tr>
                        	<TD height="26" class="table_body">接触镜使用类型</TD>
			                <TD class="table_none">
			                ...
			                <TD height="26" class="table_body">接触镜球镜范围</TD>
			               <TD class="table_none">
                            <input class="text_input80" type="text"  id="minSphyj" name="minSphyj" value="${requestScope.minSphyj}" maxlength="6" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新填写接触镜球镜度数！'}]">
                            	至
                            <input class="text_input80" type="text"  id="maxSphyj" name="maxSphyj" value="${requestScope.maxSphyj}" maxlength="6" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新填写接触镜球镜度数！'}]">
			               </TD>
                        </tr>
                        <tr>
						   <TD height="26" class="table_body" >接触镜柱镜范围</TD>
			               <TD class="table_none">
                            <input class="text_input80" type="text"  id="minCylyj" name="minCylyj" value="${requestScope.minCylyj}" maxlength="6" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新填写接触镜柱镜度数！'}]">
                            	至
                            <input class="text_input80" type="text"  id="maxCylyj" name="maxCylyj" value="${requestScope.maxCylyj}" maxlength="6" validate="[{'Type' : Type.String, 'Formula' : Formula.SphCylOrNull, 'Message' : '请重新填写接触镜柱镜度数！'}]">
			               </TD>
