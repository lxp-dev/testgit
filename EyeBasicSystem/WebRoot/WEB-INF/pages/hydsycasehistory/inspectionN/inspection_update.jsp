...
function changeFocus(obj)
{
	...
	function save(){
		...
		if($('[name*=.sopipballglassod]').length!=0){
			if(isSubmit == 0){
				...
				inspectionForm.action="inspectionUpdateHydsy.action?ruleFlag=0&nw='N' ";
	...
	$(document).ready(function(){
		...
		var sopipestimateos = '${inspectionPos[0].sopipestimateos}'.split(',');
		for(var i=0; i<sopipestimate.length; i++){
			$('[name=inspectionPo.sopipestimateos][value='+sopipestimateos[i].trim()+']').attr("checked","checked");
		}
	...
	function refractive(){
		inspectionForm.action="refractiveToolHydsy.action?source=refractiveiou";//refractiveTool.action?source=refractiveiou
		$("img").removeAttr("onclick");
		inspectionForm.submit();
	}
	...
	function specialCheck(){
		...
		if('$(oldOptometryID)'!=''){
			inspectionForm.action="specialCheckToolHydsy.action?source=specialcheckcopy";
		}
		$("img").removeAttr("onclick");
		inspectionForm.submit();
	}
		...
		+"</td></tr>";  				  
			  ...
			  if(myValue == "6"){
				  c1.innerHTML="<tr id="+rowadd+"tr><td><input type='hidden' name='inspectionPos["+Subtr(1,listindex)+"].sopipglasstype' value='6'/>"
				  								...
				  								+"<TD su=ht bgcolor='#F8E0F0' class='PrivateBorderPink'><div align='center'>"
				  								+"<input name='inspectionPos["+Subtr(1,listindex)+"].sopipantongos' value='${po.sopipantongos}' class='text_input' size='2' onKeyPress='EnterDown(this)' enter='enter7' maxlength='10'></div></TD>"
		  ...
		  if("${po.sopipglasstype }" == "3"){
			...
			c1.innerHTML=c1.innerHTML+"<tr id="+rowadd+"tr><td>"
											...
											+"<INPUT name='inspectionPos["+Subtr(1,listindex)+"].sopipcommendglassesod' value='${fn:split(po.sopipcommendglassesod,',')[0]}' syjpod='"+allTrafficCount+"' id='sopipcommendglassesod' size='8' readOnly>"
								  ...
								  <tr>
								  	...
								  	<td bgcolor="#CADEE8">&nbsp;近视眼:
									    <SELECT id="sopipneareyeos" name="inspectionPo.sopipneareyeos" style=" width:120;">
	                                          ...
	                                          <c:forEach var="optionParamPoTmp" items="${optionParamPolist}" >
			                                      <c:if test="${optionParamPoTmp.fopparentid=='44'}">
			                                      	<option value="${optionParamPoTmp.fopparamid }" ${(inspectionPos[0].sopipneareyeos == optionParamPoTmp.fopparamid)? 'selected=selected' :'' }>${optionParamPoTmp.fopparamname}</option>
			                                      </c:if>	                                      	
		                                      </c:forEach>
		                                    </select>
									  &nbsp;
									 ...
									 <SELECT id="sopipfareyeos" name="inspectionPo.sopipfareyeos" style=" width:120;">
	                                          ...
	                                          <c:forEach var="optionParamPoTmp" items="${optionParamPolist}" >
			                                      <c:if test="${optionParamPoTmp.fopparentid=='45'}">
			                                      	<option value="${optionParamPoTmp.fopparamid }" ${(inspectionPos[0].sopipfareyeos == optionParamPoTmp.fopparamid)? 'selected=selected' :'' }>${optionParamPoTmp.fopparamname}</option>
			                                      </c:if>	                                      	
		                                      </c:forEach>
		                                    </select>
									  &nbsp;
									 散光：
									  <SELECT id="sopipastigmiaos" name="inspectionPo.sopipastigmiaos" style=" width:120;">
	                                          ...
	                                          <c:forEach var="optionParamPoTmp" items="${optionParamPolist}" >
			                                      <c:if test="${optionParamPoTmp.fopparentid=='46'}">
			                                      	<option value="${optionParamPoTmp.fopparamid }" ${(inspectionPos[0].sopipastigmiaos == optionParamPoTmp.fopparamid)? 'selected=selected' :'' }>${optionParamPoTmp.fopparamname}</option>
			                                      </c:if>	                                      	
		                                      </c:forEach>
		                                    </select>
									  &nbsp;</li>
