...
<script>	
	...
	function details(id,sopecid){
		...
		if(is_iPad()){
			showPopWin("detailEyesCheckAier.action?hid="+id+"&sopecid="+sopecid,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("detailEyesCheckAier.action?hid="+id+"&sopecid="+sopecid,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
	...
	//打开配镜详细信息 
	 function printDetails(optometryID,optometryBasicID,dontshow){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initRefractiveSelectAier.action?optometryID="+optometryID+"&customerID="+'${customerID}'+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&dontshow="+dontshow,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initRefractiveSelectAier.action?optometryID="+optometryID+"&customerID="+'${customerID}'+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&dontshow="+dontshow,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
