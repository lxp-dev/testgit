...
<script>	
   ...
   window.onload=function(){
	...
	function details(id,sopecid){
		...
		if(is_iPad()){
			showPopWin("detailEyesCheckAier.action?hid="+id+"&sopecid="+sopecid+"&optometryPersonID="+$("#optometryPersonID").val()+"&optometryPersonName="+encodeURI($("#optometryPersonName").val()),970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("detailEyesCheckAier.action?hid="+id+"&sopecid="+sopecid+"&optometryPersonID="+$("#optometryPersonID").val()+"&optometryPersonName="+encodeURI($("#optometryPersonName").val()),screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
	...
	function search(optometryID,optometryBasicID,flag,chuyanfuyan,personid,sopoyoneormany){
		...
		if(flag=='0'){
			if($('#optometryPersonID').val() == personid){
				...
				if(is_iPad()){
					showPopWin("initRefractiveSelectAier.action?isfresh="+ document.getElementById("isfresh").value +"&optometryID="+optometryID+"&customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&chuyanfuyan="+sopoyoneormany+"&optometryPersonID="+$("#optometryPersonID").val()+"&optometryPersonName="+encodeURI($("#optometryPersonName").val()),970,screen.height-200,topRows,topCols,returnRefresh(true),false);
				}else{
					showPopWin("initRefractiveSelectAier.action?isfresh="+ document.getElementById("isfresh").value +"&optometryID="+optometryID+"&customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryBasicID="+optometryBasicID+"&moduleID="+'${moduleID}'+"&chuyanfuyan="+sopoyoneormany+"&optometryPersonID="+$("#optometryPersonID").val()+"&optometryPersonName="+encodeURI($("#optometryPersonName").val()),screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
				}
				document.getElementById('popupTitle').innerHTML="【验光信息详细】";
	...
	function inspectionCopy(){
		...
		showPopWin("","inspectionCopyAier.action?oldOptometryID="+'${optometryBasics[0].optometrys[0].sopoyoptometryid}'+"&customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryBasicID="+'${optometryBasics[0].sopoboptometrybasicid}',screen.width-200,screen.height-200, '',null,true);
		selectHidden();
	}
	...
	function del(optometryID,basicID){
		...
		var topCols = top.document.getElementById("btmframe").cols;
		if(is_iPad()){
			showPopWin("initOptometryBasicDeleteAier.action?basicID=" + basicID + "&optometryID="+optometryID+"&moduleID="+'${requestScope.moduleID}',400,140,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initOptometryBasicDeleteAier.action?basicID=" + basicID + "&optometryID="+optometryID+"&moduleID="+'${requestScope.moduleID}',400,140,topRows,topCols,returnRefresh(true),true);
		}
	...
	function glassHistory(){
		...
		optometryBasicForm.action="selectGlassesHistoryAier.action?customerID="+'${customerInfoPo.smecicustomerid }'+"&optometryPersonID="+$("#optometryPersonID").val()+"&optometryPersonName="+encodeURI($("#optometryPersonName").val());
		optometryBasicForm.submit();
	}
...
<input type="hidden" name="psmecicustomerid" id="psmecicustomerid" value="">
