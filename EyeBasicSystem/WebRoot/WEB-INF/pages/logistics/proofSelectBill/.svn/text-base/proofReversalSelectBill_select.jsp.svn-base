...
<script>	
	...
	function search(){
		selectBill.action = "selectSelectReversal.action?bVoucherType="+document.all.bVoucherType.value+"&moduleID=${moduleID}";
		selectBill.submit();
		showLoadingBar();
	}
	...
	function billinfo(id){
		...
		if(is_iPad()){
			showPopWin("initReversalInfoSel.action?hid="+id+"&moduleID=${moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initReversalInfoSel.action?hid="+id+"&moduleID=${moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
                ...
                vAlign=top><DIV id=tabContent__1>
                  <DIV>
                    <table width="100%" id="title0"  border=0 align=center cellpadding=0 cellspacing=0 class="privateTable">
                      ...
                      <TBODY>
					  	<TR>
						   <TD width="9%" height="26" class="table_body">冲回编号</TD>
			               <TD class="table_none" width="24%">
                            <input class="text_input160" type="text" id="billid" name="billid" maxlength=21 value="${requestScope.billid}">
			               </TD>
			               <TD width="9%" class="table_body">冲回日期</TD>
