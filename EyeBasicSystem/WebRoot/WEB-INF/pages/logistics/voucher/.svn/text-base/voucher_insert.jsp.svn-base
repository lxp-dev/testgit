        ...
        if (chk != 0){
            ...
            if (auditState != null && auditState.length != 0){
	            if (!auditState[0].checked){
	                 insertVoucherFrm.action = "insertVoucher.action?auditState=0&id="+sVoucherType;
	            }else{
	                 insertVoucherFrm.action = "insertVoucher.action?auditState=1&id="+sVoucherType;
	            }   
            }else{
                insertVoucherFrm.action = "insertVoucher.action?auditState=0&id="+sVoucherType;
            }       
	 ...
	 */         
	 function search(){
	   ...
	   var url = "supplierName="+EncodeUtf8(supplierName)+"&supplierID="+supplierID+"&bVoucherType="+bVoucherType+"&sVoucherType="+sVoucherType+"&voucherID="+voucherID
		...
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("initSelectBillSels.action?moduleID=${moduleID}&" + url,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initSelectBillSels.action?moduleID=${moduleID}&" + url,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
	 ...
	 */        
	 function openGoodSingleValues(objValue){
	      ...
	      switch(sVoucherType){
	          ...
	          case '209':
	          case '210':
	                     insertVoucherFrm.action="initInvoiceVoucherInsert.action?moduleID="+$('#moduleID').val()+"&billID="+objValue+"&id="+"F";
	                     insertVoucherFrm.submit();
	                     break;
	          //冲回
	          case '102':
	          ...
	          case '206':
	          case '208':
	                	 insertVoucherFrm.action="initReversalVoucherInsert.action?moduleID="+$('#moduleID').val()+"&billID="+objValue+"&id="+"C";
		                 insertVoucherFrm.submit();
	                     break;
	          //单据
	          default:  
	       	             insertVoucherFrm.action="selectBillGoodsInsert.action?moduleID="+$('#moduleID').val()+"&billID="+objValue+"&id="+"D";
	       	             insertVoucherFrm.submit();		
	     }
	 }
              ...
              <TR>
                      ...
                      <TBODY>
                        <TR height="26px">
                          ...
                          <li class="horizontal_onlyRight">
                            ...
                            <input type="hidden" id="supplierID" name="supplierID" class="text_input160" value="${supplierID}">
                          </li>
                            <li class="horizontal_onlyRight">
                              <img btn=btn src="${ctx }/img/newbtn/btn_change_0.png" title="选择" id="searchBtn" name="searchBtn" onclick="openSupplier();" >
                            </li></TD>
                          </TR>
                        <TR height="26px">
                ...
                <c:if test="${id == 'F'}">
                      ...
                      <TBODY>
                        ...
                        <s:iterator value="voucherEntryList">
                         <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          <TD height="26"><input type="checkbox" id="chk" name="chk" value="${sLveveBillID}" ></TD> 
                          <TD>
