...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        var total = 0;    		 
		$('input[name=voucherEntryTempPo.sCostPriceAmount]').each(function(){
    ...
    */
	function search(voucherID,voucherDate,bType,sType,sVoucherType,supplierName,createPerson,posting){
	   var url = "?voucherID="+EncodeUtf8(voucherID)+"&voucherDate="+EncodeUtf8(voucherDate)+"&bType="+EncodeUtf8(bType)+"&sType="+EncodeUtf8(sType)
	             ...
	             +"&posting="+EncodeUtf8(posting);
	   
	   	var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin('selVoucherTally.action'+url,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin('selVoucherTally.action'+url,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
                          ...
                          <TD width="24%" class="table_none">
                            ...
                            <li class="horizontal_onlyRight">
                              <img btn=btn src="${ctx }/img/newbtn/btn_jzpzxz_0.png" title='记账凭证新增' 
						         onClick="search('${po.sLvvID}','${fn:substring(po.sLvvDate,0,10)}','${po.sLvvVoucherParentTypeName}','${po.sLvvVoucherTypeName}','${po.sLvvVoucherTypeID}','${po.sLvvSupplierName}','${po.sLvvPersonName}','${po.sLvvPosting}');">
                            </li>
						  </TD>
