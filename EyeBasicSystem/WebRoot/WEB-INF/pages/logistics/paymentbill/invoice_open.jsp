...
<script>
	...
	function search(){
		insertPayMentFrm.action = "selInvoiceOpen.action";
		insertPayMentFrm.submit();
		showLoadingBar();
	}	
	...
	function detail(bid){
		...
		if(is_iPad()){
			showPopWin("initInvoiceDetail.action?invoiceID="+bid,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initInvoiceDetail.action?invoiceID="+bid,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}
    ...
    function setCheckValue(){
        ...
        $("input[id=chk]").each(function(){
         	if(chktext.indexOf($(this).attr("invoiceid"))>=0){
                      ...
                      <TBODY>
                        <TR height="26px">
                          ...
                          <TD width="9%"  class="table_body">制造商</TD>
			               ...
			               ${supplierName}
							   			<input type="hidden" id="supplierName" readonly="readonly" class="text_input160" name="supplierName" value="${supplierName}" />
							   			<input type="hidden" id="supplierID" name="supplierID" value="${supplierID }" readonly="readonly"/>
						   	</TD>
					      <TD class="table_body" width="9%">制作人编号</TD>
                          <TD class="table_none" width="24%"><input type="text" clean="clean" id="createPersonID" name="createPersonID" value="${createPersonID}" class="text_input160"/></TD>	
                        </TR>
                        <TR height="26px">
			...
			<c:if test="${not empty(invoiceEntryList)}">       
                      ...
                      <TBODY>
                        <TR class=table_title align='middle'>
                          ...
                          <TH scope=col width="10%">发票日期</TH>
                          <TH scope=col width="10%">制单人</TH>
                          <TH scope=col width="10%">未付款金额</TH>
                          
                        </TR>
                        <s:iterator value="invoiceEntryList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')" id="invoiceRow">
						  <TD height="26">
						       <input type="checkbox" id="chk" name="chk" invoiceid="${lieieinvoiceid}" onClick="setSingleValue(this);" value="{'invoiceid':'${lieieinvoiceid}','costpriceamount':'${lieiecostpriceamount}'}" />
						  </TD>
						   <TD><img btn=btn src="${ctx }/img/newbtn/search_0.png" title='详细' onClick="detail('${lieieinvoiceid}')"></TD>
						  <TD>${lieieinvoiceid }</TD>
						  <TD>${fn:substring(invoiceDate,0,10) }</TD>
