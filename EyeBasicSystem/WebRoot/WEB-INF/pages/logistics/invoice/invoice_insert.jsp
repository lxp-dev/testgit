...
<script>
	...
	function shuliangCal(obj){		
		...
		{
			$(obj).parent().parent().find('input[name=invoiceEntryPo.lieienottaxrate]').val()="0.00";
		}
	...
	function hanshuiCal(obj){		
		...
		//反填单位成本
		$(obj).parent().parent().find('input[name=invoiceEntryPo.lieienottaxrate]').val(parseFloat(accDiv(hanshuidanjia,1+parseFloat(cstietaxrate*0.01))).toFixed(2));
	...
	function save(){
	    ...
	    if (auditState != null  && auditState.length != 0 ){
		    if (!auditState[0].checked){
	            invoiceForm.action = "invoiceInsert.action?auditState=0";
	        }else{
	            invoiceForm.action = "invoiceInsert.action?auditState=1";
	        }
	    }else{
	        invoiceForm.action = "invoiceInsert.action?auditState=0";
	    }
	...
	$(document).ready(function(){
		...
		row.className = 'row';
		c1.height="26";
		c1.innerHTML = '<input id="chk" name="chk" type="checkbox" value="${cstieid}"><input type="hidden" name="invoiceEntryPo.lieieid" value="${cstieid}" /><input name="invoiceEntryPo.cstiebilltypeid" type="hidden" value="${cstiebilltypeid}">';
		...
		c8.innerHTML = '<input type="text" onblur="danweiCal(this);toFix(this)" value="${cstienottaxrate}" name="invoiceEntryPo.lieienottaxrate" id="danwei${idx.index}" class="text_input60" onkeyup="toFixAndNan(this);danweiDown(this);"/>';	
	...
	function openGoodSingleValues(objValue,startDate,endDate){
		...
		invoiceForm.action="selBills.action?billID="+objValue+"&invoiceType="+$("#invoiceType").val()+"&billStringID="+document.getElementById('billStringID').value+"&invoiceStartDate="+startDate+"&invoiceEndDate="+endDate+"&moduleID=${moduleID}";
		invoiceForm.submit();
	}
    ...
    function openSelectGoods(){
		...
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("initInvoiceSelectGoods.action?&supplierID="+document.getElementById('supplierID').value+"&supplierName="+EncodeUtf8(document.getElementById('supplierName').value)+"&invoiceType="+$("#invoiceType").val()+"&moduleID=${moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initInvoiceSelectGoods.action?&supplierID="+document.getElementById('supplierID').value+"&supplierName="+EncodeUtf8(document.getElementById('supplierName').value)+"&invoiceType="+$("#invoiceType").val()+"&moduleID=${moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
		document.getElementById('popupTitle').innerHTML="【单据查询】";
    }
	...
	function addRow(goodInfo){
		...
		var issubmit = '0';
		$("input[id=chk]").each(function(){
		    if($(this).val() == goodInfo.cstieid){
			    issubmit='1';
            }
         	if(typeid == 0 || (typeid != goodInfo.cstiebilltypeid)){
				issubmit='1';
            }
		});
		if(issubmit == '1'){
		...
		row.className = 'row';
		c1.height="26";
		c1.innerHTML = '<input id="chk" name="chk" type="checkbox" value="'+goodInfo.cstieid+'"><input type="hidden" name="invoiceEntryPo.lieieid" value="'+goodInfo.cstieid+'" /><input name="invoiceEntryPo.cstiebilltypeid" type="hidden" value="'+goodInfo.cstiebilltypeid+'">';
        c2.innerHTML = goodInfo.cstiebillid + '<input type="hidden" name="invoiceEntryPo.lieiebillid" value="'+goodInfo.cstiebillid+'" />';
        c3.innerHTML = goodInfo.cstiegoodsid + '<input type="hidden" name="invoiceEntryPo.lieiegoodsid" value="'+goodInfo.cstiegoodsid+'" />';
		c4.innerHTML = goodInfo.cstiegoodsname + '<input type="hidden" name="invoiceEntryPo.lieiegoodsname" value="'+goodInfo.cstiegoodsname+'" />';
		c5.innerHTML = goodInfo.cstiespec + '<input type="hidden" name="invoiceEntryPo.lieiespec" value="'+goodInfo.cstiespec+'" />';
		c6.innerHTML = goodInfo.cstiegoodsquantity + '<input type="hidden" name="invoiceEntryPo.lieiegoodsquantity" value="'+goodInfo.cstiegoodsquantity+'" />';
		c6.id="td1";
		c7.id="td2";		
		c9.id="td3";
		c12.id="td4";
		c13.id="td5";
		c7.innerHTML = '<input onkeyup="toFixAndNum(this);hexiaoDown(this);" onblur="shuliangCal(this);" type="text" name="invoiceEntryPo.lieiecheckgoodsquantity" value="'+goodInfo.cstiecheckgoodsquantity+'" id="hexiao${idx.index}" class="text_input60" >';	
		c8.innerHTML = '<input type="text" onblur="danweiCal(this);toFix(this)" value="'+goodInfo.cstienottaxrate+'" name="invoiceEntryPo.lieienottaxrate" id="danwei${idx.index}" class="text_input60" onkeyup="toFixAndNan(this);danweiDown(this);"/>';	
		
		c9.innerHTML =  '<input readonly="readonly" onkeyup="toFixAndNan(this);amount();" style="background-color:#ACA899" type="text" name="invoiceEntryPo.lieienottaxrateamount" value="'+goodInfo.cstienottaxrateamount+'" id="chengben${idx.index}" class="text_input60">';		
			
		c10.innerHTML = '<input type="text" value="'+goodInfo.cstietaxrate+'" name="cstietaxrate" class="text_input60"/><input type="hidden" value="'+goodInfo.cstietaxrate+'" name="invoiceEntryPo.lieietaxrate"/>';	
		c11.innerHTML = '<input onblur="hanshuiCal(this);toFix(this)" type="text" value="'+goodInfo.cstiecostprice+'" name="invoiceEntryPo.lieiecostprice" onkeyup="toFixAndNan(this);hanshuiDown(this);" id="lieiecostprice${idx.index}" class="text_input60" />';
		
		c12.innerHTML = '<input readonly="readonly" onkeyup="toFixAndNan(this);amount();" style="background-color:#ACA899" type="text" value="'+goodInfo.cstietaxamount+'" name="invoiceEntryPo.lieietaxamount" class="text_input60" id="shuie${idx.index}" />';
		c13.innerHTML = '<input readonly="readonly" onkeyup="toFixAndNan(this);amount();" style="background-color:#ACA899" type="text" name="invoiceEntryPo.lieiecostpriceamount" value="'+goodInfo.cstiecostpriceamount+'"  id="jiashui${idx.index}" class="text_input60" >';
                      ...
                      <TBODY>
                        <TR>
                          <TD align="left">
                            ...
                            <img btn=btn src="${ctx }/img/newbtn/btn_xzsp_0.png" title="选择商品" onClick="javascript:openSelectGoods()" >
                        ...
                        <TR>
                          <TD align="left" colspan="2">
                             <font color="red">发票审核后,退货单数量、核销数量、成本合计、税额合计、价税合计变为负值!</font>
                          </TD>
                        </TR>
                      </TBODY>
                    </TABLE>
