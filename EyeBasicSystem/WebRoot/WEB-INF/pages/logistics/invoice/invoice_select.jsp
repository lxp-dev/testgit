...
<script>
	...
	function update(bid){
		...
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("initInvoiceUpdate.action?invoiceID="+bid+"&moduleID="+document.getElementById('moduleID').value,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initInvoiceUpdate.action?invoiceID="+bid+"&moduleID="+document.getElementById('moduleID').value,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}		
    ...
    function openSelectBill(){
		...
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("initInvoiceSelectBill.action?moduleID=${moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initInvoiceSelectBill.action?moduleID=${moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
		document.getElementById('popupTitle').innerHTML="【单据查询】";
    }
	
	function search(){
		invoiceForm.action = "selInvoice.action";
		invoiceForm.submit();
		showLoadingBar();
	}	
	function detail(bid){
		...
		var topCols = top.document.getElementById("btmframe").cols;			
		if(is_iPad()){
			showPopWin("initInvoiceDetail.action?invoiceID="+bid+"&moduleID=${moduleID}",970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("initInvoiceDetail.action?invoiceID="+bid+"&moduleID=${moduleID}",screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}		
	...
	function insert(){
		...
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("initInvoiceInsert.action?moduleID="+document.getElementById('moduleID').value,970,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}else{
			showPopWin("initInvoiceInsert.action?moduleID="+document.getElementById('moduleID').value,screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),true);
		}		
	...
	function del(bid){	
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;
		showPopWin("initInvoiceDelete.action?invoiceID="+bid+"&moduleID="+document.getElementById('moduleID').value,450,140,topRows,topCols,returnRefresh(true),true);
                        ...
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')" id="invoiceRow" height="26">
		                  ...
		                  <c:if test="${permissionPo.keyk=='1'}">
		                  	<c:if test="${liiauditstatue != 0}">
                          		<img btn=btn src="${ctx }/img/newbtn/audit_0.png" title='反审核' onClick="unAudit('${liiid}')">
                          	</c:if>
                          	<c:if test="${liiauditstatue == 0}">
                          		<img btn=btn src="${ctx }/img/newbtn/audit_2.png" title='反审核'>
                          	</c:if>
                          	</c:if>
		                  </TD>
