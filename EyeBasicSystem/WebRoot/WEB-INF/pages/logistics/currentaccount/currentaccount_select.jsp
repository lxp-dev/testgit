...
<script type="text/javascript">
    ...
    *  查询往来账详细信息
    */ 
	function detail(supplierID,supplierName){
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin("supplierCurrentAccountDetail.action?supplierID="+supplierID+"&moduleID=${moduleID}&startDate="+$('#startDate').val()+"&endDate="+$('#endDate').val()+"&supplierName="+EncodeUtf8(supplierName),970,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}else{
			showPopWin("supplierCurrentAccountDetail.action?supplierID="+supplierID+"&moduleID=${moduleID}&startDate="+$('#startDate').val()+"&endDate="+$('#endDate').val()+"&supplierName="+EncodeUtf8(supplierName),screen.width-100,screen.height-200,topRows,topCols,returnRefresh(true),false);
		}
    ...
    */ 
	function printReport(id,name){
		...
		var DataURL = "supplierCurrentAccountDetailByReport.action?supplierID="+id+"&bgnDate="+BeginDate+"&endDate="+End+"&moduleID=${moduleID}"; 
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
    ...
    */ 
	function printReport1(){
		...
		var DataURL = "report.action?reportlet=logistics_supplierduizhangdanRpt2.cpt&__bypagesize__=false&supplierID="+$('#supplierID').val()+"&bgnDate="+BeginDate+"&endDate="+End+"&logincompanyid=${person.personcompanyid }"; 
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin(DataURL,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
    ...
    */ 
	function printReport2(){
		...
		var DataURL = "report.action?reportlet=logistics_supplierduizhangdanRpt3.cpt&__bypagesize__=false&supplierID="+$('#supplierID').val()+"&bgnDate="+BeginDate+"&endDate="+End+"&logincompanyid=${person.personcompanyid }"; 
		var topRows = top.document.getElementById("total").rows;
		var topCols = top.document.getElementById("btmframe").cols;		
		if(is_iPad()){
			showPopWin(DataURL,970,screen.height-200,topRows,topCols,returnRefresh(true),false);
    ...
    */        
	function search(){
	    ...
	    payMentBillSelFrm.action = "supplierCurrentAccountSel.action";
	    payMentBillSelFrm.submit();
		showLoadingBar();
	}
  ...
  <TR>
    <TD><!-- ͷ˵ Start -->
      <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
        <TBODY>
        <TR>
          <TD class=menubar_title width="15%"><img border='0' src='${ctx}/img/pic/module.gif' align='absmiddle' hspace='3' vspace='3'>&nbsp;往来账管理</TD>
          <TD align="left" width="45%"><%@ include file="/commons/helpMovie.jsp" %>目前操作功能：往来账查询</TD>
					...
					<table id="title2" cellspacing="2">
                      <TBODY>
                        <TR>
						  ...
						  <TD align="right">
                          <c:if test="${permissionPo.keya == '1'}">							
							<img btn=btn src="${ctx }/img/newbtn/btn_angysck_0.png" title='按制造商查看' onclick="printReport2();" >			
                            <img btn=btn src="${ctx }/img/newbtn/btn_anmxck_0.png" title='按明细查看' onclick="printReport1();" >	
						  </c:if>	
						  </TD>
                        </TR>
                      </TBODY>
					 ...
					 <table width="100%" border=0 align=center cellpadding=1 cellspacing=1 class="privateBorder">
                      <TBODY>
                        <TR class=table_title align=middle>
                          ...
                          <TH scope=col width="6%" height="26">供应商代码</TH>
                          <TH scope=col width="30%">供应商简称</TH>
						  <TH scope=col width="8%">联系人</TH>						
                          <TH scope=col width="10%">联系方式</TH>
                          <TH scope=col width="9%">累积应付</TH>	
                        </TR>
                      ...
                      <s:iterator value="supplierList">
                        <TR class="row" onMouseOver="mover(this,'#a2c1eb')" onMouseOut="mout(this,'#cadee8')">
                          ...
                          <c:if test="${permissionPo.keyb == '1'}">
                             <img btn=btn src="${ctx }/img/newbtn/search_0.png" title='详细' onClick="javascript:printReport('${bspid}','${bspsuppliername}')">
                              </c:if>
		                  </TD>
                          <TD height="26">${bspid}</TD>
                          <TD>${bspsuppliername}</TD>
                          <TD>${bspcontactperson}</TD>
                          <TD>${bspcontactphone}</TD>
                          <TD>${bspstartprice}</TD>
