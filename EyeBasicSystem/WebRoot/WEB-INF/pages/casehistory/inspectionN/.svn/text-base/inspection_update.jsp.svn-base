...
function changeFocus(obj)
{
	...
	function save(){
		...
		if($('[name*=.sopipballglassod]').length!=0){
			if(isSubmit == 0){
				...
				inspectionForm.action="inspectionUpdateN.action?ruleFlag=0&nw='N' ";
	...
	function refractive(){
		inspectionForm.action="refractiveToolN.action?source=refractiveiou";//refractiveTool.action?source=refractiveiou
		$("img").removeAttr("onclick");
		inspectionForm.submit();
	}
	...
	function specialCheck(){
		...
		if('$(oldOptometryID)'!=''){
			inspectionForm.action="specialCheckToolN.action?source=specialcheckcopy";
		}
		$("img").removeAttr("onclick");
		inspectionForm.submit();
	}
										...
										<table width="99%" border=0 align=center cellpadding=1	cellspacing=1 class="privateTable privateBorder">
											<tbody>
												<tr bgcolor="#CADEE8" height="26px">
													...
													<TD><textarea  id="sopipdoctorsay" name="inspectionPo.sopipdoctorsay" validate="[{'Type' : Type.String, 'Formula' : '', 'Expansion' : {Type : Expansion.LessThanLengthORNULL, Params : [101]}, 'Message' : '医嘱不能大于100字！'}]" style="width:100%" cols="80" rows="7">${fn:trim(inspectionPos[0].sopipdoctorsay) }</textarea></TD>
												</tr>
											</tbody>
										</table>
									</fieldset>											
