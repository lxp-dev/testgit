...
<html>
	...
	function save(){
			...
			if(checkForm(refractiveForm))
			{
				...
				refractiveForm.action="refractiveInsertN.action?source=refractiveiou";
	...
	*/
	...
	function doubleEyeFun(){
		$("img").removeAttr("onclick");
		refractiveForm.action="doubleEyeFunToolN.action?source=doubleeyefuncopy";
		refractiveForm.submit();
	}
	...
	function specialCheck(){
		$("img").removeAttr("onclick");
		refractiveForm.action="specialCheckToolN.action?source=specialcheckcopy";
		refractiveForm.submit();
	}

	function refractiveCopy(){
		$("img").removeAttr("onclick");
		refractiveForm.action="refractiveCopyN.action?chuyanfuyan="+'${chuyanfuyan}'+"&oldOptometryID="+'${oldOptometryID}'+"&customerID="+'${customerID }'+"&optometryBasicID="+'${copyBasicOptometryID}'+"&moduleID="+'${moduleID}'+"&optometryID="+'${optometryID}';
		refractiveForm.submit();
	}
	...
	function fixingInterface(){
		$("img").removeAttr("onclick");
		refractiveForm.action="fixingInterfaceToolN.action";
		refractiveForm.submit();
	}
													...
													<table width="99%" border=0 align=center cellpadding=1	cellspacing=1 class="privateTable privateBorder">
														<tbody>
															<tr bgcolor="#CADEE8" height="26px">
																...
																<TD><textarea  id="soprcustomersay" name="refractivePo.soprcustomersay" validate="[{'Type' : Type.String, 'Formula' : '', 'Expansion' : {Type : Expansion.LessThanLengthORNULL, Params : [101]}, 'Message' : '主诉不能大于100字！'}]" style="width:100%" cols="80" rows="7">${fn:trim(refractivePo.soprcustomersay) }</textarea></TD>
															</tr>
														</tbody>
													</table>
												</fieldset>
																...
																<TD class="PrivateBorderBlue">&nbsp;&nbsp;
						                                          <c:forEach var="optionParamPoTmp" items="${optionParamPolist}" >
								                                      <c:if test="${optionParamPoTmp.fopparentid=='25'}">
								                                      	<input id="enter1" name="refractivePo.soprgoals" type="checkbox" value="${optionParamPoTmp.fopparamid }">${optionParamPoTmp.fopparamname}
								                                      </c:if>	                                      	
							                                      </c:forEach>
																</TD>
															</tr>
