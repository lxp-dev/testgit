...
<%@page import="com.pengsheng.eims.util.*"%>
...
public String renderHtml(String id,String basePath,String input)
{
	...
	String uc_api =URLEncoder.encode(basePath+"camera/camera.jsp");
...
if(action==null){
	...
	function updateavatar() {
		var img1="<%=imagepath1%>";
		parent.$('tr[id=menberPic]').show();
		parent.document.getElementById("memberPicID").src="memberPicTmp/"+img1+"?t="+Math.random();
		parent.document.getElementById("memberPicPath").value="memberPicTmp/"+img1;		
	}	
	</script>
	</head>
	<body>
	...
	<%
		...
		for (Iterator iter = map.keySet().iterator(); iter.hasNext();)
		{
			...
			out.clear();
			out.print(basePath+"memberPicTmp"+infoFilePath);
		}
	...
	String pathff = request.getSession().getServletContext().getRealPath("/")+ "memberPicTmp\\";
