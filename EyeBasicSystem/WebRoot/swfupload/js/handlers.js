...
var picHeight ="";
var actionUrl ="";
var updateOrInsert ="update";
var fixid = "";
function initSwfupload(url,sizeLimit,types,typsdesc,uploadLimit,picIDStr,picDivIDStr,picWidthStr,picHeightStr,updateOrInsertStr,paramid){
		actionUrl = url;
		picID = $.trim(picIDStr);
		fixid = $.trim(paramid);
		picDivID = $.trim(picDivIDStr);
		if($.trim(picWidthStr)!=""){picWidth = picWidthStr;}
		if($.trim(picHeightStr)!=""){picHeight = picHeightStr;}
		if($.trim(updateOrInsertStr)!=""){updateOrInsert = updateOrInsertStr;}
		//初始化界面
		...
		var settings={
			...
			flash_url : contextPath+"/swfupload/js/swfupload.swf",
			...
			// Button Settings
			button_image_url : contextPath+"/swfupload/images/select-files-zh_CN.png",
...
function insertAllevent(){
	...
	$("img[data-status='0']").each(function(i,obj){
			...
			var shortUrl = url.substring(contextPath.length) + (fixid == "" ? "" : "_") + fixid;
			picObj.value = picObj.value + shortUrl +",";//添加图片路径到Input
			
			if(picDivID!=""){//图片预览
				if(picHeight==""){
					picDiv.innerHTML=picDiv.innerHTML + "<p><img src='"+ url +"' border='0' width='"+ picWidth +"' title='点击查看大图' width2='600' height2='400' onclick='imgclick(this)' style='cursor: hand;'/><a style='cursor:hand' onclick=deleteServerFile(this,'"+ shortUrl +"',picID);>删除</a></p>";
				}else{
					picDiv.innerHTML=picDiv.innerHTML + "<p><img src='"+ url +"' border='0' width='"+ picWidth +"' height='"+ picHeight +"' title='点击查看大图' width2='600' height2='400' onclick='imgclick(this)' style='cursor: hand;'/><a style='cursor:hand' onclick=deleteServerFile(this,'"+ shortUrl +"',picID);>删除</a></p>";
				}				
			}			
...
function addReadyFileInfo(fileid,fileName,message){
	var photoDiv='<div class="ke-inline-block ke-item" id="'+fileid+'">'
			...
			+'	<img src="'+contextPath+'/swfupload/images/image.png" class="ke-img" data-status="-1" width="80" height="80" />'
