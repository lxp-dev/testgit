...
public class ProcurementOrdersStatusDaoImpl extends BaseJdbcDaoSupport
		implements ProcurementOrdersStatusDao {
	 ...
	 * @return
	 */
	public int getProcurementOrdersStatusCount(
		...
		buffer.append("where C_ST_P_BillTypeId = '1'  ");
		...
		} else if (StringUtils.isNotEmpty(procurementOrdersPo
				.getCstpcompanyid())
				&& "".equals(Utility.getName(procurementOrdersPo.getCstpid()))) {
			buffer.append("and C_ST_P_ID like '%'+? + '%' ");//quyanping
		...
		if (!"".equals(Utility.getName(procurementOrdersPo.getCstpsupplierid()))) {
			buffer.append("and ");
			buffer.append("C_ST_P_SupplierID = (select B_SP_OrderSupplierID ");
	 ...
	 * @return
	 */
	public List<ProcurementOrdersPo> selectProcurementOrdersStatus(
		...
		buffer.append("order by C_ST_P_BillDate desc) as rowNum, ");
		...
		buffer.append("from Orders.orders.dbo.C_ST_Po ");
		buffer.append("inner join Orders.orders.dbo.B_Departments on B_DP_DepartmentID = C_ST_P_SupplierID ");
		buffer.append("left outer join Orders.orders.dbo.SYS_PersonInfo on ID = C_ST_P_AuditPerson ");
		buffer.append("where C_ST_P_BillTypeId = '1' ");
		...
		if (!"".equals(Utility.getName(procurementOrdersPo.getCstpid()))) {
			...
			params.add(procurementOrdersPo.getCstpid());
				...
				.getCstpcompanyid())
				&& "".equals(Utility.getName(procurementOrdersPo.getCstpid()))) {
			buffer.append("and C_ST_P_ID like '%'+? + '%' ");
	...
	public CompanyNamePo getCompanyId(CompanyNamePo companyNamePo) {
		...
		return (CompanyNamePo) queryForObject(buffer.toString(), params
				.toArray(), CompanyNamePo.class);
	}
	...
	public ProcurementOrdersPo getProcurementOrderStatus(
		...
		buffer.append("from Orders.orders.dbo.C_ST_Po ");
		buffer
				.append("inner join Orders.orders.dbo.B_Departments on B_DP_DepartmentID = C_ST_P_SupplierID ");
		buffer.append("where C_ST_P_ID = ? ");

		params.add(procurementOrdersPo.getCstpid());

		return (ProcurementOrdersPo) queryForObject(buffer.toString(), params
				.toArray(), ProcurementOrdersPo.class);
	}
	...
	public ProcurementSendPo getProcurementSend(
		...
		return (ProcurementSendPo) queryForObject(buffer.toString(), params
				.toArray(), ProcurementSendPo.class);
	}
