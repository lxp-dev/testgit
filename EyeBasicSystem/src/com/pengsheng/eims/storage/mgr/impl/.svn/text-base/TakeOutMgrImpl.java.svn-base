...
public class TakeOutMgrImpl implements TakeOutMgr {
	...
	public int getSalesOutCount(InventoryPo po) {
		return takeOutDao.getSalesOutCount(po);
	}
	public List<InventoryPo> getSalesOutList(InventoryPo po,
			int start, int size) {
		return takeOutDao.getSalesOutList(po, start, size);
	}
	...
	public void insertSalesOut(InventoryPo po,List<InventoryEntryPo> list,List<AllocationBarcodePo> allocationBarcodeList,LogisticsLogPo logPo) {
		takeOutDao.insertSalesOut(po);// 单据主表
		
		if("1".equals(po.getCstiauditstate())){
			if(!"".equals(po.getCstisourcebillid())){
				takeOutDao.insertAllocationStatus(po);
			}
		}
		...
		while (it.hasNext()) {
			...
			takeOutDao.insertSalesOutEntry(entryPo);// 单据明细表
			...
			if("1".equals(Utility.getName(po.getCstiauditstate()))){
				...
				strogeChangeDao.insertStrogeChangeLog(changePo, entryPo.getCstiebarcode());// 商品库存当月库存变更表
				strogeChangeDao.insertStrogeChangeLogTemp(changePo, entryPo.getCstiebarcode());// 商品库存当月库存变更表
			}
		}
		
		InTransitStorageTypePo inTransitStorageTypePo = inTransitDetailsDao.getInTransitStorageSwitch("lyck"); // lyck 领用出库
		if (Utility.getName(inTransitStorageTypePo.getCshstinenabled()).equals("1")){
			...
			if(!"1".equals(Utility.getName(po.getCstiauditstate()))){
				takeOutDao.insertTakeOutInTransitStorage(po);
			}			
		}
	...
	public void updateSalesOut(InventoryPo po,List<InventoryEntryPo> list,List<AllocationBarcodePo> allocationBarcodeList,LogisticsLogPo logPo) {
		takeOutDao.updateSalesOut(po);//单据主表
		takeOutDao.deleteSalesOutEntry(po);//删除单据明细表
		takeOutDao.deleteGoodsBarCode(po);//删除条码表
	...
	public InventoryPo getSalesOut(InventoryPo po) {
		...
		return takeOutDao.getSalesOut(po);
	}

	public void deleteSalesOut(InventoryPo po,LogisticsLogPo logPo) {

		takeOutDao.deleteSalesOut(po);
		takeOutDao.deleteSalesOutEntry(po);
		takeOutDao.deleteGoodsBarCode(po);
	...
	{
		return takeOutDao.getReallocationList(po);
	}

	public List<InventoryEntryPo> getSalesOutEntryList(InventoryPo po) {
		return this.takeOutDao.getSalesOutEntryList(po);
	}
	/**
	 * 获取业务单据商品条码
	 * @param po
	...
	{
		return takeOutDao.getInventoryBarcode(po);
	}
