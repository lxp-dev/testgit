...
public class WindowOtherGoodsAction extends BaseAction {

	private List<GoodsInfoPo> otherGoodsList;
	
	private WindowOtherGoodsMgr windowOtherGoodsMgr;
	...
	public WindowOtherGoodsMgr getWindowOtherGoodsMgr() {
		return windowOtherGoodsMgr;
	}

	public void setWindowOtherGoodsMgr(WindowOtherGoodsMgr windowOtherGoodsMgr) {
		this.windowOtherGoodsMgr = windowOtherGoodsMgr;
	}
	 ...
	 * @return
	 */
	public String initWindowOtherGoodsSel(){
	 ...
	 * @return
	 */
	public String selectWindowOtherGoods(){
		...
		//查询及分页
		int count = windowOtherGoodsMgr.getOtherGoodsCount(goodsInfoPo);
		if (count > 0) {
			int perPage = 0;
			...
			otherGoodsList = windowOtherGoodsMgr.selectOtherGoodsList(goodsInfoPo , page.getStart() , page.getPageSize());
	 ...
	 * @return
	 */
	public String selectOthersByGoodsbarcode() {
		...
		goodsInfoPo = new GoodsInfoPo();
		goodsInfoPo.setBgiwarehouseid(po.getBwcstockid9());
		goodsInfoPo.setBgigoodsbarcode(bgigoodsbarcode);

		systemParameterPo = (SystemParameterPo) session.get("systemParameterPo");
		int fsppageno = Integer.valueOf(systemParameterPo.getFsppageno());
		// 查询及分页
		int count = windowOtherGoodsMgr
		...
		if (count > 0) {
			...
			Pagination page = new Pagination(request, count, perPage);
			otherGoodsList = windowOtherGoodsMgr
