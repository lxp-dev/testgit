...
public class OptometryCaseHydsyAction extends BaseAction {
	private PersonPermissionMgr personPermissionMgr;
	private OptometryCaseHydsyMgr optometryCaseHydsyMgr;
	...
	public String initOptometryCaseSelN() {
		...
		statusModulePo = procurementOrdersDao.OrdersStatus("4"); //销售门店是否共享验光信息
    	...
    	{
    		departmentsList = departmentsMgr.getDepartmentOne(deppo);
    	}
	...
	public String optometryCaseSelN() {
		...
		deppo.setBdpdepartmentname(personInfoPo.getBdpdepartmentname());
		StatusModulePo statusModulePo =new StatusModulePo();
		statusModulePo = procurementOrdersDao.OrdersStatus("4"); //是否共享会员信息
		...
		if("".equals(Utility.getName(departmentID)))
		{
	    	...
	    	{
	    		departmentsList = departmentsMgr.getDepartmentOne(deppo);
	    		po.setSopoyshopcode(deppo.getBdpdepartmentid());
	    	}
		}
		else
		{
		...
		int count = optometryCaseHydsyMgr.getOptometryCaseCount(po);
		if (count > 0) {
			int perPage = 0;
			...
			optometryPos = optometryCaseHydsyMgr.getOptometryCases(po, page.getStart(),page.getPageSize());
	...
	public OptometryCaseHydsyMgr getOptometryCaseHydsyMgr() {
		return optometryCaseHydsyMgr;
	}

	public void setOptometryCaseHydsyMgr(OptometryCaseHydsyMgr optometryCaseHydsyMgr) {
		this.optometryCaseHydsyMgr = optometryCaseHydsyMgr;
	}
