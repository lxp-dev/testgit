...
public class XLSXCovertCSVReader {
    ...
    public static void main(String[] args) throws Exception {  
    	...
    	String file = "D:\\Program Files\\work\\jboss-5.1.0.GA\\server\\default\\deploy\\eims.war\\upload\\297e7a7057b687740157b6bb15880004\\分门店雷物流商品信息.xlsx";
		...
		for (int k = 0; k < 9; k++){
			strList = XLSXCovertCSVReader.readerExcel(file,wbs.getSheetName(k),9);

	        for (int i = 1; i < strList.size(); i++ ){  // i = 1 除去表头
				GoodsInfoPo po = new GoodsInfoPo();
				po.setBgiaverageflag("2");   // 成本类型				
        ...
        for (int i = 0; i < goodsInfoList.size(); i++ ){
        	System.out.println("商品代码：" + goodsInfoList.get(i).getBgigoodsid() + " 数量： " + goodsInfoList.get(i).getBgigoodsquantity() );   
        }
