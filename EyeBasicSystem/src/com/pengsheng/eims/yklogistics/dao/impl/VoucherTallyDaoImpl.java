...
public class VoucherTallyDaoImpl extends BaseJdbcDaoSupport implements VoucherTallyDao {
	...
	public List<VoucherEntryPo> getBillsAndGoods(VoucherEntryPo po){
		StringBuffer sb = new StringBuffer();
		sb.append("select L_VE_VE_BillID as sLveveBillID,L_VE_VE_GoodsID as sLveveGoodsID,isnull(B_GI_GoodsCategoryID,'') as sLveveGoodsCategoryID,isnull(B_GI_AccessoriesType,'') as sLveveAccessoriesType from L_VE_VoucherEntry left join B_GoodsInfo on L_VE_VE_GoodsID = B_GI_GoodsID where L_VE_VE_VoucherID=?");
	...
	public String getNotTaxRateAmount(String billID){
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1  isnull(sum(C_ST_IE_NotTaxRateAmount),0) from C_ST_InventoryEntry where C_ST_IE_BillID=? "); 
	...
	public String getOtherGoodsNotTaxRateAmount(String billID,String otherGoodsBigClass){
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1 isnull(sum(C_ST_IE_NotTaxRateAmount),0) from C_ST_InventoryEntry inner join B_GoodsInfo on C_ST_IE_GoodsId=B_GI_GoodsID "); 
	...
	public String getNotTaxRateAmount(String voucherID,String typeID){
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1  isnull(sum(L_VE_VE_NotTaxRateAmount),0) from L_VE_VoucherEntry left join B_GoodsInfo on L_VE_VE_GoodsID=B_GI_GoodsID where L_VE_VE_VoucherID=? ");
		...
		if (!"".endsWith(goodsCategoryID)){
			if (goodsCategoryID.length() == 1){
				if (!goodsCategoryID.equalsIgnoreCase("D")){
					...
					params.add(Utility.getName(goodsCategoryID));
				}else{
					sb.append(" and B_GI_OtherGoodsBigClass='D'");
				}				
			}else if (goodsCategoryID.indexOf(",") >= 0){
				sb.append(" and substring(L_VE_VE_GoodsID,1,1) in ( ? ");
				
				String[] array = goodsCategoryID.split(",");
				...
				if (goodsCategoryID.substring(0,1).equals("2")){
					sb.append(" and B_GI_GoodsCategoryID=? and B_GI_AccessoriesType=?");
				}else if (goodsCategoryID.substring(0,1).equals("5")){
					sb.append(" and (B_GI_GoodsCategoryID=? or (B_GI_GoodsCategoryID = '2' and B_GI_AccessoriesType=?)) ");
				}else{
	...
	public VoucherEntryPo getPercentThreeNotTaxRateAmount(VoucherEntryPo po){
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1  isnull(sum(C_ST_IE_NotTaxRateAmount),0) as sNotTaxRateAmount,isnull(sum(C_ST_IE_TaxAmount),0) as sTaxAmount from C_ST_InventoryEntry where C_ST_IE_BillID=? and C_ST_IE_GoodsId=? "); 
	...
	public VoucherEntryPo getPercentThreeReversalNotTaxRateAmount(VoucherEntryPo po){
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1  isnull(sum(L_IE_RE_NotTaxRateAmount),0) as sNotTaxRateAmount from L_RE_ReversalEntry where L_IE_RE_InvoiceID=? and L_IE_RE_GoodsID=? ");
	...
	public VoucherEntryPo getReversalNotTaxRateAmountByBillID(VoucherEntryPo po){
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1  abs(isnull(sum(L_IE_IE_NotTaxRateAmount),0)) as sNotTaxRateAmount,abs(isnull(sum(L_IE_IE_TaxAmount),0)) as sTaxAmount from L_IE_InvoiceEntry where L_IE_IE_BillID=? and L_IE_IE_GoodsID=? ");
	...
	public List<VoucherTallyPo> getVoucherTempletByID(String voucherTypeID){
		...
		sb.append("select L_S_S_ID as sLssID,L_VT_VT_OrderID as sLvtvtOrderID,L_VT_VT_BalanceDirection as sLvtsBalanceDirection,L_S_S_SubjectID as sLvtvtSubjectID,L_S_S_SubjectName as sLvtvtSubjectName,isnull(L_VT_VT_HBConstant,0) as sLvtsHBConstant,isnull(L_VT_VT_isReplace,'0') as sLvtvtIsReplace ");
	...
	public String getSalesValueByID(String billID,String typeID){
		...
		sb.append("select top 1  isnull(sum(C_ST_IE_CostPriceAmount),0) from C_ST_InventoryEntry left join B_GoodsInfo on C_ST_IE_GoodsId = B_GI_GoodsID where C_ST_IE_BillID=? and ( 1=0 ");
		
		params.add(Utility.getName(billID));
		
		if (!"".equals(Utility.getName(typeID))){
			String[] array = Utility.getName(typeID).split(",");
			String[] childarray = null;
			
			for (int i = 0; i < array.length; i++){
				if (Utility.getName(array[i]).indexOf(".") >= 0){
					childarray = Utility.getName(array[i]).split(".");
					
					if (Utility.getName(Utility.getName(array[i]).substring(0,1)).equals("2")){
						sb.append(" or (B_GI_GoodsCategoryID = ? and B_GI_AccessoriesType = ?) ");
						params.add(Utility.getName(array[i].substring(0,array[i].indexOf("."))));
						params.add(Utility.getName(array[i].substring(array[i].indexOf(".")+1,array[i].length())));
					}else if (Utility.getName(Utility.getName(array[i]).substring(0,1)).equals("5")){
						sb.append(" or (B_GI_GoodsCategoryID = ? or (B_GI_GoodsCategoryID = '2' and B_GI_AccessoriesType = ?)) ");
						params.add(Utility.getName(array[i].substring(0,array[i].indexOf("."))));
						params.add(Utility.getName(array[i].substring(array[i].indexOf(".")+1,array[i].length())));
					}else {
						sb.append(" or B_GI_GoodsCategoryID = ? ");
						params.add(Utility.getName(array[i].substring(0,array[i].indexOf("."))));
					}
				}else{
					sb.append(" or B_GI_GoodsCategoryID = ? ");
	...
	public List<SubjectPo> getSubjectList(SubjectPo po,int start,int size){
		...
		sb.append("select ROW_NUMBER() Over(order by L_S_S_ID) as rowNum,L_S_S_ID as sLssID,L_S_S_SubjectID as sLssSubjectID,L_S_S_SubjectName as sLssSubjectName from L_S_Subject where L_S_S_SubjectID is not null and L_S_S_SubjectID<>'' ");		
		if (!"".equals(Utility.getName(po.getsLssSubjectID()))){
			sb.append(" and L_S_S_SubjectID=? ");
	...
	public int getSubjectCount(SubjectPo po){
		...
		sb.append("select count(L_S_S_ID) from L_S_Subject where L_S_S_SubjectID is not null and L_S_S_SubjectID<>'' ");		
		if (!"".equals(Utility.getName(po.getsLssSubjectID()))){
			sb.append(" and L_S_S_SubjectID=? ");
			params.add(po.getsLssSubjectID());
		}
	...
	public String getPanyingkuiNotTaxRateAmount(String billID){
		...
		StringBuffer sb = new StringBuffer();
		sb.append("select top 1  isnull(sum(L_VE_VE_NotTaxRateAmount),0) from L_VE_VoucherEntry where L_VE_VE_BillID = ? "); 
