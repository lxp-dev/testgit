...
public class EyesCheckBjtrAction extends BaseAction {
	...
	public String selEyesCheckN() throws Exception {
		...
		int count = eyesCheckBjtrMgr.getEyesCheckCount(po);
		if (count > 0) {
			int perPage = 0;
			...
			eyesChecklist = eyesCheckBjtrMgr.getEyesCheckList(po, page.getStart(),
					page.getPageSize());
	...
	public String insertEyesCheckN() throws Exception {
		...
		eyesCheckBjtrMgr.insertEyesCheck(eyesCheckPo,logPo);

		this.clearMessages();
		this.addActionMessage(getText("struts.messages.insert.sucess"));
		request.setAttribute("url", "'initEyesCheckInsertBjtr.action?moduleID="+moduleID+"'");
	...
	public String detailEyesCheckN() throws Exception {
		...
		eyesCheckPo = eyesCheckBjtrMgr.getEyesCheck(po);
...
//		systemParameterPo = (SystemParameterPo) session.get("systemParameterPo");
//		int fsppageno = Integer.valueOf(systemParameterPo.getFsppageno());
		int count = eyesCheckBjtrMgr.getEyesChargePutCount(po);
		if (count > 0) {
			int perPage = 0;
			if (request.getParameter("perPage") != null) {
				perPage = new Integer((String) request.getParameter("perPage"))
			...
			Pagination page = new Pagination(request, count, perPage);
			eyeslist = eyesCheckBjtrMgr.getEyesChargePutList(po, page.getStart(),
					page.getPageSize());
			request.setAttribute(Pagination.PAGINATION, page);
		} else {
			eyeslist = null;
