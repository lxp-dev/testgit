 ...
 * 镜架订做片action
 */
public class LensCustomARAction extends BaseAction {
	
	private SystemParameterMgr systemParameterMgr;
	private SystemParameterPo systemParameterPo;
	private String isFirstExec;
	private List<RefractiveSetPo> refractiveSetPos;
	private LensCustomMgr lensCustomMgr;
	...
	private List<GoodsInfoPo> lensCustoms;
	...
	public LensCustomMgr getLensCustomMgr() {
		return lensCustomMgr;
	}

	public void setLensCustomMgr(LensCustomMgr lensCustomMgr) {
		this.lensCustomMgr = lensCustomMgr;
	}
	...
	public List<GoodsInfoPo> getLensCustoms() {
		return lensCustoms;
	}

	public void setLensCustoms(List<GoodsInfoPo> lensCustoms) {
		this.lensCustoms = lensCustoms;
	}
	 ...
	 * @throws Exception
	 */
	public String initLensCustomSelAR() throws Exception {
	 ...
	 * 查询订做镜片类
	 */
	public String selLensCustomAR() throws Exception {
		...
		goodsInfoPo.setMaxSph(maxSphjp);
		goodsInfoPo.setMinSph(minSphjp);
		goodsInfoPo.setMaxCyl(maxCyljp);
		goodsInfoPo.setMinCyl(minCyljp);
		...
		int count = lensCustomMgr.getLensCustomCount(goodsInfoPo);
		if (count > 0) {
			int perPage = 0;
			...
			lensCustoms = lensCustomMgr.getLensCustomList(goodsInfoPo, page
	 ...
	 * 初始化订做镜片类新增
	 */
	public String initLensCustomInsertAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
	 ...
	 * 新增订做镜片类
	 */
	public String insertLensCustomAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
		...
		String parent = Utility.getName(request.getParameter("parent"));
		String goodsTree = Utility.getName(request.getParameter("goodsTree"));
		GoodsInfoPo po = lensCustomMgr.getLensCustom(goodsInfoPo);
		...
		logPo.setsLogContent("订做镜片:" + goodsInfoPo.getBgigoodsid() + "新增!");
		this.clearMessages();
		if (po.getBgigoodsid() == null) {
			if(po1.getBgigoodsbarcode()==null)
			{
				...
				lensCustomMgr.insertLensCustom(goodsInfoPo,logPo);
	 ...
	 * 初始化订做镜片类修改
	 */
	public String initLensCustomUpdateAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
		...
		goodsInfoPo = lensCustomMgr.getLensCustom(goodsInfoPo);
		unitList = unitMgr.getUnitList();
	 ...
	 * 初始化订做镜片类详细
	 */
	public String initLensCustomDetailsAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
	 ...
	 * 修改订做镜片类
	 */
	public String updateLensCustomAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
		...
		selectGoodsLevelList = brandMgr.selectGoodsLevelList(null);
		systemParameterPo = (SystemParameterPo) session.get("systemParameterPo");
		GoodsInfoPo vgoodspo = lensCustomMgr.getLensCustom(goodsInfoPo);
		String[] goodsnametype = systemParameterPo.getFspcjp().trim().split(",");
		String viewgoodsname = goodsInfoPo.getBgigoodsname();
		for(int i=0; i<goodsnametype.length; i++){
			if(goodsnametype[i].trim().equals("B_GI_Spec")){
				...
				if(!vgoodspo.getBgiiscustomize().trim().equals("D")){
		...
		logPo.setsLogContent("订做镜片:" + goodsInfoPo.getBgigoodsid() + "修改!");
		lensCustomMgr.updateLensCustom(goodsInfoPo,logPo);
		this.clearMessages();
	 ...
	 * 初始化订做镜片类删除
	 */
	public String initLensCustomDeleteAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
	 ...
	 * 删除订做镜片类
	 */
	public String deleteLensCustomAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
		...
		logPo.setsLogContent("订做镜片:" + goodsInfoPo.getBgigoodsid() + "删除!");
		
		int flag = lensFinishedMgr.getGoodsCount(goodsInfoPo);
		this.clearMessages();
		if (flag == 0) {
			lensCustomMgr.deleteLensCustom(goodsInfoPo,logPo);
	 ...
	 * 初始化订做镜片类停用
	 */
	public String initLensCustomDisableAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
	 ...
	 * 停用订做镜片类
	 */
	public String disableLensCustomAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
		...
		logPo.setsLogContent("订做镜片:" + goodsInfoPo.getBgigoodsid() + "停用!");
		lensCustomMgr.updateLendsCustomDisable(goodsInfoPo,logPo);
	 ...
	 * 初始化订做镜片类启用
	 */
	public String initLensCustomAbleAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
	 ...
	 * 启用订做镜片类
	 */
	public String ableLensCustomAR() throws Exception {
		/** ********************** 权限设置 ***************************** */
		String moduleID = Utility.getName(request.getParameter("moduleID"));
		request.setAttribute("moduleID", moduleID);
		PersonInfoPo personInfoPo = (PersonInfoPo) session.get("person");
		...
		logPo.setsLogContent("订做镜片:" + goodsInfoPo.getBgigoodsid() + "启用!");
