 ...
 * dao 实现类
 */
public class UpdateGoodsAttributeDaoImpl extends BaseJdbcDaoSupport implements
		UpdateGoodsAttributeDao {
	...
	public void updateGoodsAttribute(GoodsInfoPo po) {
		...
		if("4D".equals(Utility.getName(po.getBgigoodscategoryid()))){
			if(!"".equals(Utility.getName(po.getBgiusetype()))){
				sb.append("    B_GI_UseType = ?, ");
				params.add(po.getBgiusetype());
			}
