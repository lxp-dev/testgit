...
//			e.printStackTrace();
	...
	public static void main(String[] args) {
		   //明文加密：
		   String key = "mysflying"; //初始化密钥。
		   Encrypt encrypt = new Encrypt();
		   encrypt.setKey(key);    //调用set函数设置密钥。
		   encrypt.setEncString("000001 2099-01-01");//将要加密的明文传送给Encrypt.java进行加密计算。
		   String Mi=encrypt.getStrMi();  //调用get函数获取加密后密文。
		   System.out.println(Mi);
		   
		   encrypt.setDesString(Mi);   //将要解密的密文传送给Encrypt.java进行解密计算。
		   String M=encrypt.getStrM(); //调用get函数获取解密后明文。
